...
from ucx_kb.rag.models import HybridContext, SearchResult
...
class TestGetHybridContext:
    ...
    @patch("ucx_kb.rag.hybrid.get_similar_analyses")
    @patch("ucx_kb.rag.hybrid.semantic_search")
    @patch("ucx_kb.rag.hybrid.get_learnings_for_topic")
    def test_combines_sources(self, mock_learnings, mock_search, mock_similar):
        ...
        with patch("ucx_kb.graph.layer.TradingGraph") as mock_graph:
...
class TestBuildAnalysisContext:
    ...
    @patch("ucx_kb.rag.hybrid.get_hybrid_context")
