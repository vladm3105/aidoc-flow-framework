...
def mock_neo4j():
    ...
    with patch("ucx_kb.graph.layer.GraphDatabase") as mock_db:
...
def mock_ollama():
    ...
    with patch("ucx_kb.graph.extract._call_ollama_rate_limited") as mock:
        mock.return_value = "[]"
        yield mock
...
def mock_config():
    ...
    with patch("ucx_kb.graph.extract._config", config):
        yield config
...
def test_client():
    ...
    from ucx_kb.graph.webhook import app
...
def mock_trading_graph():
    ...
    with patch("ucx_kb.graph.webhook.TradingGraph") as mock_class:
...
def mock_extract_document():
    ...
    with patch("ucx_kb.graph.webhook.extract_document") as mock:
        yield mock
...
def mock_extract_text():
    ...
    with patch("ucx_kb.graph.webhook.extract_text") as mock:
        yield mock
