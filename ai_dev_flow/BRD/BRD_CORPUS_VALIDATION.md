...
if git diff --cached --name-only | grep -q "^docs/BRD/"; then
...
# Check file sizes
wc -l docs/BRD/BRD-[0-9]*_*.md | sort -n
```
...
ls docs/BRD/BRD-07_*.md && \
  sed -i 's/| BRD-07 .* Planned |/| BRD-07 | AI Gateway | Complete | 1.0 |/' docs/BRD/BRD-000_index.md
```
...
for f in docs/BRD/BRD-[0-9]*_*.md; do
  lines=$(wc -l < "$f")
