...
**Tag Format:** `@brd: BRD-001:REQ-ID` (for multi-requirement docs) or `@brd: BRD-001` (for single-doc refs)
...
4. **Tag Discovery**: All downstream artifacts MUST include `@brd: BRD-NNN:REQ-ID` tags referencing this BRD
...
Strategy (External) → BRD (Layer 1)
                  ...
                  Downstream Artifacts Tag with @brd:
                        ↓
            PRD (Layer 2): @brd: BRD-009:FR-015
                        ↓
         EARS (Layer 3): @brd: BRD-009:FR-015, @prd: PRD-016...
                        ↓
         [Complete chain through Code and Tests]
```

**Key Principle**: While BRD has no upstream tags, **every downstream artifact** from PRD through Tests MUST include `@brd` tags, creating complete end-to-end traceability from business requirements to production code.
