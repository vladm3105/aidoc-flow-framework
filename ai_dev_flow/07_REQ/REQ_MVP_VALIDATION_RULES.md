...
**Purpose**: Validate Section 9.5 deployment requirements exist for automated generation
**Type**: Warning + Info

**Required Subsections** (8 total):
```markdown
...
| Rule | Type | Check |
|------|-------|--------|
| Infrastructure includes compute and database | Warning | Table has compute and database rows |
| Cloud provider referenced via @adr | Warning | Section 9.5.1 references @adr |
| All 6 deployment scripts specified | Info | Section 9.5.3 table has 6 scripts |
| All 6 Ansible playbooks specified | Info | Section 9.5.4 table has 6 playbooks |
| Observability includes logging/metrics/tracing | Info | Section 9.5.5 has all 3 types |
| Security includes secrets management | Info | Section 9.5.6 has secrets row |
| Cost constraints have budget values | Info | Section 9.5.7 has budget amounts |

**Error Messages**:
```
⚠️  WARNING: Deployment Requirements section missing (Section 9.5)
⚠️  WARNING: Deployment scripts incomplete (missing 1+ of 6 required scripts)
⚠️  WARNING: Ansible playbooks incomplete (missing 1+ of 6 required playbooks)
⚠️  WARNING: Infrastructure requirements missing compute or database
```

**Fix**:
1. Add Section 9.5 with all 8 subsections using REQ-MVP-TEMPLATE.md
2. Ensure Section 9.5.1 Infrastructure Requirements table has compute and database rows
3. Ensure Section 9.5.3 Deployment Scripts table has all 6 scripts
4. Ensure Section 9.5.4 Ansible Playbook table has all 6 playbooks
5. Reference cloud provider decisions using @adr tags

**Reference**: `REQ-MVP-TEMPLATE.md` Section 9.5 (Deployment Requirements)

---

### CHECK 22: Downstream Deployment Artifacts Traceability ⭐ NEW

**Purpose**: Verify downstream artifacts table includes deployment scripts, Ansible playbooks, IaC templates
**Type**: Warning

**Required Artifacts**:
```markdown
#### Deployment Artifacts (Generated by Autopilot)
- Shell Scripts: setup.sh, install.sh, deploy.sh, rollback.sh, health-check.sh, cleanup.sh
- Ansible Playbooks: provision_infra.yml, configure_instances.yml, deploy_app.yml, configure_monitoring.yml, configure_security.yml, backup_restore.yml
- IaC Templates: terraform/ or cloudformation/
- Docker Config: Dockerfile, docker-compose.yml
- CI/CD Pipeline: .github/workflows/deploy.yml
```

**Validation Check**:
- Section 10.2 Downstream Artifacts includes "#### Deployment Artifacts (Generated by Autopilot)" subsection
- All 6 shell scripts listed with TBD status
- All 6 Ansible playbooks listed with TBD status
- IaC templates referenced with correct format

**Error Message**:
```
⚠️  WARNING: Downstream artifacts table missing deployment artifacts section
⚠️  WARNING: Deployment scripts table incomplete
⚠️  WARNING: Ansible playbooks table incomplete
```

**Fix**:
1. Add "#### Deployment Artifacts (Generated by Autopilot)" subsection to Section 10.2
2. List all 6 shell scripts with TBD status
3. List all 6 Ansible playbooks with TBD status
4. Add IaC templates, Docker config, and CI/CD pipeline rows

**Reference**: `REQ-MVP-TEMPLATE.md` Section 10.2 (Downstream Artifacts)
