...
# ============================================
# CHECK 21: Deployment Requirements Section ⭐ NEW
# ============================================
echo "CHECK 21: Deployment Requirements Section"
echo "-----------------------------------------"

# Check for Section 9.5
if ! grep -q "^## 9.5 Deployment Requirements" "$REQ_FILE"; then
  echo "  ℹ️  INFO: Section 9.5 Deployment Requirements not found (optional for simple deployments)"
else
  echo "  ✅ Found: ## 9.5 Deployment Requirements"
  
  # Check for Deployment Complexity field
  if grep -q "| **Deployment Complexity** |" "$REQ_FILE"; then
    echo "  ✅ Deployment Complexity field found in Document Control"
  else
    echo "  ⚠️  WARNING: Deployment Complexity field missing from Document Control"
    ((WARNINGS++))
  fi
  
  # Check for Tier Selection
  if grep -q "^### 9.5.0 Deployment Tier Selection" "$REQ_FILE"; then
    echo "  ✅ Section 9.5.0 Deployment Tier Selection found"
  else
    echo "  ⚠️  WARNING: Section 9.5.0 Deployment Tier Selection missing"
    ((WARNINGS++))
  fi
  
  # Check for Infrastructure Requirements
  if grep -q "^### 9.5.1 Infrastructure Requirements" "$REQ_FILE"; then
    echo "  ✅ Section 9.5.1 Infrastructure Requirements found"
  else
    echo "  ⚠️  WARNING: Section 9.5.1 Infrastructure Requirements missing"
    ((WARNINGS++))
  fi
  
  # Check for Environment Configuration
  if grep -q "^### 9.5.2 Environment Configuration" "$REQ_FILE"; then
    echo "  ✅ Section 9.5.2 Environment Configuration found"
  else
    echo "  ⚠️  WARNING: Section 9.5.2 Environment Configuration missing"
    ((WARNINGS++))
  fi
  
  # Check for Deployment Scripts Requirements
  if grep -q "^### 9.5.3 Deployment Scripts Requirements" "$REQ_FILE"; then
    echo "  ✅ Section 9.5.3 Deployment Scripts Requirements found"
  else
    echo "  ⚠️  WARNING: Section 9.5.3 Deployment Scripts Requirements missing"
    ((WARNINGS++))
  fi
  
  # Check for Ansible Playbook Requirements
  if grep -q "^### 9.5.4 Ansible Playbook Requirements" "$REQ_FILE"; then
    echo "  ✅ Section 9.5.4 Ansible Playbook Requirements found"
  else
    echo "  ⚠️  WARNING: Section 9.5.4 Ansible Playbook Requirements missing"
    ((WARNINGS++))
  fi
fi
...
# ============================================
# CHECK 22: Downstream Deployment Artifacts Traceability ⭐ NEW
# ============================================
echo "CHECK 22: Downstream Deployment Artifacts Traceability"
echo "-----------------------------------------"

# Check for Section 10.2 Downstream Artifacts with deployment subsection
if grep -q "^### 10.2 Downstream Artifacts" "$REQ_FILE"; then
  echo "  ✅ Section 10.2 Downstream Artifacts found"
  
  # Check for deployment artifacts subsection
  if grep -q "^#### Deployment Artifacts (Generated by Autopilot)" "$REQ_FILE"; then
    echo "  ✅ Deployment Artifacts subsection found"
  else
    echo "  ⚠️  WARNING: Deployment Artifacts subsection missing from Section 10.2"
    ((WARNINGS++))
  fi
  
  # Check for shell scripts list
  if grep -qE "(scripts/(setup\.sh|install\.sh|deploy\.sh|rollback\.sh|health-check\.sh|cleanup\.sh))" "$REQ_FILE"; then
    echo "  ✅ Shell scripts list found in downstream artifacts"
  else
    echo "  ⚠️  WARNING: Shell scripts list missing from downstream artifacts"
    ((WARNINGS++))
  fi
  
  # Check for Ansible playbooks list
  if grep -qE "(ansible/(provision_infra\.yml|configure_instances\.yml|deploy_app\.yml|configure_monitoring\.yml|configure_security\.yml|backup_restore\.yml))" "$REQ_FILE"; then
    echo "  ✅ Ansible playbooks list found in downstream artifacts"
  else
    echo "  ⚠️  WARNING: Ansible playbooks list missing from downstream artifacts"
    ((WARNINGS++))
  fi
  
  # Check for IaC templates reference
  if grep -qE "(terraform/|cloudformation/)" "$REQ_FILE"; then
    echo "  ✅ IaC templates reference found in downstream artifacts"
  else
    echo "  ⚠️  WARNING: IaC templates reference missing from downstream artifacts"
    ((WARNINGS++))
  fi
  
  # Check for downstream traceability tags
  if grep -qE "@deployment:|@ansible:|@iac:" "$REQ_FILE"; then
    echo "  ✅ Downstream traceability tags found in Section 10.3"
  else
    echo "  ⚠️  WARNING: Downstream traceability tags missing from Section 10.3"
    ((WARNINGS++))
  fi
  
else
  echo "  ⚠️  WARNING: Section 10.2 Downstream Artifacts missing"
  ((WARNINGS++))
fi
