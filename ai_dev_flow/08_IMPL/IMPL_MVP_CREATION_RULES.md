...
| Deliverables | Yes | List of 09_CTR/10_SPEC/TASKS to produce |

### 3.3 Mandatory Sections (IMPL-TEMPLATE.md Structure)

| Section | Purpose |
|---------|---------|
| ## 1. Document Control | Metadata table with IMPL ID, status, dates, related REQs |
| ## 2. Position in Document Workflow | Workflow diagram and scope distinction |
| ## 3. Project Context and Strategy (PART 1) | Overview, objectives, scope, dependencies |
| ## 4. Phased Implementation (PART 2) | Work breakdown by phases with deliverables |
| ## 5. Project Management and Risk (PART 3) | Resources, timeline, risk register |
| ## 6. Tracking and Completion (PART 4) | Deliverables checklist, validation, sign-off |
| ## 7. Traceability | Upstream and downstream references |
| ## 8. References | Internal and external document links |
| ## 9. Template Instructions | Usage guidance (template files only) |
...
> ⚠️ Bad: Uses old Part-only format. New format requires sequential section numbers with Part label in parentheses.
...
```markdown
### IMPL.01.29.01: Phase 1 - Core Risk Engine
...
> **Element ID Format**: `IMPL.{DOC_NUM}.29.{SEQ}` where 29 = Implementation Phase element type code.
...
> ⚠️ Bad: Missing element ID, no structured attributes, no deliverables listed.
...
**Project Management Risks Only** (technical risks go in 05_ADR/SPEC):
...
**IMPL.01.29.01 Deliverables** (Phase 1):
- [ ] CTR-03: Risk API Contract created
- [ ] SPEC-03: Risk Engine Specification created
- [ ] TASKS-03: Risk Engine Tasks created

**IMPL.01.29.02 Deliverables** (Phase 2):
- [ ] CTR-04: Limits API Contract created
- [ ] SPEC-04: Limits Service Specification created
- [ ] TASKS-04: Limits Service Tasks created
```

> **Note**: Section 6 = PART 4 (Tracking and Completion) in the new template structure.
...
IMPL produces (as deliverables) with traceability tags:
- `@ctr: CTR.NN.EE.SS` - API contracts
...
- **WHAT**: Deliverables (09_CTR/10_SPEC/TASKS documents)
...
- **Tests**: Test specifications → 04_BDD/TASKS
...
4. **Missing deliverables** - Always list 09_CTR/10_SPEC/TASKS IDs
...
python scripts/validate_cross_document.py --document docs/08_IMPL/IMPL-NN_slug.md --auto-fix
...
**Blocking**: YES - Cannot proceed to 09_CTR/SPEC creation until Phase 1 validation passes with 0 errors.
