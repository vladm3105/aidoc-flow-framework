#!/bin/bash
# CTR Validation Helper - Run all CTR validators on a file or directory
...
Run all CTR validation tools on a file or directory.
...
OPTIONS:
    ...
    --skip-ids              Skip CTR IDs validator

EXAMPLES:
    # Single file (all validators)
    $0 --file /path/to/CTR-01_iam.md

    # Directory (all validators)
    $0 --directory /path/to/08_CTR

    # Custom SPEC-ready threshold
    $0 --directory /path/to/08_CTR --min-score 85

    # Skip template check (for directory validation)
    $0 --directory /path/to/08_CTR --skip-template
EOF
    exit 0
}
...
echo -e "${BLUE}╔══════════════════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║              CTR VALIDATION SUITE                                ║${NC}"
echo -e "${BLUE}╚══════════════════════════════════════════════════════════════════╝${NC}"
echo ""
...
if [[ "$MODE" == "directory" ]] && [[ "$SKIP_QUALITY" == false ]]; then
    run_validator "Quality Gate Validator" \
        "\"$SCRIPT_DIR/validate_ctr_quality_score.sh\" \"$TARGET\""
    echo ""
fi
...
if [[ "$SKIP_SPEC" == false ]]; then
    if [[ "$MODE" == "file" ]]; then
        run_validator "SPEC-Readiness Validator" \
            "python3 \"$SCRIPT_DIR/validate_ctr_spec_readiness.py\" --ctr-file \"$TARGET\" --min-score $MIN_SCORE"
    else
        run_validator "SPEC-Readiness Validator" \
            "python3 \"$SCRIPT_DIR/validate_ctr_spec_readiness.py\" --directory \"$TARGET\" --min-score $MIN_SCORE"
    fi
    echo ""
fi
...
if [[ "$MODE" == "file" ]] && [[ "$SKIP_TEMPLATE" == false ]]; then
    run_validator "Template Compliance Checker" \
        "\"$SCRIPT_DIR/validate_ctr.sh\" \"$TARGET\""
    echo ""
fi
...
if [[ "$SKIP_IDS" == false ]]; then
    if [[ "$MODE" == "file" ]]; then
        run_validator "CTR IDs Validator" \
            "python3 \"$SCRIPT_DIR/validate_ctr_ids.py\" --ctr-file \"$TARGET\""
    else
        run_validator "CTR IDs Validator" \
            "python3 \"$SCRIPT_DIR/validate_ctr_ids.py\" --directory \"$TARGET\""
    fi
    echo ""
fi
