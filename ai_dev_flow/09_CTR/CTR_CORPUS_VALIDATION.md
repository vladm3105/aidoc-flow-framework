...
| Field | Value |
|-------|-------|
| Document ID | CTR_CORPUS_VALIDATION |
| Version | 1.0 |
| Created | 2026-01-04 |
| Purpose | Quality gate for complete CTR corpus |
| Trigger | Run after ALL CTR files are complete |
| Scope | Entire CTR corpus validation |
| Layer | Layer 9 (Optional) → Layer 10 transition gate |
...
All CTR Complete (Optional Layer)
        ↓
CTR_CORPUS_VALIDATION.md (corpus-level) ← Quality Gate
        ↓
PASS → Begin SPEC Creation (Layer 10)
...
| `IPLAN-NN` | 12 | IPLANs don't exist during CTR creation |
...
# CTR corpus validation on staged CTR files
if git diff --cached --name-only | grep -qE "^docs/09_CTR/.*\.(md|yaml|yml)$"; then
  echo "Running CTR corpus validation..."
  ./scripts/validate_ctr_corpus.sh docs/CTR --errors-only
  if [ $? -ne 0 ]; then
    echo "❌ CTR corpus validation failed. Fix errors before committing."
    exit 1
  fi
  echo "✓ CTR corpus validation passed"
fi
```
...
CTR corpus validation should pass before creating SPEC documents (when CTR layer is enabled):
...
- [CTR-TEMPLATE.md](./CTR-TEMPLATE.md) - CTR document template
- [CTR-TEMPLATE.yaml](./CTR-TEMPLATE.yaml) - CTR schema template
