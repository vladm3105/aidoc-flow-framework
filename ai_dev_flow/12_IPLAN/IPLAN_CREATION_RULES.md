...
# =============================================================================
---
title: "IPLAN Creation Rules [DEPRECATED]"
tags:
  - creation-rules
  - layer-12-artifact
  - shared-architecture
...
# IPLAN Creation Rules [DEPRECATED]

## Index-Only Generation Workflow

- Maintain `IPLAN-00_index.md` as the authoritative source of planned and active IPLAN files (mark planned items with Status: Planned).
- Generators use: `IPLAN-00_index.md` + selected template profile (MVP by default; full when explicitly requested in settings or prompt).
...
- **Content**:
  ...
  - Status Tracking Table for all 11_TASKS/Services
...
./scripts/validate_iplan.sh 12_IPLAN/IPLAN-NN_*.md

# Quick check for required tag
grep -q "layer-12-artifact" docs/12_IPLAN/IPLAN-NN_*.md || echo "ERROR: Missing layer-12-artifact tag"

# Quick check for tags section
grep -q "^tags:" docs/12_IPLAN/IPLAN-NN_*.md || echo "ERROR: Missing tags: section"
```
...
python scripts/validate_cross_document.py --document docs/12_IPLAN/IPLAN-NN_slug.md --auto-fix
