...
| Field | Value |
|-------|-------|
| Document ID | REQ_CORPUS_VALIDATION |
| Version | 1.0 |
| Created | 2026-01-04 |
...
| `IPLAN-NN` | 12 | IPLANs don't exist during REQ creation |
...
# REQ corpus validation on staged REQ files
if git diff --cached --name-only | grep -q "^docs/REQ/"; then
  echo "Running REQ corpus validation..."
  ./scripts/validate_req_corpus.sh docs/REQ --errors-only
  if [ $? -ne 0 ]; then
    echo "❌ REQ corpus validation failed. Fix errors before committing."
    exit 1
  fi
  echo "✓ REQ corpus validation passed"
fi
```
...
REQ corpus validation should pass before creating SPEC documents:
