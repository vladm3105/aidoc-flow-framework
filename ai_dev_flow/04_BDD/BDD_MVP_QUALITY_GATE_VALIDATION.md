---
title: "BDD Quality Gate Validation"
tags:
...
# BDD Quality Gate Validation Rules
...
| **Quality Gate Validation** | After ALL BDD complete | Entire BDD set | This document |
...
This section tracks common errors discovered during Quality Gate validation. Populate during actual validation runs.
...
```bash
#!/bin/bash
# BDD Quality Gate Validation Script
...
echo "=== BDD Quality Gate Validation ==="
...
for suite in 01 02 03 04 05 06 07 08 09 10; do
  count=$(grep -h "Scenario:" docs/04_BDD/BDD-${suite}_*/*.feature 2>/dev/null | wc -l)
  files=$(ls docs/04_BDD/BDD-${suite}_*/*.feature 2>/dev/null | wc -l)
...
name: BDD Quality Gate Validation
...
# BDD Quality Gate validation on staged BDD files
if git diff --cached --name-only | grep -qE "^docs/04_BDD/.*\.(md|feature)$"; then
  echo "Running BDD Quality Gate validation..."
  ./scripts/validate_bdd_corpus.sh docs/BDD --errors-only
  if [ $? -ne 0 ]; then
    echo "BDD Quality Gate validation failed. Fix errors before committing."
    exit 1
  fi
  echo "BDD Quality Gate validation passed"
fi
```
...
BDD Quality Gate validation should pass before creating ADR documents:
