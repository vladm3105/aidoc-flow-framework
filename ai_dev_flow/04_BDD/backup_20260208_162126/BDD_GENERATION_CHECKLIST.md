...
- [ ] Create suite folder: `docs/04_BDD/BDD-NN_{slug}/`
- [ ] Create `docs/04_BDD/BDD-NN_{slug}/BDD-NN.0_index.md` from `BDD-SECTION-0-TEMPLATE.md`
- [ ] Update YAML frontmatter:
  - [ ] Set `title` to actual suite title
  - [ ] Update `parent_doc` reference (if applicable)
  - [ ] Update `upstream_artifacts` with 01_BRD/02_PRD/EARS IDs
...
4. **Requirement Groups** (03_EARS/PRD Alignment)
...
rg -n "WITHIN\s+[0-9]+\s+(seconds?|minutes?)" docs/04_BDD/BDD-NN_*/BDD-NN*.feature
rg -n "after\s+[0-9]+\s+(attempts?|retries?)" docs/04_BDD/BDD-NN_*/BDD-NN*.feature
```
...
**File Organization**:
- [ ] All `.feature` files are inside suite folder: `docs/04_BDD/BDD-NN_{slug}/`
...
- [ ] Create `docs/04_BDD/archive/` subdirectory
...
```bash
git add docs/04_BDD/BDD-NN_*/BDD-NN.0_index.md
git add docs/04_BDD/BDD-NN_*/BDD-NN.*.feature
