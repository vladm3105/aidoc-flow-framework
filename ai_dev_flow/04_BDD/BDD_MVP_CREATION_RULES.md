...
- **Location**: `docs/04_BDD/BDD-NN_{descriptive_slug}/` within project docs directory
...
### Purpose
Unify BDD naming with 02_PRD/BRD section-based standards. All BDD files use section-based numbering (dot notation) with a nested suite folder structure at `docs/04_BDD/BDD-NN_{slug}/`.
...
**Key Rules**:
- ALL `.feature` files inside `docs/04_BDD/BDD-NN_{slug}/` suite folder
...
#### 2. Requirement Groups (03_EARS/PRD Alignment)
...
python scripts/validate_cross_document.py --document docs/04_BDD/BDD-NN_slug.md --auto-fix
