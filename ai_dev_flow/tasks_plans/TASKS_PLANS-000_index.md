# TASKS_PLANS-000: Implementation Work Plans Master Index

## Purpose

This document serves as the master index for all Implementation Work Plans (tasks_plans) in the project. Use this index to:
...
flowchart LR
    ...
    TASKS --> TP[Layer 12: tasks_plans<br/>Impl Work Plans]
...
|----|-------|--------|--------------|---------------|-----------|------------------|---------|-----------|------|
| *No tasks_plans documents yet* | - | - | - | - | - | - | - | - | - |

### Example Entry (for reference):
```markdown
| TASKS_PLANS-001 | Implement Gateway Connection | In Progress | TASKS-001 | ✅ 9/9 tags | Gateway Service | 80 hours | 2025-11-12 | - | [Link](./TASKS_PLANS-001_implement_gateway_connection_20251112_092843.md) |
```
...
|--------|---------|-------------|
| **Draft** | Planning phase | tasks_plans document created, not started |
...
**Layer 12 Position**: All tasks_plans MUST include these tags:
...
python /opt/data/docs_flow_framework/ai_dev_flow/scripts/validate_tags_against_docs.py \
  --file tasks_plans/TASKS_PLANS-NNN_*.md
```

## Adding New Implementation Work Plans

When creating a new tasks_plans document:

1. **Copy Template**:
   ```bash
   cp /opt/data/docs_flow_framework/ai_dev_flow/tasks_plans/TASKS_PLANS-TEMPLATE.md \
      /opt/data/docs_flow_framework/ai_dev_flow/tasks_plans/TASKS_PLANS-NNN_descriptive_name_YYYYMMDD_HHMMSS.md
   ```

2. **Assign ID**: Use next sequential number (TASKS_PLANS-001, TASKS_PLANS-002, ...)
...
5. **Validate Tags**:
   ```bash
   python scripts/validate_tags_against_docs.py --file tasks_plans/TASKS_PLANS-NNN_*.md
   ```

6. **Update Traceability Matrix**:
   ```bash
   python scripts/update_traceability_matrix.py \
     --artifact tasks_plans/TASKS_PLANS-NNN_*.md \
...
- **One Plan Per File**: Each `TASKS_PLANS-NNN` file covers one implementation session or phase
...
| Parent TASKS | tasks_plans Documents | Count | Combined Effort |
|--------------|----------------------|-------|-----------------|
| *No parent TASKS yet* | - | 0 | 0 hours |

### Example:
```markdown
| TASKS-001 | TASKS_PLANS-001, TASKS_PLANS-002 | 2 | 160 hours |
```

## Index by Component Type

| Component Type | tasks_plans Documents | Count |
|----------------|----------------------|-------|
...
|--------|-------|-------------|
| Total tasks_plans | 0 | Total implementation work plans created |
...
Use this section to track which tasks_plans are suitable for resumption:
...
### Example:
```markdown
| TASKS_PLANS-001 | 2025-11-12 14:30 | Phase 2.3 (50%) | TASK-2.3: Protocol interface | None | ✅ Yes |
```

## Related Documents

- **Template**: [TASKS_PLANS-TEMPLATE.md](./TASKS_PLANS-TEMPLATE.md) - Use this to create new implementation work plans
- **README**: [README.md](./README.md) - Learn about tasks_plans purpose, structure, and cumulative tagging
- **Traceability Matrix**: [TASKS_PLANS-000_TRACEABILITY_MATRIX.md](./TASKS_PLANS-000_TRACEABILITY_MATRIX.md) - Complete traceability tracking
...
**Question**: "Which tasks_plans implements TASKS-001?"
...
**Question**: "Are all tags present in TASKS_PLANS-001?"
...
**Answer**: Check "Estimated Effort" column for relevant tasks_plans → See "Index by Component Type" for totals
...
All tasks_plans in this index should pass these validations:

```bash
# Validate all tasks_plans have required tags
...
python scripts/generate_traceability_matrix.py --artifact-type tasks_plans
...
```mermaid
flowchart TB
    Start([Parent TASKS Complete]) --> Create[Create tasks_plans from Template]
...
Before adding a tasks_plans to this index, verify:
...
- [ ] Upstream documents reference this tasks_plans in downstream traceability
