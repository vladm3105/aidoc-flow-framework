# SYS Corpus Validation Rules

## Document Control

| Field | Value |
|-------|-------|
| Document ID | SYS_CORPUS_VALIDATION |
| Version | 1.0 |
| Created | 2026-01-04 |
| Purpose | Quality gate for complete SYS corpus |
| Trigger | Run after ALL SYS files are complete |
| Scope | Entire SYS corpus validation |
...
All SYS Complete
        ↓
SYS_CORPUS_VALIDATION.md (corpus-level) ← Quality Gate
        ↓
...
| `IPLAN-NN` | 12 | IPLANs don't exist during SYS creation |
...
**Threshold**: All SYS files should have REQ-Ready Score ≥90%

**Validation Logic**:
```bash
# Check REQ-Ready scores
for f in "$SYS_DIR"/SYS-[0-9]*_*.md; do
  if [[ "$(basename $f)" =~ _index ]]; then continue; fi
  score=$(grep -oE "REQ-Ready Score[^0-9]*[0-9]+" "$f" | grep -oE "[0-9]+" | head -1)
  if [[ -n "$score" && $score -lt 90 ]]; then
    echo "WARNING: $(basename $f) has REQ-Ready Score $score% (target: ≥90%)"
  fi
done
```
...
| CORPUS-W015 | REQ-Ready Score below 90% | CORPUS-15 |
...
# SYS corpus validation on staged SYS files
if git diff --cached --name-only | grep -q "^docs/SYS/"; then
  echo "Running SYS corpus validation..."
  ./scripts/validate_sys_corpus.sh docs/SYS --errors-only
  if [ $? -ne 0 ]; then
    echo "❌ SYS corpus validation failed. Fix errors before committing."
    exit 1
  fi
  echo "✓ SYS corpus validation passed"
fi
```
...
SYS corpus validation should pass before creating REQ documents:
...
- [SYS-TEMPLATE.md](./SYS-TEMPLATE.md) - SYS document template
