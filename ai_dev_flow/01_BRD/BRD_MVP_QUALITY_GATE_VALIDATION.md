---
title: "BRD Quality Gate Validation"
tags:
...
| **Quality Gate Validation** | After ALL BRDs complete | Entire BRD set | This document |
...
```bash
#!/bin/bash
# BRD Quality Gate Validation Script
...
echo "=== BRD Quality Gate Validation ==="
...
name: BRD Quality Gate Validation
...
if git diff --cached --name-only | grep -q "^docs/01_BRD/"; then
  echo "Running BRD Quality Gate validation..."
...
This section captures verified counts from Quality Gate validation runs. Update after each validation.
