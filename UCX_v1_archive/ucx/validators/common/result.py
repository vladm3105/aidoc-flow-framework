...
- FixerContext: Fixer session context for LLM hand-off (v1.17.0+)
"""
...
@dataclass
class FixerContext:
    """Fixer session context for LLM hand-off (v1.17.0+).

    Records what the script-based fixer did, enabling LLM remediation
    personas to complete partial fixes and avoid undoing protected changes.
    """
    ...
    skipped_count: int = 0
    llm_completion: List[Dict[str, Any]] = field(default_factory=list)
    llm_only: List[Dict[str, Any]] = field(default_factory=list)
    fixer_applied: List[Dict[str, Any]] = field(default_factory=list)
    ...
    def from_json(cls, json_str: str) -> Optional["FixerContext"]:
        ...
        Returns:
            FixerContext instance or None if parsing/validation fails
        """
        try:
    ...
    def has_content(self) -> bool:
        ...
        return (
            self.fixed_count > 0
            or self.partial_fix_count > 0
            or self.skipped_count > 0
            or len(self.llm_completion) > 0
            or len(self.llm_only) > 0
            or len(self.fixer_applied) > 0
        )
...
class UnifiedValidationResult:
    ...
    # Fixer hand-off context (v1.17.0+)
    ...
    def format_report(self, doc_id: str, doc_type: str = "BRD", version: int = 1) -> str:
        ...
        # Section 7: Fixer Session Summary (v1.17.0+)
        if self.fixer_context:
            lines.append(self._format_fixer_section())
    ...
    def _format_fixer_section(self) -> str:
        """Format Section 7: Fixer Session Summary with embedded JSON.

        Returns:
            Markdown string for Section 7, or empty string if no fixer context
        """
        ...
        if ctx.fixer_applied:
            ...
            for item in ctx.fixer_applied[:10]:  # Limit to 10 items
                changes_preview = ", ".join(item.get("changes", [])[:2])
                if changes_preview:
                    lines.append(f"- `{item.get('code', '')}` in `{item.get('file', '')}`: {changes_preview}")
                else:
