...
**PRD Fixer Coverage (v1.21.7+)**:

Deterministic PRD fixer handlers now cover:
- `PRD-W006` (missing Section 10 subsections)
- `PRD-W011` (missing feature IDs in Section 7)
- `PRD-W012` (missing/format normalization for user story IDs in Section 8)
- `PRD-W019` (missing quality IDs in Section 21)
- `PRD-W021` (missing release/launch criteria structure in Section 14)

LLM/manual handoff actions are generated for semantic warnings:
- `PRD-W004` (BRD traceability gaps)
- `PRD-W009` (acceptance criteria semantics)
- `PRD-W013` (user story narrative format)
- `PRD-W014` (priority notation consistency)

Section 14 launch criteria templates intentionally avoid PRD element IDs because Section 14 has no valid PRD type codes in the schema mapping.
