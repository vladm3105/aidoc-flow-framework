...
class TestResolvePrdReviewTarget:
    def test_source_file_resolves_to_validation_sibling(self, tmp_path):
        source = tmp_path / "PRD-01_platform_architecture.md"
        source.write_text("# source", encoding="utf-8")

        resolved = resolve_prd_review_target(source)

        assert resolved == tmp_path / "PRD-01_platform_architecture_validation.md"

    def test_validation_file_stays_as_is(self, tmp_path):
        validation = tmp_path / "PRD-01_platform_architecture_validation.md"
        validation.write_text("# validation", encoding="utf-8")

        resolved = resolve_prd_review_target(validation)

        assert resolved == validation

    def test_directory_with_single_validation_file_resolves_to_file(self, tmp_path):
        validation = tmp_path / "PRD-01_platform_architecture_validation.md"
        validation.write_text("# validation", encoding="utf-8")

        resolved = resolve_prd_review_target(tmp_path)

        assert resolved == validation

    def test_directory_with_multiple_validation_files_keeps_directory(self, tmp_path):
        (tmp_path / "PRD-01_a_validation.md").write_text("# a", encoding="utf-8")
        (tmp_path / "PRD-02_b_validation.md").write_text("# b", encoding="utf-8")
