...
- Categories are expressed by folder paths (e.g., `reqs/api/av`, `reqs/risk/lim`).
...
- Requirements (REQ)
  - H1 ID: `REQ-NNN` or `REQ-NNN-YY` (e.g., `# REQ-001: Alpha Vantage Integration`). Do not use category-coded IDs like `REQ-API-AV-001`.
  - Filename: `REQ-NNN_{slug}.md` or `REQ-NNN-YY_{slug}.md` under category folders: `reqs/{category}/{subcategory}/REQ-NNN_{slug}.md` (e.g., `reqs/api/av/REQ-001_alpha_vantage_integration.md`).
...
- ADRs
  ...
  - Filename: `adrs/ADR-NNN_{slug}.md` or `adrs/ADR-NNN-YY_{slug}.md` (e.g., `ADR-033_risk_limit_enforcement_architecture.md`).
  - Variable Length: NNN = 3-4 digits (001-999, 1000+), YY = 2-3 digits [OPTIONAL] (01-99, 100+)
  - Notes: Use sub-numbering (-YY) when single architectural decision requires multiple related documents.
- BDD Features and Tags
  - Filename: `bbds/BDD-NNN_{slug}.feature` or `bbds/BDD-NNN-YY_{slug}.feature`.
  - Variable Length: NNN = 3-4 digits (001-999, 1000+), YY = 2-3 digits [OPTIONAL] (01-99, 100+)
  - Tags (mandatory):
    - `@requirement:[REQ-NNN](../reqs/.../REQ-NNN_{slug}.md#REQ-NNN)` or `@requirement:[REQ-NNN-YY](...)`
    - `@adr:[ADR-NNN](../adrs/ADR-NNN_{slug}.md#ADR-NNN)` or `@adr:[ADR-NNN-YY](...)` (if applicable)
  - Tags appear before `Scenario:` using valid relative paths + anchors.
  - Index: maintain `bbds/BDD-000_index.feature`.
...
- Technical Specifications (SPEC)
  ...
  - Filename: `specs/{type}/SPEC-NNN_{slug}.yaml` or `specs/{type}/SPEC-NNN-YY_{slug}.yaml` (e.g., `specs/services/SPEC-003_position_limit_service.yaml`).
  - Variable Length: NNN = 3-4 digits (001-999, 1000+), YY = 2-3 digits [OPTIONAL] (01-99, 100+)
...
- API Contracts (CTR)
  - H1 ID: `CTR-NNN` or `CTR-NNN-YY` (e.g., `# CTR-001: Position Risk Validation Contract`).
...
- Implementation Plans (IMPL)
  - H1 ID: `IMPL-NNN` or `IMPL-NNN-YY` (e.g., `# IMPL-001: Risk Management System Implementation`)
  - Filename: `impl_plans/IMPL-NNN_{slug}.md` or `impl_plans/IMPL-NNN-YY_{slug}.md` (e.g., `impl_plans/IMPL-001_risk_management_system.md`)
  - Variable Length: NNN = 3-4 digits (001-999, 1000+), YY = 2-3 digits [OPTIONAL] (01-99, 100+)
...
- AI Tasks (TASKS)
  - H1 ID: `TASKS-NNN` or `TASKS-NNN-YY` (e.g., `# TASKS-003: Position Limit Service Implementation`)
  - Filename: `ai_tasks/TASKS-NNN_{slug}.md` or `ai_tasks/TASKS-NNN-YY_{slug}.md` with a tasks index at `ai_tasks/TASKS-000_index.md`.
  - Variable Length: NNN = 3-4 digits (001-999, 1000+), YY = 2-3 digits [OPTIONAL] (01-99, 100+)
...
- Business Requirements Documents (BRD)
  - H1 ID: `BRD-NNN` or `BRD-NNN-YY` (e.g., `# BRD-009-01: Broker Integration Prerequisites`)
...
- Product Requirements Documents (PRD)
  - H1 ID: `PRD-NNN` or `PRD-NNN-YY` (e.g., `# PRD-003: Position Risk Limits`)
...
- System Architecture Documents (SYS)
  - H1 ID: `SYS-NNN` or `SYS-NNN-YY` (e.g., `# SYS-003: Position Risk Limits`)
...
- EARS Requirements (EARS)
  - H1 ID: `EARS-NNN` or `EARS-NNN-YY` (e.g., `# EARS-003: Position Limit Enforcement`)
...
- Structure (this example):
  - `reqs/{category}/{subcategory}/REQ-NNN_{slug}.md` or `REQ-NNN-YY_{slug}.md`
  - `adrs/ADR-NNN_{slug}.md` or `ADR-NNN-YY_{slug}.md`
  - `bbds/BDD-NNN_{slug}.feature` or `BDD-NNN-YY_{slug}.feature`
  - `specs/{type}/SPEC-NNN_{slug}.yaml` or `SPEC-NNN-YY_{slug}.yaml`
  - `contracts/CTR-NNN_{slug}.md` + `CTR-NNN_{slug}.yaml` (optional subdirs: `contracts/{agents,mcp,infra}/`)
  - `impl_plans/IMPL-NNN_{slug}.md` or `IMPL-NNN-YY_{slug}.md`
  - `ai_tasks/TASKS-NNN_{slug}.md` or `TASKS-NNN-YY_{slug}.md`
...
- Formats:
  - REQ in ADR: `[REQ-NNN](../reqs/.../REQ-NNN_{slug}.md#REQ-NNN)` or `[REQ-NNN-YY](...)`
  - ADR in BDD: `@adr:[ADR-NNN](../adrs/ADR-NNN_{slug}.md#ADR-NNN)` or `@adr:[ADR-NNN-YY](...)`
  - REQ in BDD: `@requirement:[REQ-NNN](../reqs/.../REQ-NNN_{slug}.md#REQ-NNN)` or `@requirement:[REQ-NNN-YY](...)`
  - REQ/ADR in CTR:
    - `[REQ-NNN](../reqs/.../REQ-NNN_{slug}.md#REQ-NNN)` or `[REQ-NNN-YY](...)` in Traceability section
    - `[ADR-NNN](../adrs/ADR-NNN_{slug}.md#ADR-NNN)` or `[ADR-NNN-YY](...)` in Traceability section
  - CTR in SPEC:
  ...
  - REQ/ADR in SPEC:
    - `requirements_source:
      - "[REQ-NNN](../../reqs/.../REQ-NNN_{slug}.md#REQ-NNN)"` or `"[REQ-NNN-YY](...)"`
    - `architecture:
      - "[ADR-NNN](../../adrs/ADR-NNN_{slug}.md#ADR-NNN)"` or `"[ADR-NNN-YY](...)"`
  - BDD in SPEC verification:
    - `verification:
      - BDD: "[feature_name.feature(:LNN)](../../bbds/feature_name.feature#LNN)"`
  - BRD in BRD:
...
- PRD: `prd/PRD-003_position_risk_limits.md` (H1: `# PRD-003: Position Risk Limits`)
- SYS: `sys/SYS-003_position_risk_limits.md` (H1: `# SYS-003: Position Risk Limits`)
- EARS: `ears/EARS-003_position_limit_enforcement.md` (H1: `# EARS-003: Position Limit Enforcement`)
- REQ: `reqs/risk/lim/REQ-003_position_limit_enforcement.md` (H1: `# REQ-003: Position Limit Enforcement`)
- ADR: `adrs/ADR-033_risk_limit_enforcement_architecture.md` (H1: `# ADR-033: Risk Limit Enforcement Architecture`)
- CTR: `contracts/CTR-001_position_risk_validation.md` + `CTR-001_position_risk_validation.yaml` (H1: `# CTR-001: Position Risk Validation Contract`, YAML: `contract_id: position_risk_validation`)
- BDD: `bbds/BDD-003_risk_limits_requirements.feature`
- SPEC: `specs/services/SPEC-003_position_limit_service.yaml` (id: `position_limit_service`)
- IMPL: `impl_plans/IMPL-001_risk_management_system.md` (H1: `# IMPL-001: Risk Management System Implementation`)
- TASKS: `ai_tasks/TASKS-003_position_limit_service.md` (H1: `# TASKS-003: Position Limit Service Implementation`)
- BRD: `docs/BRD/BRD-001_foundation_overview.md` (H1: `# BRD-001: Foundation & Overview`)

Examples (ai_dev_flow) - Sub-Documents (XXX-YY)
- BRD-009 multi-document group (broker integration pilot):
  - Prerequisites: `docs/BRD/BRD-009-01_prerequisites.md` (H1: `# BRD-009-01: Broker Integration Prerequisites`)
  - Main specification: `docs/BRD/BRD-009-02_broker_integration_pilot.md` (H1: `# BRD-009-02: Broker Integration Pilot`)
...
- IB (Interactive Brokers), AV (Alpha Vantage), MKT (Market), ORD (Order), EXEC (Execution), POS (Position), LIM (Limit), RISK (Risk), ACCT (Account), PFOL (Portfolio), CFG (Configuration), AUTH (Authentication), AUTHZ (Authorization), REDIS, PUBSUB, BQ (BigQuery), CSQL (Cloud SQL), GCR (Cloud Run), GSM (Secrets Manager)
...
@requirement:[REQ-009-01](../reqs/broker/ib/REQ-009-01_prerequisites.md#REQ-009-01)
@adr:[ADR-030-02](../adrs/ADR-030-02_connection_management.md#ADR-030-02)
```
