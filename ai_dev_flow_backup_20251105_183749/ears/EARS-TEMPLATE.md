...
**Position**: EARS is in Layer 1 (Business Layer) - transforms PRD into formal WHEN/THEN requirements.
...
```
WHILE the Alpha Vantage API returns HTTP 429 rate limit errors,
THE Market Data Client SHALL serve cached responses if freshness is within 5 minutes
and surface circuit breaker state to upstream callers.
```
...
```
IF the broker API rate limit is exceeded (HTTP 429 received),
...
```
IF concurrent position updates would cause database deadlock (detected via lock timeout),
THE Position Manager SHALL rollback transaction, apply jittered retry with exponential backoff,
...
#### Example 5: Circuit Breaker Activation
```
IF [Critical_Metric: e.g., error rate, resource usage, business metric] exceeds [Threshold: e.g., 2% of baseline, 100 errors/min],
THE [Protection_Component: e.g., Circuit Breaker, Safety Controller] SHALL activate protection mode,
...
|--------|---------------------|----------------------|---------------|----------|-------------------|
| NFR-COMP-001 | [Regulation: e.g., GDPR, HIPAA, SOX] | THE System SHALL retain all [Record_Type: e.g., transactions, audit logs] for [Period: e.g., 7 years] | [Jurisdiction: e.g., EU, US, Global] | High | Compliance audit, retention policy review |
...
✓ WHEN market data arrives, THE system SHALL process and cache it WITHIN 50ms at p95 latency.
```

#### Pitfall 3: Compound Requirements
```
✗ WHEN market opens, THE system SHALL scan stocks, calculate Greeks, evaluate strategies,
...
✓ THE Dashboard SHALL display position summary within 3 clicks from login.
```

#### Pitfall 5: Ambiguous Subjects
```
✗ WHEN errors occur, retry should happen.
✓ IF the broker API returns HTTP 503, THE API Client SHALL retry with exponential backoff
...
|-------------|-------------|----------------|-------------------|--------------|
| BRD | [BRD-NNN](../brds/BRD-NNN_...md) | [Business requirement title] | Sections 2.4, 4.x | Business objectives driving these requirements |
...
|--------|-----------|-------------------------|--------------|
| [ADR-NNN](../adrs/ADR-NNN_...md#ADR-NNN) | [Architecture decision title] | EARS statements [IDs] drive technology choice | This EARS requirement necessitates the architectural approach |
| [ADR-NNN](../adrs/ADR-NNN_...md#ADR-NNN) | [Architecture decision title] | EARS NFR-PERF-001 requires this pattern | Performance requirement drives architectural pattern |

#### 7.2.2 Atomic Requirements

| REQ ID | Requirement Title | Source EARS Statements | Relationship |
|--------|------------------|----------------------|--------------|
| [REQ-NNN](../reqs/.../REQ-NNN_...md#REQ-NNN) | [Detailed requirement] | Derived from EARS statements [IDs] | Detailed implementation requirement |
| [REQ-NNN](../reqs/.../REQ-NNN_...md#REQ-NNN) | [Detailed requirement] | Derived from EARS statements [IDs] | Detailed implementation requirement |
...
|---------|-------------------|------------------------------|--------------|
| [SPEC-NNN](../specs/.../SPEC-NNN_...yaml) | [YAML specification] | Implements EARS statements [IDs] | Code generation specification |
| [SPEC-NNN](../specs/.../SPEC-NNN_...yaml) | [YAML specification] | Implements EARS statements [IDs] | Code generation specification |
...
|-------------------|----------------------|------------------|--------------|---------------|
| Event-001 | Market data processing latency | [BDD-NNN.feature](../bbds/BDD-NNN.feature#L10) | Scenario: Process market data within latency target | Unit + Integration |
| State-001 | Maintenance mode behavior | [BDD-NNN.feature](../bbds/BDD-NNN.feature#L25) | Scenario: Reject trades during maintenance | Integration |
| Unwanted-001 | Rate limit error handling | [BDD-NNN.feature](../bbds/BDD-NNN.feature#L40) | Scenario: Handle API rate limit gracefully | Unit + Integration |
| Ubiquitous-001 | Data normalization | [BDD-NNN.feature](../bbds/BDD-NNN.feature#L55) | Scenario: Normalize Alpha Vantage to IB schema | Unit |
...
- [Business Requirements Document](../brds/BRD-NNN_[project_name].md) - Source business objectives
- [Functional Requirements](../brds/BRD-NNN_[project_name].md#appendix-i) - Detailed functional requirements
...
- [ADR Template](../adrs/ADR-TEMPLATE.md) - Architecture decision record format
- [BDD Feature Template](../bbds/BDD-TEMPLATE.feature) - Behavior-driven development scenarios
- [YAML Specification Standard](../../docs/specs/YAML_SPECIFICATION_STANDARD.md) - Specification format
...
- **[Domain_Practice: e.g., Risk Management, Quality Assurance]**: [Best practice guidelines, industry resources]
