...
**Example:**
```markdown
WHEN the Market Analysis Agent requests historical data, the client SHALL retrieve data from Alpha Vantage and cache the response with endpoint-appropriate TTL.
```
...
**Example:**
```markdown
WHILE Alpha Vantage is degraded, the client SHALL use the last valid cached response if freshness SLA is met.
```
...
**Examples:**
```markdown
THE client SHALL normalize responses to the internal schema used by IB Gateway to enable seamless failover.
