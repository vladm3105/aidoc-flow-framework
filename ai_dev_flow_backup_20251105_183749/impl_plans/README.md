...
| **Granularity** | "Phase 1: Build risk management (Week 1-2)" | "Step 3: Generate PositionLimitService class from SPEC-003.yaml" |
...
**Example Flow**:
1. **REQ-003**: "System must enforce position limits"
2. **IMPL-001**: "Risk Management System" - Phase 1: Build position limits (Week 1-2, Agent Team) → Deliverables: CTR-003, SPEC-003, TASKS-003
...
```
docs_templates/ai_dev_flow/impl_plans/
├── README.md                          # This file
├── IMPL-000_index.md                  # Master index of all IMPL plans
├── IMPL-TEMPLATE.md                   # Template for new IMPL plans
├── IMPL_IMPLEMENTATION_PLAN.md        # Example: Plan for creating impl_plans/ system
...
- [ ] **Token Limit**: File under 10,000 tokens
...
```markdown
## Phase 1: Position Limit Service (Week 1-2)

**Team**: Agent Development Team (3 developers)

**Deliverables**:
- CTR-003: Position Limit API Contract
- SPEC-003: Position Limit Service Specification
...
```markdown
## Phase 1: Position Limit Service
...
**Good** (Business Value):
```markdown
## Phase 1: Core Risk Management (Sprint 1-2)
- Position limits (CTR-003, SPEC-003)
- Portfolio risk calculator (CTR-005, SPEC-005)
...
- Basic position limits (SPEC-003)
...
**Solution**: Move technical details to SPEC. IMPL only says "create SPEC-003 for position limit service"
...
**Problem**: Phase says "Build risk management" but doesn't list which CTR/SPEC/TASKS

**Solution**: Always enumerate deliverables: "CTR-003, SPEC-003, TASKS-003"

### Pitfall 5: No Team Assignment
...
cp docs_templates/ai_dev_flow/impl_plans/IMPL-TEMPLATE.md \
   docs/impl_plans/IMPL-042_my_feature.md
```
...
- **Token Limit**: File < 10,000 tokens?
...
- **IMPL-000_index.md**: Master index of all IMPL plans
- **IMPL-TEMPLATE.md**: Template for new IMPL plans
- **TASKS README**: Understanding code generation plans (ai_tasks/README.md)
