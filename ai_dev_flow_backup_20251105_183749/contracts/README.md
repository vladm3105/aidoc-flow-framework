...
|--------------|---------|----------|
| **agents/** | Agent-to-agent communication | Portfolio Orchestrator API, Strategy Agent interfaces |
| **mcp/** | MCP server contracts | Risk Validator MCP, Greeks Calculator MCP, Market Data MCP |
...
- **Alignment**: Directory structure mirrors SPEC organization
  - `contracts/agents/CTR-001` → `specs/agents/SPEC-001`
  - `contracts/mcp/CTR-010` → `specs/mcp/SPEC-010`
...
- Trade execution (creates side effects)
- State mutations (position updates)
...
**Example**: Risk validation, Greeks calculation, portfolio state query
...
**Example**: Trade execution events, portfolio rebalancing triggers
...
### 8.4 Bulk Operations
**Use Case**: Batch validation, bulk updates, portfolio rebalancing
...
|-------------|-------------|----------------|-------------------|--------------|
| REQ | [REQ-003](../reqs/.../REQ-003.md) | Position Limit Enforcement | Section 3.1 | Interface requirement |
| ADR | [ADR-008](../adrs/ADR-008.md) | Centralized Risk Parameters | Architecture pattern |
```
...
|---------------|-------------|----------------|--------------|
| SPEC | [SPEC-005](../specs/.../SPEC-005.yaml) | Risk Validation Service | Provider implementation |
...
[CTR-001](../contracts/agents/CTR-001_portfolio_orchestrator_api.md)
```
...
This service implements **[CTR-001: Position Risk Validation](../contracts/CTR-001_position_risk_validation.md)** as the provider.
...
```python
# tests/contracts/risk_validation/test_provider_contract.py
import pact
...
```python
# tests/contracts/risk_validation/test_consumer_contract.py
import pact
...
def test_consumer_expects_ctr001_schema(pact_consumer):
    ...
    result = client.validate_position(position)
...
yamllint contracts/CTR-001_position_risk_validation.yaml

# JSON Schema validator - Schema structure validation
check-jsonschema --schemafile contracts/CTR-001_position_risk_validation.yaml
```

### 13.2 OpenAPI Linters
```bash
# Spectral - OpenAPI/AsyncAPI linter
spectral lint contracts/CTR-001_position_risk_validation.yaml
```
...
| **Organize by type** | Optional: contracts/{agents,mcp,infra}/ |
