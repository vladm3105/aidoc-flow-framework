...
-   **Single Source of Truth**: Avoid duplicating information. If `01-database-schema.md` defines the standard, other docs should link to it, not copy it.
...
-   **Changelog**: Maintain a `CHANGELOG.md` strictly for specification changes. "Added field `cost_allocation_tag` to schema in v1.1".

## 5. Traceability
-   **Spec-to-Code Linking**: In implementation repos, comments should reference spec sections.
    ```python
    # See docs/core/03-agent-routing-spec.md Section 4.1
    def route_request(context): ...
    ```
-   **Issue Linking**: Every PR in this repo should link to a "Documentation" or "Planning" issue.

## 6. Publishing
-   **Static Site Generator**: Considere using MkDocs, Hugo, or Docusaurus to render these Markdown files into a searchable website.
...
-   **Docs Repo**: `my-project-specs` (The "Why" and "What")
-   **Code Repo**: `my-project-impl` (The "How")
...
2.  **In Code**: Reference the ID in docstrings.
    ...
    def login(user):
        """
        Implementation of [AUTH-001]
        See: https://github.org/project-specs/blob/main/core/auth.md#auth-001
        """
    ```
