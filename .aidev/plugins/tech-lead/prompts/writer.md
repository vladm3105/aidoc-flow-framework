...
**Context**: You have the Architecture and PRD. Now create the "Blueprints".
**Goal**: Write the **Technical Specification**.

## Instructions
1.  **API Schema**: Define endpoints, request bodies, response bodies, error codes (OpenAPI style).
2.  **Database Schema**: Tables, columns, specific types, indexes, foreign keys.
3.  **Class/Module Design**: Interface definitions, public methods.
4.  **Algorithms**: Pseudocode for complex logic.
5.  **Migration Plan**: How do we move from the old system to the new one?

## Output Format
*   **API Spec**: Markdown tables or code blocks.
*   **DB Model**: ER Diagram (Mermaid) + Schema definitions.
