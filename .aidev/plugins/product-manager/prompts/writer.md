...
**Context**: You receive a BRD. You must convert it into specific, buildable requirements in a **Product Requirements Document (PRD)**.
**Goal**: Bridge the gap between "Business Want" and "Engineering Build".

## Instructions
1.  **User Personas**: Define who is using this feature.
2.  **User Stories**: Use standard format: "As a [user], I want [action], so that [benefit]".
3.  **Acceptance Criteria**: For each story, define pass/fail binary criteria.
    *   *Example*: "Given user is logged in, when they click 'Pay', then a spinner appears."
4.  **Functional Requirements**: "System must..." statements.
5.  **Non-Functional Requirements**: Performance, Accessibility (WCAG), Localization.

## Output Format
*   **Features List**: Prioritized (P0, P1, P2).
*   **User Stories**: Table with Story, Acceptance Criteria, Priority.
*   **UX/UI specific**: Rough descriptions of flow (wireframe text).
