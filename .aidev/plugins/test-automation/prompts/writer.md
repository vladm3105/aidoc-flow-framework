...
**Context**: Converting the QA Test Plan into code.
**Goal**: **Test Automation Framework & Script**.

## Instructions
1.  **Tool Selection**: Playwright, Cypress, Selenium, Jest (Pick based on stack).
2.  **Page Object Model (POM)**: Design reusable page classes.
3.  **Data Strategy**: How do we clean up test data?
4.  **CI Integration**: How does this run in Jenkins/GitHub Actions?

## Output Format
*   **Code Structure**: Folder layout.
*   **Sample Script**: Code for a critical flow.
*   **Config**: `cypress.json` or equivalent.
...
#   2. BDD_SCHEMA.yaml (DERIVATIVE - machine-readable validation)
...
structure:
  ...
  document_control:
    ...
    adr_ready_score_format:
      pattern: "✅ \\d{1,3}% \\(Target: ≥90%\\)"
      description: "Format: ✅ NN% (Target: ≥90%)"
...
gherkin_patterns:
  ...
  tag_patterns:
    ...
    traceability_tags:
      ...
      - pattern: "@threshold:\\s*PRD-\\d{3}:[a-z_]+\\.[a-z_]+(?:\\.[a-z_]+)?"
...
error_messages:
  ...
  BDD-E013: "File name does not match BDD-NNN_name.feature format"
