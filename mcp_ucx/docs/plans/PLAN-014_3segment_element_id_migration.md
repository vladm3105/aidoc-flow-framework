...
2. **Hash length**: 4-char hex (65,536 values per document). Collision: extend to 5-8 chars for the colliding element only. Detection happens at document generation time (mcp_ucx `sdd_create` or manual authoring). Duplicate `{yaml_key}:{title}` pairs within a document are invalid — each element must have a unique title within its parent key.
...
### Creation prompts (6 files in `mcp_ucx/prompts/templates/creation/`)
...
## Phase 7: mcp_ucx Persona Skills (15 files)

Location: `/opt/data/ucx_framework/mcp_ucx/skills/personas/`
...
| `mcp_ucx/src/mcp_server/tool_registry.py` | Check for ID format references |
...
| File | Action |
|------|--------|
| `mcp_ucx/docs/README.md` | Update changelog index, add v1.6.0 entry |
| `mcp_ucx/docs/architecture/MCP_CLI_REFERENCE.md` | Update element ID examples if present |
| `mcp_ucx/docs/specs/SPEC-003_mcp_creation_validation_profile_contracts.md` | Update element ID format in contracts |
| `mcp_ucx/skills/README.md` | Update element ID format if referenced |

### mcp_ucx Changelog & Roadmap

| File | Action |
|------|--------|
| `mcp_ucx/docs/ROADMAP.md` | Update: v1.5.0 → v1.6.0, add v1.6.0 release entry |
| `mcp_ucx/docs/CHANGELOG/CHANGELOG_v1.6.0.md` | CREATE — 3-segment IDs, template updates, regex, prompt template fixes |
