...
Unified report naming across all mcp_ucx tools. Convention: `{DOC-ID}.{STAGE}.{FORMAT}`. Sub-framework registry (sdd, gov, kb). Derived copies renamed from `_validation`/`_remediated` to `_validate_copy`/`_remediate_copy`.
