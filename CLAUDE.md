...
- **An absence is the easiest defect to assert and the hardest to verify.**
  ...
  **second copy of a check that was already running and already right**; the real gap
  ...
  an absence down.**
- **A guard's negation-exemption is a hole sized by its TOKENIZER, not by you, and adding a
  correct prohibition sentence can silently disarm it.** `test_iplan_code_inventory_lifecycle`
  applies `_PROHIBITION` per **sentence**, and its `_normalize` collapses every whitespace run
  first — so its `_sentences` split on `\n` is **dead code**, and a markdown table with no
  `.`+whitespace becomes ONE unit. `doc-iplan-fixer`'s Fix-Phases table measures **1,922
  characters** as a single "sentence" carrying `code_inventory` twice. One `never` / `not`
  anywhere in it exempts the whole table and **the suite stays green because nothing happens**.
  Measured baseline: **7 `code_inventory`-bearing sentences across the four `doc-iplan*` skills,
  0 exempt** — pinned by `test_iplan_session_handoff_draft.GD25GuardIsNotDisarmed`, which caught
  a live instance during GD-26 (an audit carve-out ending "not a missing section" took it to 1).
  **Give every new prohibition clause its own sentence**, and when writing such a guard, split
  table rows and list items into their own units *before* normalizing. Sibling of the two bullets
  above; the difference is that here the disarming edit is *prose*, and no test names it.
- **`framework/**` must be engine-agnostic and `framework/governance/DECISIONS.md` is NOT
  exempt.** `tests/conformance/test_spec_hygiene.py::EngineTokenHygiene` rejects `hermes`,
  `\bmcp\b`, `.claude/` and `\bplugin\b` anywhere under `framework/`. Its allowlist is
  `(path, token)` pairs, so `plugin` and `doc-` are permitted **in that decisions file only** —
  the engine name never is. Write "the other engine", "Platform A", "Platform B", as GD-25 and
  GD-26 do. A GD entry naming the engine reds a spec-hygiene test that most authors do not know
  applies to prose.
