...
- **GD-15 — YAML is the mandatory format and the source of truth**, for templates *and* for the instance artifacts authored from them; Markdown is optional and descriptive. Principle 2 of `governance/DOC_GOVERNANCE_CORE.md` previously scoped this to templates only and left instance format undeclared, while three surfaces disagreed about what an artifact is: the registry declared `.yaml` for all eight layers, the acceptance goldens were mixed, and the example corpus was all `.md`. Markdown never carries a fact the YAML lacks, and a `.md` restating YAML content is **generated, not authored**. `registry/LAYER_REGISTRY.yaml`'s `extensions` is now stated as the normative instance-format field, and its index-template header comment is scoped to index docs so it no longer reads as a claim about instances.
- **GD-16 — an IPLAN file-manifest entry carries its TDD test cases in a line-local `tdd_ref` field.** The value is a `@tdd:` tag and **must be quoted** (`@` is a YAML reserved indicator); the key and the tag share one line, so a citation appearing only in the traceability block is not a build record; the field is optional per entry, with completeness judged from the TDD side. This mirrors the TDD layer's own `bdd_scenario:`/`bdd_ref:` carriers. Added to `IPLAN-TEMPLATE.yaml` and `IPLAN-MVP-TEMPLATE.yaml`, documented in the layer README, and guarded by `tests/conformance/test_iplan_carrier.py` (which asserts **both** templates — the canonical one was as unguarded as the skeleton).
- **Why a carrier rather than a coverage rule.** Element-level coverage stops at TDD — `REALIZING_LAYERS` has no IPLAN key — so nothing asks whether a test case reached an implementation plan; measured on the corpus, 7 of 35 `TDD-01` elements are cited by `IPLAN-01` while its audit returned PASS at 100/100 ([#543](https://github.com/vladm3105/aidoc-flow-framework/issues/543)). The obvious fix does not work: a rule built on the document-scoped realization primitive is silenced by one traceability line listing every id, which is the vacuous-pass loophole `ACC01` already documents. The rule (`COV04`) is a successor change and must be carrier-scoped; this release ships the field it needs.

### Fixed — a trace tag ending a quoted YAML scalar was silently discarded ([#542](https://github.com/vladm3105/aidoc-flow-framework/issues/542)) (2026-08-26)

- `sdd_doc_lint`'s tag value capture terminated on whitespace and a pipe but **not on a quote**, so a tag closing a quoted scalar glommed the closing `"` into the value, failed the anchored `ELEM_FORM`/`DOC_FORM` check, and was dropped from the edge graph with **no finding emitted**. Only the final tag of a scalar was affected — the single-tag case, which is the common one, always. The framework's own layer templates prescribe exactly that form (47 occurrences across the eight canonical `<X>-TEMPLATE.yaml` files), so the defect reached any corpus authored to template; `ACC01` had been working around it rule-locally since it was written.
- Both tag regexes now exclude quotes, matching the exclusion `_THRESHOLD` has carried for the same bug class. Safe because every tag value is validated against anchored forms that admit no quote. **Measured blast radius: zero** findings changed across the example corpus and three golden tiers. Regression: `tests/unit/test_tag_quote_termination.py`, registered in the conformance shim so it rides a required context.
...
Unblocked by the founder setting `LITELLM_DOC_API_KEY` on this repo (the secrets existed
on `aidoc-flow-operations`, and a personal account has no org secrets to inherit).
**25 of 28 → 28 of 28.**

- **`dry_run: true`, and it stays there.** The flow proposes edits as a PR comment plus a
  patch artifact and writes nothing. Graduation to live needs ≥5 clean dry-run cycles
  (IPLAN-0025 §3 P4) **and** two founder-tier prerequisites still absent:
  `AIDOC_FLOW_BOT_ID`/`AIDOC_FLOW_BOT_KEY` on this repo, and `aidoc-flow-bot[bot]` in
  `.github/ai-review/config.json#trust.ai_review`. Verified in canon's reusable that
  both bot secrets are `required: false` and referenced only in the
  `dry_run != 'true'` branch, so dry-run runs correctly without them.
- **`litellm_allow_insecure_http: true` is required here, not optional.** This repo's
  `LITELLM_BASE_URL` is the `http://` Docker-bridge address, and canon's client refuses
  a non-HTTPS proxy without the flag — the same reason `ai-review.yml` sets it. Required
  by the URL scheme, not by repo visibility.
- **The `allowed_paths` allowlist is the safety boundary, and it was verified as one.**
  `planner.py:187` filters proposals through `fnmatch.fnmatchcase` against it
  (`:110-113` validates the shape). Tested by replicating that matcher over 22 paths:
  all 8 intended docs allowed, all 14 forbidden paths denied — including `framework/**`
  (a style edit trips GATE-SPEC), `platforms/claude-code-plugin/framework/**` (a
  generated byte-identical mirror whose drift guard an edit breaks), `examples/**`
  (fixtures whose whole purpose is that the framework produces them unaided),
  `tools/sdd_doc_lint/fixtures/**` (deliberately malformed), and both decision logs.
- **Tighter than canon's defaults for the pilot:** `max_edits_per_pr` 8 → 4,
  `max_prs_per_day` 5 → 2. `plans/HANDOFF.md` and `ROADMAP.md` are classified
  **high-risk** (propose, never auto-merge); the decision logs and work queues are
  high-risk *and* excluded from `allowed_paths`, as defence-in-depth if the allowlist is
  ever widened.
- **`.github/doc-maintainer-conventions.md` is hand-authored**, as canon requires — it
  encodes this repo's actual discipline (the `CONTRIBUTING.md` doc-of-record matrix, the
  three independent version streams, the mechanical-vs-semantic split, "a wrong cause is
  worse than no entry") and the table of forbidden paths with *why each edit is
  incorrect*, not merely unwanted.
- **No `_comment` key in `doc-maintainer.json`.** A `_comment` inside a canon config is
  what made `.github/ai-review/config.json` fail its own schema under v2
  (#356/#341); rationale lives in the conventions file instead.
- **Watch item:** the caller keeps canon's `cron: '7,37 * * * *'` byte-exact — a
  reconciler that fires twice an hour on the two-slot self-hosted pool shared with the
  required `ai-review`. Kept as canon ships it so the pilot measures real behaviour; if
  `ai-review` starts queueing behind it, narrow the cron and record the divergence.
