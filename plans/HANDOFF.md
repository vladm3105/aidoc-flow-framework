...
> **Status: SHIPPED — impl PR #305 merged 2026-07-11** (Hermes-only MINOR). All real
> checks green (569 hermes + 208 conformance, ai-review, CodeQL, lint); the merge used
> `--admin` per founder authorization because the required `call / composition` check is
> in a repo-wide `startup_failure` state (see the CI-blocker note below). Plan PR #301
> merged earlier.
>
> **⚠️ OPEN CI BLOCKER (repo-wide, P0):** the required `call / composition` check has
> `startup_failure` on EVERY PR + on `main` since 2026-07-10 ~20:49 (last success 18:43).
> The caller `.github/workflows/composition.yml` is valid and pins the immutable
> `@ci/v1.8.1` tag (unchanged sha); the callee `composition.yml` is byte-identical at
> `ci/v1.8.1` and `ci/v1.9.0`. No relevant merge to `main` in the break window. Most
> likely a **GitHub Actions access/visibility change on the `aidoc-flow-ci` repo** (ci
> commit #120 at 19:54 was "private repos use self-hosted runners by default") making its
> reusable workflows unresolvable from this repo → `startup_failure` ("workflow file
> issue"). **Fix is an org/repo Actions-settings change (founder/CI-owner), not a
> repo-file edit.** Until fixed, all PRs need `--admin` merge. Tracked in the pre-prod
...
> "D-0038 auto-satisfied" banner + new H-15/H-16), **PR-DOCS #291** (docs/version
> drift sweep; VERSION held at `0.7.3`; extended `sync-version-refs.sh` to cover the
> Hermes pyproject + README, closing FRAMEWORK-TODO `HERMES-README-VERSION-DRIFT`),
> **PR-CODE #292** (6 MCP source fixes C1-C6 → `0.7.4`, each with a regression test —
> threading.Lock env-lock, O_EXCL TOCTOU, datetime.now(UTC), asyncio.to_thread saga,
> cleanup unlink-safety+`failed` surfacing, dead scoring code), **PR-BDD #293**
> (native BDD authoring → YAML-BDD → `0.8.0`, closing **H-15**, + a Hermes-side
> structural drift guard), **PR-ADAPT #294** (`.aidoc/profile.yaml` runtime
> consumption → `0.9.0`, partly closing **H-16** — profile loader + `review_mode`
> alias + glossary/section_toggle prompt injection). **Current hermes version:
> `0.9.0`** (spec `0.36.2` unchanged; Platform-A-only initiative). 544 hermes pytest
>
> - 193 conformance green. **H-16 follow-up (deferred):** structural enforcement of
> `active_layers`/`section_toggles`, the `audit_threshold` gate, and
> `quality_loop_max_iterations` (needs the outer review loop — H-7). A hermes release
> tag (`hermes/v0.9.0`; last tag `hermes/v0.1.1`) remains uncut.
