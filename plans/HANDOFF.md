...
> **✅ SESSION 2026-07-29 (LATER) — CANON-PARITY-001: the canon migration is
> complete on both dimensions. Sixteen call sites at `@ci/v2.16.0`; the tracker
> is EMPTY (#373 closed by adoption, not by a pin edit).**
>
> **The insight that organised the work:** re-pinning and adopting are different
> dimensions, and only the first was ever automated. A `--repin` rewrites `uses:`
> strings; it cannot adopt a surface this repo does not call, so four canon
> surfaces stayed absent through every green bump, and `codeql` stayed
> hand-rolled. `check-pin-currency.sh` and `check-drift.sh` both iterate over
> callers **that exist**, so they reported "all pins current ✅" the whole time.
> The census that finds the gap is canon's `install/templates/manifest.json`
> walked against the working tree — **run it when the canon minor moves, not just
> the re-pin.**
>
> **PR #378 (merged):** `codeql` became a canon caller — which is what actually
> closed [#373](https://github.com/vladm3105/aidoc-flow-framework/issues/373),
> since canon's reusable already SHA-pins `github/codeql-action@e4fba868`
> (v4.37.3). **#373 was a symptom of an unmigrated surface, not an independent
> defect**; editing the local `uses:` lines would have closed the issue and left
> the hand-rolled workflow to drift again at the next action release. Also
> adopted `dep-scan`, `sast-scan`, `trivy-scan`, `markdown-lint`.
> **PR #379 (open, yours to merge — touches `CLAUDE.md`):** the corrections +
> **D-0071**. **PR #380:** this file + the PR template.
>
> **Use `install/deploy-ci-wizard.sh scaffold <repo> <dir> [wf…]`** to adopt a
> surface: byte-exact callers at the pin, written to a scratch dir, never
> committed — so it cannot clobber a customized caller the way `--update` would.
> Three of five landed byte-exact and add **zero** drift.
>
> **Five things a next session must not re-derive:**
>
> 1. **`report-only` protects the verdict, not the toolchain.** `sast-scan` went
>    red on first run *with* `fail-on-findings: false` set: semgrep installs into
>    a `python3 -m venv` and the `aidoc-flow-runner` image ships **no Python**, so
>    it died at *"ensurepip is not available"* before any findings logic. The
>    check name (`sast-scan FAILURE`) names the wrong cause — it reads as
>    "semgrep found something". Overridden to `ubuntu-latest`; filed as
>    [aidoc-flow-ci#349](https://github.com/vladm3105/aidoc-flow-ci/issues/349).
>    **Revert the override only when that lands.** `dep-scan` and `trivy-scan`
>    share the pool and pass — they `curl` static binaries.
> 2. **`markdown-lint`'s globs are load-bearing, and `fail-on-findings: false` is
>    permanent.** Canon's bare `**/*.md` = **260 issues in 78 files** here, *all*
>    in paths where a fix is **forbidden**: `framework/` trips GATE-SPEC, and
>    `platforms/claude-code-plugin/framework/` is a generated byte-identical
>    mirror (D-0022) whose drift guard a style fix breaks. Scoped to the
>    `.pre-commit-config.yaml` exclusion set: **447 files, 0 issues**. And the
>    flag stays `false` **not** as a debt rollout — the blocking markdown gate is
>    the `markdownlint-cli` pre-commit hook under the required `Lint / format /
>    security hooks` context, and two different engines disagreeing on rule
>    defaults must never block a merge.
> 3. **The runner pool serves up to five jobs per PR against two slots, and that
>    is fine.** `ci-runner@.service` is `Restart=always` / `RestartSec=5`, so the
>    single-use slots **self-replenish**; the added self-hosted jobs serialize
>    rather than starve the required `ai-review`. Verified from the unit file, not
>    assumed.
> 4. **Two false readings, both cheap to repeat.** The manifest lists
>    `.github/pull_request_template.md` and its presence check is
>    **case-sensitive** — this repo's `PULL_REQUEST_TEMPLATE.md` reads as absent,
>    caught only because `pre-commit`'s case-conflict hook rejected the duplicate.
>    And `gh api …/contents/<missing> --jq '.name'` emits the string **`null`**,
>    which `[ -n "$n" ]` reads as *present* — that inverted "no sibling repo
>    adopts `sast-scan`" into "all seven do" until re-checked by listing the
>    directory. **Never truth-test a jq scalar that can be `null`.**
> 5. **`codeql` needs no `build-mode`.** The local workflow set
>    `build-mode: none`; canon's reusable exposes no such input and runs
>    `autobuild`, which is a no-op for interpreted languages. Both
>    `call / Analyze (actions)` and `call / Analyze (python)` pass — measured on
>    #378, so don't re-litigate it.
>
> **Verified at execution:** `pre-commit run --all-files` green (incl. the
> conformance suite), twice, idempotent; pin-currency *all pins current* vs
> `ci/v2.16.0`; drift **10 → 12** with compared callers **10 → 15** (the two new
> divergences are `codeql`'s `languages` and `markdown-lint`'s globs, each
> carrying in-file rationale per `check-drift.sh`'s own resolution guidance);
> #378 reached `CLEAN` and merged **without `--admin`**.
>
> **Still open, deliberately:**
>
> - **`doc-maintainer` is the one unadopted surface, and it is credential-blocked
>   — not declined.** `LITELLM_DOC_API_KEY`, `AIDOC_FLOW_BOT_ID` and
>   `AIDOC_FLOW_BOT_KEY` exist on `aidoc-flow-operations` but **not here**, and
>   there are no org-level secrets to inherit (personal account → the API returns
>   422). Dry-run needs only the LiteLLM doc key. Founder route:
>   `export LITELLM_BASE_URL LITELLM_DOC_API_KEY; bash install/set-litellm-secrets.sh
>   --repos "vladm3105/aidoc-flow-framework" --doc`. Live mode additionally needs
>   the App permissions raised to PR/Issues write, `aidoc-flow-bot[bot]` added to
>   `.github/ai-review/config.json#trust.ai_review`, and a hand-authored
>   `.github/doc-maintainer-conventions.md`.
> - **`PIN-CURRENCY-NO-READER`** (TODO-only) — give the existing weekly output a
>   destination. Not a new detector; see the retraction below.
> - **`plans/HANDOFF.md` is 3272 lines** because sessions appended instead of
>   regenerating, which the workspace rule forbids. A proper split — volatile
>   banner rewritten each merge, durable traps kept — is worth one focused PR and
...
>    is self-consistent and silent. That is `check-pin-currency.sh`'s job.
>    ~~and this repo runs it **nowhere**, which is how the mixed-pin state
>    accumulated. Filed as `NO-PIN-CURRENCY-CHECK` in `FRAMEWORK-TODO.md`
>    (TODO-only).~~ **RETRACTED 2026-07-29 (CANON-PARITY-001, D-0071): the
>    struck clause is FALSE.** `check-pin-currency.sh` runs on every weekly
>    `standards-drift` run — canon's `check-standards-drift.sh` tail
>    (`:499-515`) invokes it — and it fired on 2026-07-27 naming all ten stale
>    pins *and* the `--repin` remedy. The first sentence stands; the cause did
>    not. The real gap is that a warning-only annotation on a weekly scheduled
>    job has **no reader**, restated as `PIN-CURRENCY-NO-READER`. One
>    `gh run view --log | grep pin-currency` would have caught this before it
>    was written down.
