...
| R8 | *(**SUPERSEDED on placement** — `IPLAN-SELF-DESCRIPTION-001` / GD-23, 2026-08-31. Row retained so R-numbering stays stable, matching F-3 and the R3/R4 VOID rows.)* `title` ships **top-level and scalar**, not under `metadata:`: this row cited the OKF D1 contract as its authority, and D1 declares *the required **top-level** keys* — so it contradicted what it cited. `metadata.title` existed in no template. The `artifact_type` half is **not** superseded; the identity question it belongs to is #588. Do **not** add `type` | C3 | 1 |
...
| 5 | R8, R9, R10, R12 — OKF template fields, MVP reconciliation, the layers 1-8 projection generator, contract rows | not written | blocked on `OKF-CONFORMANCE-001` D1; R12's index row also on D3. **R8's `title` half shipped early** via `IPLAN-SELF-DESCRIPTION-001` (GD-23, framework `0.48.0`) — top-level and scalar, superseding this row's `metadata:` placement; its `artifact_type` half stays here, and the identity question is #588 |
