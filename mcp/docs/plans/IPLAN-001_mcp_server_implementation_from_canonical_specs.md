...
- SPEC-001: mcp/docs/specs/SPEC-001_mcp_core_architecture_workflow_contracts.md
- SPEC-002: mcp/docs/specs/SPEC-002_mcp_review_scoring_handoff_identity_contracts.md
- SPEC-003: mcp/docs/specs/SPEC-003_mcp_creation_validation_profile_contracts.md
- SPEC-004: mcp/docs/specs/SPEC-004_mcp_reporting_lineage_artifact_contracts.md
...
- Canonical contract docs exist under mcp/docs/specs as the only normative source.
...
|------------|------|--------|
| mcp/docs/specs/SPEC-001_mcp_core_architecture_workflow_contracts.md | Blocks | Complete |
| mcp/docs/specs/SPEC-002_mcp_review_scoring_handoff_identity_contracts.md | Blocks | Complete |
| mcp/docs/specs/SPEC-003_mcp_creation_validation_profile_contracts.md | Blocks | Complete |
| mcp/docs/specs/SPEC-004_mcp_reporting_lineage_artifact_contracts.md | Blocks | Complete |
...
- Contract docs remain authoritative under `mcp/docs/specs`.
- Runtime implementation code exists only under `mcp/src/mcp_server`.
- `mcp/skills/` and `mcp/prompts/templates/` are the canonical scaffold source used by `mcp init` to create project-specific UCX files; they are never loaded by the runtime directly.
...
- Project onboarding: `mcp init --project {project_root}` scaffolds `{project_root}/docs/UCX/` from canonical templates in `mcp/skills/` and `mcp/prompts/templates/`; existing project files are never overwritten.
...
- MCP bundled files under `mcp/skills/` and `mcp/prompts/templates/` are the `mcp init` scaffold source; they are never loaded at runtime.

Implementation mapping from UCX_v1_archive:
- `UCX_v1_archive/skills/*` -> `mcp/skills/personas/*`
- `UCX_v1_archive/review/*`, `UCX_v1_archive/creation/*`, `UCX_v1_archive/remediation/*` -> `mcp/prompts/templates/{review|creation|remediation}/*`
- `UCX_v1_archive/ucx/*` modules -> `mcp/src/mcp_server/*` package modules
- `UCX_v1_archive/tests/*` -> `mcp/tests/*` (with explicit `contract/` row coverage)
- `UCX_v1_archive/docs/plans/*` and standards docs -> `mcp/docs/architecture/*` and `mcp/docs/plans/*` as applicable
...
- Implement `mcp init --project {project_root}` command: scaffold `{project_root}/docs/UCX/` by copying canonical templates from `mcp/skills/` and `mcp/prompts/templates/`; skip files that already exist (no overwrite).
...
- Ensure no runtime code path loads from `mcp/skills/` or `mcp/prompts/templates/`.
...
- No runtime code path loads from `mcp/skills/` or `mcp/prompts/templates/`; verified by test.
...
- Execute `mcp/docs/plans/TEST-CHECKLIST-001_mcp_new_contract_rows.md` (TC-001..TC-014) and collect pass/fail plus evidence links per test case.
...
- Publish checklist evidence summary referencing `mcp/docs/plans/TEST-CHECKLIST-001_mcp_new_contract_rows.md`.
- Lock repository-level policy for legacy report handling (`import|ignore|fail-fast`) before cutover.
- Record legacy report handling policy in `mcp/docs/policies/legacy_report_policy.md`.
...
- Legacy report policy artifact exists at `mcp/docs/policies/legacy_report_policy.md` and matches configured runtime policy.
...
- Checklist evidence: `mcp/docs/plans/TEST-CHECKLIST-001_mcp_new_contract_rows.md`
- Compliance artifact: `mcp/docs/plans/COMPLIANCE-REPORT-001_mcp_canonical_contracts.md`
- Release-readiness artifact: `mcp/docs/plans/RELEASE-READINESS-001_mcp_cutover_status.md`
...
| 1.8 | 2026-03-24 | AI Collaboration | Implemented `mcp init --project` scaffolding with no-overwrite copy semantics from canonical `mcp/skills` and `mcp/prompts/templates` to project `docs/UCX`; added CLI + scaffold unit tests and revalidated MCP contract/integration slices. |
...
| 2.4 | 2026-03-24 | AI Collaboration | Published canonical compliance and release-readiness artifacts for Workstream G/H, added explicit legacy report policy contract helpers with tests, and recorded repository policy artifact `mcp/docs/policies/legacy_report_policy.md`. |
