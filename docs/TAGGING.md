...
| `claude-code-plugin/v0.24.0` | Plugin release `0.24.0` close (IDGEN-NO-GENERATOR, MINOR) | Claude Code plugin — no skill computes SHA-256 in-prompt: all 13 `doc-*` authoring/fixing skills rewritten; BRD skills call `rehash --compute`, the other five layers emit a stable opaque 4-hex identifier (no `id_state: provisional` — canonicalization cannot run for them yet, so the mark could never be discharged); fixers gained a preserve-existing-valid-ID clause. |
| `hermes/v0.12.0` | Hermes release `0.12.0` close (IDGEN-NO-GENERATOR, MINOR) | Hermes platform — 5 prompt templates + 9 loaded reference documents corrected; the references were shipping runnable ad-hoc hash code with per-file normalization variants, none matching the standard, and were absent from #342's census. |
| `claude-code-plugin/v0.25.0` | Plugin release `0.25.0` close (PLUGIN-PREPROD-001, MINOR) | Claude Code plugin — the pre-production hardening cut: review hook no longer executes code from the user's working directory (`PYTHONSAFEPATH=1`) and gates on project adoption; the permission-model bypass is disclosed and opt-in; the saga driver cannot wedge permanently or report `PASS` on reviews that never ran, and its exit codes narrow (`0` = `CLOSED` only, plus `4`/`5`/`127`); `--threshold` becomes a live gate; PyYAML / Python-floor absence exits `3` instead of surfacing a traceback as lint findings; shipped agents declare `tools:` instead of inheriting. **Tag cut 2026-08-02** → `e6c6539d`; GitHub Release published as a **pre-release** (the plugin is a declared pre-1.0 preview), closing finding M6. |
...
| `framework/v0.21.0` | CHG-RT-001 close (MINOR) | Framework spec — CHG layer brought to per-layer parity with the 8 SDD layers. New CHG crew entry in `REVIEW_CREWS.yaml` (integration_lead:30 / architect:20 / chaos_engineer:15 / operator:15 / auditor:10 / security_engineer:10 = 100). 6 new playbook files under `framework/playbooks/09_CHG/`. `REVIEW_TEAM.md` §Playbooks notes CHG playbook contract. CHG remains an overlay (not a lifecycle layer per `framework/governance/chg/README.md`). |
| `claude-code-plugin/v0.20.1` | platform README Version cell drift fix (PATCH) | Claude Code plugin — fixed `platforms/claude-code-plugin/README.md` Platform info table `Version` cell that had drifted to `0.6.3` since plugin v0.7.0 (`scripts/sync-version-refs.sh` awk'd only bare `^X.Y.Z$` lines, missing inline table cells). Cell canonicalized to tag form `claude-code-plugin/v<X.Y.Z>`; sync script extended to also propagate the tag form in `platforms/claude-code-plugin/README.md` and `platforms/hermes/README.md`. Closes the recurrence class. Same drift bug in `platforms/hermes/README.md` (`hermes/v0.1.0` vs `0.3.0`; framework spec `0.1.0` vs `0.21.1`) flagged in `plans/FRAMEWORK-TODO.md` as `HERMES-README-VERSION-DRIFT`; tag form will auto-fix at Hermes's next bump. |
| `claude-code-plugin/v0.21.0` | SAGA-PARITY-001 Phase 4 (MINOR) | Claude Code plugin — the 6 remaining layer autopilots `doc-{ears,bdd,adr,spec,tdd,iplan}-autopilot` migrated from the legacy in-session generation loop to the preemptive saga-driver pattern already used by `brd`/`prd`. Each `## Workflow` is now `### Saga-driven generation loop` (`review_mode: team`, invokes `saga_driver.py --layer <NN_TYPE>`) + `### Linear Pipeline` (`review_mode: single_pass`, the prior steps verbatim). `review_mode` added to the 6 SKILLs' `adapts:` + reconciled into `doc-prd-autopilot`. New `tests/conformance/platforms/test_autopilot_saga_parity.py` guards all 8. Closes the divergence where a user-invoked autopilot diverged from the saga path the acceptance harness tests. |
| `claude-code-plugin/v0.22.0` | MODEL-PRECHECK-ROLLOUT (MINOR) | Claude Code plugin — the 8 layer autopilots now surface the per-layer model recommendation at the entry point. New `## Model precheck` section (before `## Workflow`) reads `model.per_layer`/`model.default`/`model.precheck` from `.claude/aidoc-flow.config.yaml` and PRINTS the recommendation + `/model <rec>` switch command (it does not compare against the session model — a skill can't read its own model id). `precheck`: `warn` (print) / `silent` / `block` (print + confirm). Step-1 saga directive reworded to "first orchestration action" so the notice runs before the driver. New `tests/conformance/platforms/test_model_precheck.py`. Closes the documented-but-unimplemented `precheck` behavior; base/audit/fixer deferred (headless under the driver). |
| `claude-code-plugin/v0.23.0` | YAML-BDD-SCHEMA PR-5 (MINOR) | Claude Code plugin — the four `doc-bdd*` skills (`doc-bdd`, `-audit`, `-fixer`, `-autopilot`) rewritten to author + validate the **YAML-native BDD scenario** form (structured `scenarios:` list with `id`/`type`/`priority`/element-level `ears:` list/given-when-then phase lists/spec_trace + a `feature:` block with no `ears`) instead of Gherkin `@`-tags. Audit dimensions, fixer content phases, and autopilot generation updated; `BDD-SCHEMA-001` + the `ears:`-as-required-upstream contract reflected. Completes the YAML-BDD arc on the authoring side (framework spec `0.29.0`, corpus BDD-01 migrated in PR-4). |
| `claude-code-plugin/v0.23.1` | H-14 GD-05 plugin strip (PATCH) | Claude Code plugin — the review lens now honors the `REVIEW_TEAM.md` author-self-claim strip MUST (GD-05, framework `0.33.0`). The plugin's agentic lens reads the artifact directly, so it de-anchors by an explicit disregard instruction (not physical strip, which it cannot do): the 9 `doc-*-audit` fan-out briefs + strip sections, the 9 `doc-*-fixer` inline lens briefs, `review-team`, and `traceability-auditor` now instruct the lens to not read, cite, or weight `*_ready_score`/`*_score`/`readiness_score`/`audit_score`/`gate_ready` when forming its `lens_score`. Closes the plugin-side analog of the Hermes content-blindness (D-0051). |
| `claude-code-plugin/v0.23.4` | FRWK-REVIEW-002 PR-B (PATCH) | Claude Code plugin — plugin mechanics from the 2026-07-09 review: removed the dead `--threshold` flag from `saga_driver.py` + all 8 autopilot command blocks (the real gate is the `audit_threshold` knob), added `CHG` to the advisory hook's case-arm, wired `/aidoc-flow:save-plan` to the `work_plans_dir` config key (legacy CLAUDE.md fallback), promoted the released `0.23.x` changelog entries to `##` sections, added `spec`/`tdd` to `plugin.json` keywords, documented the CHG model-precheck exclusion. No framework change. |
| `claude-code-plugin/v0.23.3` | FRWK-REVIEW-002 PR-A (PATCH) | Claude Code plugin — skill-content drift from the 2026-07-09 plugin + core-docs review: audit-report path unified across all 18 audit/fixer skills (68 sites; legacy versioned name → relocated `.aidoc/audit/<NN>_<LAYER>-audit.md`), iteration-cap governance citation corrected (`REVIEW_SAGA` → `REVIEW_REMEDIATION_FLOW`) and backported to all 9 audits, `review_mode` adaptation made consistent, phantom `bdd_to_gherkin.py` reference removed from `doc-bdd`, verdict-outcome clarifier propagated, 9 creator/autopilot descriptions disambiguated. No framework change. |
| `claude-code-plugin/v0.23.2` | PLUGIN-PROD-READINESS-001 (PATCH) | Claude Code plugin — production-readiness batch (D-0060). **BLOCKER fix:** the 9 `doc-*-audit` skills + `agents/synthesizer.md` resolved their vendored playbooks / `REVIEW_TEAM.md` via `${CLAUDE_PLUGIN_ROOT}/../../framework/…`, which escapes the plugin root and fails in every distributed install (the weighted-crew review collapsed to zero coverage) — dropped the `/../../` (11 refs). Plus: reconciled `doc-ears` + `doc-ears-audit` to the D54-F04 latency-vs-non-latency quantification model; bumped the deprecated-stub removal milestone `v0.7.0 → v1.0.0` (8 occurrences); added a "known lint baseline" note to the url-shortener example README + dropped a phantom `docs/.version` line. No framework change. |
| `claude-code-plugin/v0.20.0` | bug-report/feedback LLM-drafted issues (MINOR) | Claude Code plugin — `/aidoc-flow:bug-report` and `/aidoc-flow:feedback` accept a user prompt argument and have the LLM draft a concise title + structured body from the prompt + conversation context (recent commands, errors, files) + environment stamp. Drafted title + body URL-encoded into `?title=&body=`; preview shown in chat; user reviews on github.com and clicks Submit; plugin never auto-submits. Refined `.github/ISSUE_TEMPLATE/bug_report.md` and `feedback.md` to match. Secret-redaction guardrail in command prose. Supersedes the in-flight v0.19.1 PATCH design. |
| `claude-code-plugin/v0.19.0` | PLUGIN-USER-COMMANDS close (MINOR) | Claude Code plugin — 11 user-facing commands under `commands/`: meta (`about`, `help`, `bug-report`, `contact-us`, `feedback`), workflow (`status`, `next`), lifecycle (`uninstall`), config (`configure`, `budget`, `model`). Optional project-local `.claude/aidoc-flow.config.yaml` with schema in `docs/CONFIG.md`. New `.github/ISSUE_TEMPLATE/feedback.md` backend for `/feedback`. Conformance test `tests/conformance/platforms/test_plugin_config_schema.py` keeps the doc and the three config command files in sync on enums. Honest caveats baked in: `/budget` is a behavior knob (not a token cap), `/model` is advisory (cannot switch session model), `/uninstall` is a guided exit (native `/plugin uninstall` does removal). D-0033 a/b/c. |
| `claude-code-plugin/v0.18.0` | CHG-RT-001 close (MINOR) | Claude Code plugin — companions framework/v0.21.0. `doc-chg-audit/SKILL.md` (200 → 693 lines) gains `## Review Mode` + `## Saga interaction` + `## Break-circuit policy` + `## Content Sub-Checks` (A1/A2/A3/BA1/SE1) + playbook injection. `doc-chg-fixer/SKILL.md` (125 → 344 lines) gains `## Remediate Mode` + `## Saga interaction` + `## Break-circuit policy`. `doc-chg-autopilot/SKILL.md` (116 → 191 lines) gains saga-driven generation loop invoking `python3 saga_driver.py --layer 09_CHG`. `tools/saga_driver.py` `_LAYER_CREWS` gains `09_CHG` entry. Live CHG cascade verification against url-shortener pending. |
