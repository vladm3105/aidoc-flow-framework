...
| `claude-code-plugin/v0.10.1` | Plugin release `0.10.1` close (synthesizer schema + saga observability, PATCH) | Claude Code plugin — two infrastructure tightenings surfaced by the SPEC-RT-001 live cascade (2026-06-09). (1) Synthesizer agent contract (`agents/synthesizer.md`) now lists `check` as a required field on every `findings[]` entry in `verdict.json`; downstream consumers (fixers, traceability matrices, observability dashboards) read `findings[*].check` to roll up by playbook check, and dropping the field broke every consumer. SPEC's synthesizer happened to drop it while ADR/BDD happened to preserve it — pure LLM stochasticity; the spec gap allowed both behaviors. New conformance test `test_synthesizer_verdict_schema.py` (3 tests) enforces the now-explicit contract on every committed verdict.json (synthetic verdicts marked `synthetic: true` exempted). (2) Saga driver (`tools/saga_driver.py`) now stamps `dispatch:<phase>` / `complete:<phase>` events to a new `saga.events[]` field on every subprocess invocation, closing the journal observability gap where fixer cycles left no trace. New `append_event(saga, kind, **extra)` helper. No SKILL changes. |
| `claude-code-plugin/v0.10.2` | Plugin release `0.10.2` close (SAGA-DETERMINISM-001, PATCH) | Claude Code plugin — saga driver reconciles `transitions[]` and walks `saga.status` deterministically when the audit SKILL's LLM-driven journal stamping is incomplete. Surfaced by SPEC-RT-001 worktree cascade (2026-06-09): same byte-identical audit SKILL prompt produced 35 per-branch transitions on ADR but 0 on SPEC across 3 audit cycles. The SKILL prompt asks for two writes per branch event — update `branches[<lens>]` dict + append a transition — and the LLM stochastically does the first while skipping the second. Result: `saga.status` stuck at `FANOUT_STARTED` (since `FANOUT_STARTED → FANIN_REDUCED` is illegal per `_ALLOWED_TRANSITIONS`), harness B2 reported FAIL despite `verdict.json` `combined_status: PASS`. New `reconcile_post_audit(ctx, saga)` helper backfills missing per-branch transitions from `branches[]` ground truth (marked `reconciled: true`) and walks run-scope status `FANOUT_STARTED → BRANCH_RUNNING → BRANCH_COMPLETED` via the allowed-transition graph, so the existing PASS code path (`BRANCH_COMPLETED → FANIN_REDUCED`) fires correctly. Idempotent (no-op when SKILL stamped completely; partial-stamp also handled). 6 unit tests at `tests/unit/test_saga_reconcile_post_audit.py` including a regression test on the verbatim captured SPEC saga.json fixture. Architecturally completes the cooperative→preemptive migration started by SAGA-PARITY-001 Phase 2 Amendment 1: saga-state-machine bookkeeping is now deterministic at the driver, not LLM-delegated. |
