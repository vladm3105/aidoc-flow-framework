...
def _create_canonical_scaffold(root: Path) -> None:
    ...
    (root / "skills/persona_mappings.yaml").write_text(
        'version: "1.0"\ncreation:\n  brd:\n    personas: [architect]\n    mode: sequential\n',
        encoding="utf-8",
    )
...
def test_scaffold_update_overwrites_stale_files(tmp_path: Path) -> None:
    ...
    (canonical_root / "skills/personas/architect.md").write_text(
        "architect persona v2", encoding="utf-8"
    )
    ...
    assert (project_root / "UCX/skills/personas/architect.md").read_text(
        encoding="utf-8"
    ) == "architect persona"
    ...
    assert (project_root / "UCX/skills/personas/architect.md").read_text(
        encoding="utf-8"
    ) == "architect persona v2"
    assert (project_root / "UCX/templates/layers/01_BRD/BRD-MVP-TEMPLATE.yaml").read_text(
        encoding="utf-8"
    ) == "doc_id: BRD-01-v2\n"
...
def test_scaffold_update_protects_persona_mappings(tmp_path: Path) -> None:
    ...
    (canonical_root / "skills/persona_mappings.yaml").write_text(
        'version: "2.0"\ncreation:\n  brd:\n    personas: [architect]\n    mode: sequential\n',
        encoding="utf-8",
    )
