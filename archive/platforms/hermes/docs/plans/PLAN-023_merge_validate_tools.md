...
| File | Change |
|------|--------|
| `mcp_ucx/src/mcp_server/tool_registry.py` | Merge tool defs, handlers, next-action, pipeline, `passed`→`is_valid` split |
| `mcp_ucx/src/mcp_server/cli/main.py` | Merge CLI, deprecate validate-fix, add --validation-report/--executor/--timeout |
...
10. `pytest mcp_ucx/tests/ -x` — all tests pass
