...
- `remediate`
- `validate-fix`
- `remediate --fix` (`remediate-fix` CLI alias is deprecated)
...
2. `validate-fix` generates `_validated` derived artifact(s) from source.
3. `remediate --fix` generates versioned `_remediate_v{N}` derived artifact(s).
...
- `{DOC_ID}.ucx.remediate.json`
- `{DOC_ID}.ucx.remediate.txt`
...
- `{DOC_ID}.ucx.validate_fix.json`
- `{DOC_ID}.ucx.validate_fix.txt`
- one or more `*_validated.{ext}` derived artifacts (`.md`, `.yaml`, `.yml`)
...
- `{DOC_ID}.ucx.remediate_fix.json`
- `{DOC_ID}.ucx.remediate_fix.txt`
- one or more `*_remediate_v{N}.{ext}` derived artifacts (`.md`, `.yaml`, `.yml`)
