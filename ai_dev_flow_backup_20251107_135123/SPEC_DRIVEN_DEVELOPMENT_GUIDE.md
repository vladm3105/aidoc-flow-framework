...
| Document Type | Template Location | Focus Area |
|---------------|------------------|------------|
| **BRD** | `brds/BRD-000_TRACEABILITY_MATRIX-TEMPLATE.md` | Business objectives, stakeholder needs |
| **PRD** | `prd/PRD-000_TRACEABILITY_MATRIX-TEMPLATE.md` | Product features, user stories |
| **EARS** | `ears/EARS-000_TRACEABILITY_MATRIX-TEMPLATE.md` | Formal WHEN-THE-SHALL-WITHIN requirements |
| **BDD** | `bbds/BDD-000_TRACEABILITY_MATRIX-TEMPLATE.md` | Test scenarios, acceptance criteria |
| **ADR** | `adrs/ADR-000_TRACEABILITY_MATRIX-TEMPLATE.md` | Architecture decisions, technology choices |
| **SYS** | `sys/SYS-000_TRACEABILITY_MATRIX-TEMPLATE.md` | System requirements, technical specifications |
| **REQ** | `reqs/REQ-000_TRACEABILITY_MATRIX-TEMPLATE.md` | Atomic requirements, implementation specs |
| **IMPL** | `impl_plans/IMPL-000_TRACEABILITY_MATRIX-TEMPLATE.md` | Project plans, WHO/WHEN coordination |
| **CTR** | `contracts/CTR-000_TRACEABILITY_MATRIX-TEMPLATE.md` | API contracts, interface definitions |
| **SPEC** | `specs/SPEC-000_TRACEABILITY_MATRIX-TEMPLATE.md` | Technical blueprints, YAML specifications |
| **TASKS** | `ai_tasks/TASKS-000_TRACEABILITY_MATRIX-TEMPLATE.md` | Code generation plans, implementation steps |
| **Complete** | `TRACEABILITY_MATRIX_COMPLETE-TEMPLATE.md` | End-to-end workflow, all document types |

### When to Create/Update Matrices

**Create matrices**:
1. **Milestone Completions**: After completing major development phases (e.g., Phase 1 complete with 15 ADRs)
2. **Release Preparation**: Before production releases to ensure complete traceability
3. **Audit Requirements**: For compliance and quality audits requiring documentation trails
4. **Project Handoffs**: When transitioning work between teams or phases

**Update matrices**:
1. **After Document Creation**: When creating 5+ documents of the same type
2. **Before Major Changes**: Before architectural or requirement changes requiring impact analysis
3. **Quarterly Reviews**: Regular documentation health checks (coverage, orphans, gaps)
4. **Post-Incident**: After production issues requiring root cause traceability

### Matrix Creation Workflow

1. **Select Template**: Choose appropriate matrix template from document type folder
2. **Copy to Production Location**: Copy template to project docs folder
3. **Populate Matrix**: Fill in actual document data
4. **Validate Matrix**: Run validation scripts
...
- Example: For complex specs exceeding 100K tokens, create SPEC-003_part1.yaml (interfaces/state), SPEC-003_part2.yaml (performance/verification); reference as [SPEC-003_part1.yaml](./specs/services/SPEC-003_part1.yaml), with [index.md](./index.md) enumerating splits and dependencies
