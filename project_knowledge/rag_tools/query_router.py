#!/usr/bin/env python3
"""Query router for project_knowledge contracts.

Routes queries to project_knowledge contract types based on query intent:
...
def classify_query(query: str) -> RoutingDecision:
    ...
    if max_score == 0:
        # Default to project_knowledge RAG search for unknown queries
        return RoutingDecision(
            service="rag",
            query_type=QueryType.UNKNOWN,
            confidence=0.5,
            reasoning="No clear indicators, defaulting to project_knowledge RAG search"
        )
    ...
    if query_type == QueryType.FACTUAL:
        service = "rag"
        reasoning = "Factual/documentation query - using project_knowledge RAG search"
    else:
        service = "graph"
        reasoning = f"{query_type.value.title()} query - using project_knowledge graph context"
...
def execute_query(query: str, decision: RoutingDecision) -> dict:
    """Build project_knowledge contract payload for the selected route.
    ...
    Returns:
        Dict payload compatible with project_knowledge contracts.
    """
...
def main():
    parser = argparse.ArgumentParser(description="Route queries to project_knowledge contracts")
