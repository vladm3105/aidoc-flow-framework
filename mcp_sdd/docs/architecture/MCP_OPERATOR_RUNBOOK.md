...
4. When the validation report contains errors, confirm that derived `_validated` artifact and `validate_fix_report.*` files are also produced automatically. These fix artifacts were previously generated by the separate `sdd_validate_fix` tool, which is now merged into `sdd_validate`.
...
> **Deprecated**: `sdd_validate_fix` is merged into `sdd_validate` as of v1.13.0. Use `sdd_validate` instead. When validation errors are found, `sdd_validate` now produces the derived `_validated` artifact and `validate_fix_report.*` files automatically. See section 3.5.
