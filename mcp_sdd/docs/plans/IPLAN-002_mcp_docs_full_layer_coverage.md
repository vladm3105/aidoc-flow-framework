...
- Any behavior not currently implemented in `mcp_sdd/src/mcp_server` must be documented only in a clearly labeled "Future State" subsection.
...
|---|---|---|---|
| L0 | Navigation and Inventory | Docs index and map | `mcp_sdd/docs/README.md` |
| L1 | Architecture Overview | Runtime architecture and boundaries | `mcp_sdd/docs/architecture/MCP_RUNTIME_ARCHITECTURE.md` |
| L2 | CLI and Tool Surface | Command contracts and examples | `mcp_sdd/docs/architecture/MCP_CLI_REFERENCE.md` |
| L3 | Source Input Contracts | Source modes, precedence, and constraints | `mcp_sdd/docs/specs/SPEC-005_mcp_source_input_ingestion_contracts.md` |
| L4 | Creation Flow Contracts | `init` + `create-build` end-to-end contracts | `mcp_sdd/docs/specs/SPEC-006_mcp_creation_flow_operational_contracts.md` |
| L5 | Review/Remediation Ops | `review-build` and remediation handoff operations | `mcp_sdd/docs/specs/SPEC-007_mcp_review_remediation_operational_contracts.md` |
| L6 | Policy Layer | Versioning, deprecation, compatibility policy | `mcp_sdd/docs/policies/DOC_COMPATIBILITY_AND_DEPRECATION_POLICY.md` |
| L7 | Validation and Quality Gates | Documentation QA and release gates | `mcp_sdd/docs/policies/DOC_QUALITY_GATES.md` |
| L8 | Runbooks | Operator runbooks and troubleshooting | `mcp_sdd/docs/architecture/MCP_OPERATOR_RUNBOOK.md` |
| L9 | Traceability and Audit | Coverage matrix and evidence checklist | `mcp_sdd/docs/plans/DOC-COVERAGE-MATRIX-001_mcp_layers.md` |
...
- Create `mcp_sdd/docs/README.md` with:
  - Layer map (L0-L9)
...
Acceptance criteria:
- `create-build`, `review-build`, `init` parameter contracts match `mcp_sdd/src/mcp_server/cli/main.py` exactly.
...
1. Runtime code and tests under `mcp_sdd/src/mcp_server` and `mcp/tests`
2. Canonical specs (`mcp_sdd/docs/specs`)
3. Policies (`mcp_sdd/docs/policies`)
4. Runbooks and architecture guides (`mcp_sdd/docs/architecture`)
5. Plans (`mcp_sdd/docs/plans`)
...
- Every behavioral rule must cite concrete module paths under `mcp_sdd/src/mcp_server`.
...
- Any PR touching:
  ...
  - `mcp_sdd/src/mcp_server/prompts/context_builder.py`
