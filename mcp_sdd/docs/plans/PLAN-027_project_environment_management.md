...
mcp_sdd (UCX) works with multiple projects per session — each tool call specifies `--project`. Project `.env` files contain API keys and provider credentials needed by executors (claude, codex, etc.), but the server never loads them. Executors only get `os.environ` + static `config.env`. If the parent shell doesn't have the keys exported, executor calls fail silently.
...
**`mcp_sdd/src/mcp_server/env_manager.py`** (new):
...
**`mcp_sdd/docs/CHANGELOG/CHANGELOG_v1.17.0.md`** (new):
...
| File | Action |
|------|--------|
| `mcp_sdd/pyproject.toml` | Edit — add python-dotenv |
| `mcp_sdd/src/mcp_server/env_manager.py` | **Create** |
| `mcp_sdd/src/mcp_server/executor/cli_runner.py` | Edit — add project_env param, fix merge (lines 36-39) |
| `mcp_sdd/src/mcp_server/executor/dispatcher.py` | Edit — add project_env param, forward (line 30) |
| `mcp_sdd/src/mcp_server/tool_registry.py` | Edit — env load in _maybe_run_executor (line 451), Tool def (line 152) + dispatch (line 756) |
| `mcp_sdd/src/mcp_server/preflight/runner.py` | Edit — enhanced .env check (lines 200-203) |
| `mcp_sdd/src/mcp_server/cli/main.py` | Edit — env-show subcommand |
| `mcp_sdd/tests/unit/test_env_manager.py` | **Create** — 12 tests |
| `mcp_sdd/tests/integration/test_executor_env.py` | **Create** — 3 tests |
| `mcp_sdd/tests/unit/test_server.py` | Edit — tool count 22→23 |
| `mcp_sdd/tests/unit/test_preflight_runner.py` | Edit — env check test |
| `mcp_sdd/docs/CHANGELOG/CHANGELOG_v1.17.0.md` | **Create** |
| `mcp_sdd/docs/README.md` | Edit — version bump, env section |
| `mcp_sdd/docs/ROADMAP.md` | Edit — version bump, v1.17.0 section |
| `mcp_sdd/docs/architecture/MCP_CLI_REFERENCE.md` | Edit — env-show command |
| `mcp_sdd/docs/architecture/MCP_OPERATIONAL_FLOWS.md` | Edit — env loading flow |
| `mcp_sdd/docs/architecture/MCP_UNIFIED_CONTEXT_FRAMEWORK.md` | Edit — env isolation model |
...
# Unit tests
PYTHONPATH=mcp_sdd/src python -m pytest mcp_sdd/tests/ -x -q

# CLI smoke test
PYTHONPATH=mcp_sdd/src python -m mcp_server.cli.main env-show --project /opt/data/b-local/b-local-docs
PYTHONPATH=mcp_sdd/src python -m mcp_server.cli.main env-show --project /opt/data/b-local/b-local-docs --format json

# Preflight should now report env keys
PYTHONPATH=mcp_sdd/src python -c "
...
**`mcp_sdd/src/mcp_server/project_context.py`** (new):
...
| File | Action |
| --- | --- |
| `mcp_sdd/src/mcp_server/project_context.py` | **Create** — session state, config default, resolve chain |
| `mcp_sdd/src/mcp_server/tool_registry.py` | Edit — `_PROJECT_TOOLS`, injection, 2 tool defs + dispatch, descriptions |
| `mcp_sdd/src/mcp_server/executor/registry.py` | Edit — config format migration with backward-compat shim |
| `mcp_sdd/src/mcp_server/cli/main.py` | Edit — env var fallback for `--project`, `get-project` subcommand |
| `mcp_sdd/src/mcp_server/server.py` | Edit — update docstring tool count |
| `mcp_sdd/tests/unit/test_project_context.py` | **Create** — ~12 tests |
| `mcp_sdd/tests/unit/test_tool_injection.py` | **Create** — ~5 tests |
| `mcp_sdd/tests/unit/test_server.py` | Edit — tool count 23→25 |
...
SDD_DEFAULT_PROJECT=/opt/data/b-local/b-local-docs \
  PYTHONPATH=mcp_sdd/src python -m mcp_server.cli.main preflight --context any

# CLI get-project
SDD_DEFAULT_PROJECT=/opt/data/b-local/b-local-docs \
  PYTHONPATH=mcp_sdd/src python -m mcp_server.cli.main get-project

# Explicit still overrides
PYTHONPATH=mcp_sdd/src python -m mcp_server.cli.main preflight \
