...
- SPEC-001: mcp_sdd/docs/specs/SPEC-001_mcp_core_architecture_workflow_contracts.md
- SPEC-002: mcp_sdd/docs/specs/SPEC-002_mcp_review_scoring_handoff_identity_contracts.md
- SPEC-003: mcp_sdd/docs/specs/SPEC-003_mcp_creation_validation_profile_contracts.md
- SPEC-004: mcp_sdd/docs/specs/SPEC-004_mcp_reporting_lineage_artifact_contracts.md
...
- Canonical contract docs exist under mcp_sdd/docs/specs as the only normative source.
...
|------------|------|--------|
| mcp_sdd/docs/specs/SPEC-001_mcp_core_architecture_workflow_contracts.md | Blocks | Complete |
| mcp_sdd/docs/specs/SPEC-002_mcp_review_scoring_handoff_identity_contracts.md | Blocks | Complete |
| mcp_sdd/docs/specs/SPEC-003_mcp_creation_validation_profile_contracts.md | Blocks | Complete |
| mcp_sdd/docs/specs/SPEC-004_mcp_reporting_lineage_artifact_contracts.md | Blocks | Complete |
...
- Contract docs remain authoritative under `mcp_sdd/docs/specs`.
- Runtime implementation code exists only under `mcp_sdd/src/mcp_server`.
- `mcp_sdd/skills/` and `mcp_sdd/prompts/templates/` are the canonical scaffold source used by `mcp init` to create project-specific UCX files; they are never loaded by the runtime directly.
...
- Project onboarding: `mcp init --project {project_root}` scaffolds `{project_root}/docs/UCX/` from canonical templates in `mcp_sdd/skills/` and `mcp_sdd/prompts/templates/`; existing project files are never overwritten.
...
- MCP bundled files under `mcp_sdd/skills/` and `mcp_sdd/prompts/templates/` are the `mcp init` scaffold source; they are never loaded at runtime.

Implementation mapping from UCX_v1_archive:
- `UCX_v1_archive/skills/*` -> `mcp_sdd/skills/personas/*`
- `UCX_v1_archive/review/*`, `UCX_v1_archive/creation/*`, `UCX_v1_archive/remediation/*` -> `mcp_sdd/prompts/templates/{review|creation|remediation}/*`
- `UCX_v1_archive/ucx/*` modules -> `mcp_sdd/src/mcp_server/*` package modules
- `UCX_v1_archive/tests/*` -> `mcp_sdd/tests/*` (with explicit `contract/` row coverage)
- `UCX_v1_archive/docs/plans/*` and standards docs -> `mcp_sdd/docs/architecture/*` and `mcp_sdd/docs/plans/*` as applicable
...
- Implement `mcp init --project {project_root}` command: scaffold `{project_root}/docs/UCX/` by copying canonical templates from `mcp_sdd/skills/` and `mcp_sdd/prompts/templates/`; skip files that already exist (no overwrite).
...
- Ensure no runtime code path loads from `mcp_sdd/skills/` or `mcp_sdd/prompts/templates/`.
...
- No runtime code path loads from `mcp_sdd/skills/` or `mcp_sdd/prompts/templates/`; verified by test.
...
- Execute `mcp_sdd/docs/plans/TEST-CHECKLIST-001_mcp_new_contract_rows.md` (TC-001..TC-014) and collect pass/fail plus evidence links per test case.
...
- Publish checklist evidence summary referencing `mcp_sdd/docs/plans/TEST-CHECKLIST-001_mcp_new_contract_rows.md`.
- Lock repository-level policy for legacy report handling (`import|ignore|fail-fast`) before cutover.
- Record legacy report handling policy in `mcp_sdd/docs/policies/legacy_report_policy.md`.
...
- Legacy report policy artifact exists at `mcp_sdd/docs/policies/legacy_report_policy.md` and matches configured runtime policy.
...
- Checklist evidence: `mcp_sdd/docs/plans/TEST-CHECKLIST-001_mcp_new_contract_rows.md`
- Compliance artifact: `mcp_sdd/docs/plans/COMPLIANCE-REPORT-001_mcp_canonical_contracts.md`
- Release-readiness artifact: `mcp_sdd/docs/plans/RELEASE-READINESS-001_mcp_cutover_status.md`
...
| 1.8 | 2026-03-24 | AI Collaboration | Implemented `mcp init --project` scaffolding with no-overwrite copy semantics from canonical `mcp/skills` and `mcp_sdd/prompts/templates` to project `docs/UCX`; added CLI + scaffold unit tests and revalidated MCP contract/integration slices. |
...
| 2.4 | 2026-03-24 | AI Collaboration | Published canonical compliance and release-readiness artifacts for Workstream G/H, added explicit legacy report policy contract helpers with tests, and recorded repository policy artifact `mcp_sdd/docs/policies/legacy_report_policy.md`. |
