# PLAN-016: mcp_sdd Cross-Section Validation & BRD Template Improvements

## Context

During BRD-04 (Data Model & Ledger) review, we found 9 friction points that stem from missing machine-enforceable validation in `mcp_sdd`. Current `sdd_validate` only checks frontmatter fields, required tags, and section presence — it has no cross-section consistency rules.
...
**Goal**: Add two-tier cross-section validation to `mcp_sdd` server via `sdd_validate`, update the BRD template, and establish patterns reusable across all SDD layers.
...
```
mcp_sdd/src/mcp_server/validation/
...
**File**: `mcp_sdd/src/mcp_server/validation/runner.py`
...
- `mcp_sdd/templates/BRD-TEMPLATE.yaml`
...
**File**: `mcp_sdd/templates/BRD-MD-TEMPLATE.md` (~120 lines)
...
- `mcp_sdd/tests/unit/test_cross_section.py` (~200 lines) — generic rule tests across multiple doc types
- `mcp_sdd/tests/unit/test_brd_rules.py` (~250 lines) — BRD-specific rule tests
...
|------|--------|-----------|------|
| `mcp_sdd/src/mcp_server/validation/cross_section.py` | **Create** | ~200 lines | Generic (all layers) |
| `mcp_sdd/src/mcp_server/validation/brd_rules.py` | **Create** | ~200 lines | BRD-specific |
| `mcp_sdd/src/mcp_server/validation/runner.py` | **Modify** | +80-100 lines | Infrastructure |
| `mcp_sdd/src/mcp_server/validation/__init__.py` | **Modify** | +4 exports | Infrastructure |
| `mcp_sdd/templates/BRD-TEMPLATE.yaml` | **Modify** | +40 lines | BRD template |
| `ai_dev_ssd_flow/01_BRD/BRD-TEMPLATE.yaml` | **Sync** | Same | BRD template |
| `mcp_sdd/templates/BRD-MD-TEMPLATE.md` | **Create** | ~120 lines | BRD template |
| `ai_dev_ssd_flow/DIAGRAM_STANDARDS.md` | **Modify** | +25 lines | Framework standard |
| `mcp_sdd/tests/unit/test_cross_section.py` | **Create** | ~200 lines | Generic tests |
| `mcp_sdd/tests/unit/test_brd_rules.py` | **Create** | ~250 lines | BRD tests |
