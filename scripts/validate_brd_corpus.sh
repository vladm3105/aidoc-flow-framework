#!/bin/bash
# =============================================================================
# BRD Corpus Validation Script (Pre-PRD Quality Gate)
# =============================================================================
...
echo "=========================================="
echo "BRD Corpus Validation (Pre-PRD Gate)"
echo "=========================================="
...
check_placeholders() {
  ...
  for pattern in "${patterns[@]}"; do
    ...
    done < <(grep -rniE "$pattern" "$BRD_DIR" 2>/dev/null | grep -v "BRD_CORPUS_VALIDATION\|BRD_POST_CREATION_VALIDATION\|BRD_VALIDATION_RULES\|BRD-000_index" || true)
  done
...
check_diagrams() {
  ...
  if [[ $no_diagrams -eq 0 ]]; then
    echo -e "${GREEN}  ✓ All BRDs have diagrams${NC}"
  fi
  echo ""
}
...
check_duplicates() {
  ...
  # Check for element IDs defined in wrong files
  ...
  for f in "$BRD_DIR"/BRD-[0-9]*_*.md "$BRD_DIR"/BRD-[0-9]*/BRD-[0-9]*.md; do
    ...
    # These are either cross-references (OK) or misplaced definitions (Error)
    ...
    done < <(grep -nE "BRD\.[0-9]+\.[0-9]+\.[0-9]+" "$f" 2>/dev/null | grep -E ":[0-9]+:\|" || true)
  done
  shopt -u nullglob
