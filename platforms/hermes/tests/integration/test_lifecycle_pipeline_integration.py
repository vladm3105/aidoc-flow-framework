...
def test_staged_workflow_resolves_required_artifacts_across_full_lifecycle(tmp_path: Path) -> None:
    ...
    _touch(
        tmp_path / build_derived_artifact_name(doc_id=doc_id, slug=slug, stage="validation-fixed")
    )
    ...
    _touch(
        tmp_path
        / build_lifecycle_report_name(
            doc_id=doc_id, source_stage="validation-fixed", report_type="remediation", version=1
        )
    )

    create_inputs = resolve_operation_inputs(
        folder=tmp_path, doc_id=doc_id, slug=slug, operation="create"
    )
    validate_inputs = resolve_operation_inputs(
        folder=tmp_path, doc_id=doc_id, slug=slug, operation="validate"
    )
    validate_fix_inputs = resolve_operation_inputs(
        folder=tmp_path, doc_id=doc_id, slug=slug, operation="validate_fix"
    )
    review_inputs = resolve_operation_inputs(
        folder=tmp_path, doc_id=doc_id, slug=slug, operation="review"
    )
    remediate_content_inputs = resolve_operation_inputs(
        folder=tmp_path, doc_id=doc_id, slug=slug, operation="remediate_content"
    )
    remediate_apply_inputs = resolve_operation_inputs(
        folder=tmp_path, doc_id=doc_id, slug=slug, operation="remediate_apply"
    )

    assert create_inputs["status"] == "ready"
    assert validate_inputs["source_artifact"] == build_source_artifact_name(
        doc_id=doc_id, slug=slug
    )
    assert validate_fix_inputs["validation_report"] == build_validation_report_name(doc_id=doc_id)
    assert review_inputs["validation_fixed_artifact"] == build_derived_artifact_name(
        doc_id=doc_id, slug=slug, stage="validation-fixed"
    )
    assert remediate_content_inputs["review_report"] == build_family_report_name(
        doc_id=doc_id, family="audit", version=1
    )
...
def test_review_operation_fails_explicitly_when_validation_fixed_artifact_absent(
    tmp_path: Path,
) -> None:
    doc_id = "SPEC-002"
    ...
    assert result["expected_filename_pattern"] == build_derived_artifact_name(
        doc_id=doc_id, slug=slug, stage="validation-fixed"
    )


def test_versioned_report_write_retries_after_collision_and_allocates_next_version(
    tmp_path: Path,
) -> None:
    _touch(
        tmp_path / build_family_report_name(doc_id="SPEC-003", family="audit", version=1),
        "existing v1",
    )
    ...
    written = write_versioned_report_atomic(
        report_dir=tmp_path,
        report_name_factory=lambda version: build_family_report_name(
            doc_id="SPEC-003", family="audit", version=version
        ),
        content="new content",
