...
def is_functional_change(files: list[str]) -> tuple[bool, str]:
    ...
    for file in files:
        ...
        is_non_functional = any(
            re.search(pattern, file, re.IGNORECASE) for pattern in NON_FUNCTIONAL_PATTERNS
        )
        ...
        is_functional = any(
            re.search(pattern, file, re.IGNORECASE) for pattern in FUNCTIONAL_PATTERNS
        )
...
def main():
    parser = argparse.ArgumentParser(description="Determine if QA testing is required")
