...
This document describes architecture boundaries, data flow, operational modes, constraints, and validation criteria for `framework` execution.
...
- Runtime must support `framework` document flow and naming/traceability policies.
...
4. Full `framework` stage completion succeeds on benchmark document set.
...
- SDD flow assets: `/opt/data/ucx_framework/framework`
