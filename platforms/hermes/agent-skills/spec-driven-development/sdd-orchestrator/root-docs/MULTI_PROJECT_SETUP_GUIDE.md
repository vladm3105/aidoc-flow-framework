...
| **SDD Templates** | `.templates/framework/` | N/A | Symlink only (v2 legacy) |
| | `.templates/framework/` | N/A | Symlink only (v3 recommended) |
...
backup_if_needed "$PROJECT_DIR/.templates/framework"
backup_if_needed "$PROJECT_DIR/.templates/governance"
ln -sf "$FRAMEWORK_DIR/framework" "$PROJECT_DIR/.templates/framework"
...
cp framework/AI_EXPERTS/review.template.yaml docs/AI_EXPERTS/review.yaml
...
ln -s ../../ucx_framework/framework/scripts/pre_commit_hooks/library/pre-commit-config.project.yaml .pre-commit-config.yaml
...
vim framework/01_BRD/BRD-MVP-TEMPLATE.md
...
ls -la ${PROJECT_PATH}/.templates/framework/layers/01_BRD/
...
- Updated project layout to include `.envrc` and `framework/` symlink
- Added UCX CLI commands documentation

### Version 2.1 (2026-02-17T00:00:00)

- **Dual Framework Support**: Added `governance` alongside `framework`
