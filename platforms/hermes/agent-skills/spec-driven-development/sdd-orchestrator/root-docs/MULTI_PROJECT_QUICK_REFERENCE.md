...
- `ucx-sdd-bridge`: lifecycle orchestration for `framework` (MCP-only document layers)
...
# → Copy framework/AI_EXPERTS/review.template.yaml to docs/AI_EXPERTS/review.yaml
...
ls -la /opt/data/project_name/.templates/framework/layers/01_BRD/
...
cat framework/01_BRD/BRD-TEMPLATE.yaml

# 2. Copy to project docs
cp framework/layers/01_BRD/BRD-TEMPLATE.yaml \
...
cat /opt/data/project_name/.templates/framework/layers/01_BRD/BRD-TEMPLATE.yaml

# 2. Copy to project docs
cp .templates/framework/layers/01_BRD/BRD-TEMPLATE.yaml \
...
> **Note**: Autopilot is a v2 artifact. For v3 projects, use the MCP tools (`sdd_create`, `sdd_run_lifecycle`) instead. Autopilot scripts are preserved in `framework/AUTOPILOT/` for legacy projects.

```bash
# Run MVP Autopilot with TDD mode
python3 framework/AUTOPILOT/scripts/mvp_autopilot.py \
  --root . \
  --intent "My MVP" \
  --slug my_mvp \
  --tdd-mode \
