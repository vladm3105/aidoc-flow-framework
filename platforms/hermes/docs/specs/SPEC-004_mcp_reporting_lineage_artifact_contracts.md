...
- Section 4.1 defines saga review artifact family naming for orchestration outputs.
- Section 4.2 defines audit-wrapper/fixer-compatible naming specializations and deterministic selection rules.
- Where both naming families are available for the same workflow handoff, Section 4.2 selection rules take precedence for fixer intake.
...
### 4.1 Saga Review Artifact Family Contract

When review executes in saga-parallel mode, optional orchestration artifacts must use canonical naming rooted in `DOC_ID`.

Canonical saga artifact patterns:

- Saga journal: `{DOC_ID}_{source_stage}_saga_journal_vNNN.json`
- Saga branch summary: `{DOC_ID}_{source_stage}_saga_branch_summary_vNNN.json`
- Saga reducer summary: `{DOC_ID}_{source_stage}_saga_reducer_summary_vNNN.json`
- Saga synthesis summary: `{DOC_ID}_{source_stage}_saga_synthesis_summary_vNNN.json`

Canonical filename examples (runtime-aligned):

- `BRD-00_validation-fixed_saga_branch_summary_v001.json`
- `BRD-00_validation-fixed_saga_reducer_summary_v001.json`
- `BRD-00_validation-fixed_saga_synthesis_summary_v001.json`

Rules:

- Saga artifacts are append-only and versioned.
- Saga artifacts must not replace canonical prompt artifacts (`review_prompt.*`).
- Version allocation follows Section 9 bounded collision policy.
- If saga artifacts are emitted, they must include lineage fields defined in Section 7.
- Saga artifacts are orchestration JSON artifacts and are not required to follow Section 5 markdown frontmatter rules.
- Saga branch summary artifact must be emitted for both `CLOSED` and `ESCALATED` outcomes.
- Saga reducer and synthesis artifacts must be emitted for `CLOSED` outcomes; they may be omitted for `ESCALATED` outcomes.

Failure modes:

- Non-versioned saga artifact filenames in canonical workflows.
- Saga artifact emitted without required lineage metadata.

### 4.2 Audit/Review/Fix Report Family Contract
...
- Source artifact: `{DOC_ID}_{slug}.{ext}` (`.md`, `.yaml`, `.yml`)
- Validation-fixed artifact: `{DOC_ID}_{slug}_validated.{ext}`
- Remediated artifact: `{DOC_ID}_{slug}_remediate_v{N}.{ext}`
- Legacy remediated compatibility artifact: `{DOC_ID}_{slug}_remediate_copy.{ext}` (read-only compatibility)

Rules:

- Source artifact has no stage suffix.
- `_validated`, `_remediate_v{N}`, and `_remediate_copy` suffixes are reserved for MCP-derived copies only.
...
- `branch_summary_path` points to the emitted branch summary artifact.
- `reducer_summary_path` points to the emitted reducer summary artifact when available.
- `synthesis_summary_path` points to the emitted synthesis summary artifact when available.
