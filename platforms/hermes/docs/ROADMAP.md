...
This roadmap defines planned documentation and governance milestones for MCP documentation under **ucx_hermes/docs**.

Historical notes:

- References to `mcp_ucx` in this roadmap identify legacy release history only.
...
- Tool count: 19 → 20 (12 deterministic, 2 orchestration, 6 LLM-dependent) [historical snapshot]
...
- Tool count: 20 → 19 (12 deterministic, 1 orchestration, 6 LLM-dependent) [historical snapshot]
...
- Review phase: max 5 personas per layer (BRD 11→5, PRD 10→5, ADR 7→5, SPEC 6→5, BDD 6→5)
...
- New module: `mcp_ucx/src/mcp_server/env_manager.py` — `.env` loading with mtime-based cache, system variable blocklist, BOM handling, permission warnings
...
- New module: `mcp_ucx/src/mcp_server/project_context.py` — session state, config default, 4-level resolve chain
...
- `litellm` added as optional dependency (`pip install ucx_hermes[api]`)
...
### v1.22.0 - ucx_flow_v3 Readiness (TDD/IPLAN)
...
| Scope | Migrate validation, tooling, and prompts to ucx_flow_v3 layer model with TDD/IPLAN support |
...
- Update validation pipeline to ucx_flow_v3 canonical template root.
...
- plans/PLAN-033_mcp_ucx_v3_readiness.md
...
### v2.1.0 - Parallel Persona Review Saga Orchestration

| Field | Value |
| --- | --- |
| Status | Planned |
| Type | Minor |
| Scope | Hermes fan-out/fan-in persona review with saga retries, compensation, deterministic reducer, and escalation controls |

Planned scope:

- Add parallel multi-persona review orchestration with bounded execution pools.
- Add saga journal and compensation actions for branch timeout, schema, and merge conflicts.
- Add deterministic reducer contract for fan-in finding merge and provenance.
- Integrate saga escalation status into merge gate conditions.
- Add unit and integration coverage for happy and failure paths.

References:

- plans/IPLAN-006_parallel_persona_review_saga_orchestration.md
...
- Added validation runner package under `ucx_hermes/src/mcp_server/validation/` with JSON/TXT report outputs.
...
- This roadmap covers documentation scope under ucx_hermes/docs.
