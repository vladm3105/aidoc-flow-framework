...
Report naming across the SDD framework is inconsistent — 6 different patterns coexist. This plan defines a unified standard and implements it in mcp_ucx. Legacy naming is deprecated with no backward compatibility — clean break.
...
**Status**: Implemented (2026-04-02, mcp_ucx v1.11.0 / framework v0.18.0)

**Target Release**: mcp_ucx v1.11.0 / ucx_framework v0.18.0

**Scope**: Standards document + mcp_ucx implementation. Legacy report files will be deleted, not migrated.
...
| `kb` | UCX Knowledge | `project-knowledge` | FTS5 + semantic search, frontmatter indexing |
...
| Stage Code | Full Name | mcp_ucx Tool | Description |
|------------|-----------|-------------|-------------|
...
- mcp_ucx CHANGELOG v1.11.0
- mcp_ucx ROADMAP — add v1.11.0
...
| `mcp_ucx/src/mcp_server/consistency/runner.py` | Modify — filenames + derived detection | +15 |
| `mcp_ucx/src/mcp_server/link_validation/runner.py` | Modify — filenames | +10 |
| `mcp_ucx/src/mcp_server/prescreening/runner.py` | Modify — filenames | +10 |
| `mcp_ucx/src/mcp_server/scan/runner.py` | Modify — if outputs reports | +5 |
| `mcp_ucx/src/mcp_server/tool_registry.py` | Modify — `_inspect_document_folder` + dispatch paths | +20 |
| `mcp_ucx/tests/unit/test_report_naming.py` | **Create** — naming tests | ~80 |
