...
use `doc-review` for the code/PR dimension, `contract-tester` for contract
conformance, and `trace-check` / `doc-validator` to confirm spec and
...
3. **Spec & test conformance**: does it match the SPEC; does it meet the Test
   Architect's coverage bar; are new tests meaningful (not assertion-free)?
