---
name: doc-prd-fixer
description: Apply fixes to a PRD from the latest doc-prd-audit report - structure, links, element IDs, content, references, and upstream BRD drift. Use after an audit reports issues.
metadata:
  tags:
    - sdd-workflow
    - layer-2-artifact
...
2. **Resolve responsible lenses per finding.** Each blocking finding
   ...
   Lens → agent map for PRD:

   | Lens | Agent |
   |------|-------|
   | `product_owner` | `aidoc-flow:requirements-analyst` |
...
|-------|-------|------------------------|
| 0 — Structure | nested-folder rule | move PRD into `docs/02_PRD/PRD-NN_{slug}/`; rename folder to match ID; fix `../01_BRD/` → `../../01_BRD/` relative links after the move |
| 1 — Missing files | referenced-but-absent | create glossary / appendix / reference placeholders from templates |
| 2 — Links | broken/abs paths | recompute relative paths; convert absolute → relative |
| 3 — Element IDs | legacy/invalid IDs | re-derive `PRD.NN.SS.xxxx` (section number + content hash); drop legacy `PRD.NN.xxxx`, `FR-XXX`/`US-XXX`/`AC-XXX`/`F-XXX` prefixes; fix `SS`≠host-section mismatches |
...
**Element ID re-derivation:** `PRD.{doc_id}.{section_id}.{hash}` — assign a
...
field extraction is defined for BRD §7 only (Phase 2+ for PRD). Preserve an
existing valid ID — re-deriving one breaks every downstream citation.
Authority: `framework/governance/ID_NAMING_STANDARDS.md`. `section_id` = the section the element lives in. Document-level refs
(`SPEC-NN`, `ADR-NN`, `IPLAN-NN`) stay in dash form.

**Tiered upstream drift** (BRD source): <5% change → Tier 1 auto-merge (patch
bump); 5–15% → Tier 2 auto-merge + detailed changelog (minor bump); >15% →
Tier 3 archive current + regenerate via autopilot (major bump). Never delete
upstream-removed content — mark `[DEPRECATED]` and retain for traceability.
...
| `manual-required` | domain content cannot be inferred (unresolved TODO/TBD, §10 messaging, persona detail) |

## Content-Preservation Rules

- Never delete existing product content; insert template blocks only where a
...
- Renumber only within the same section file; flag if a cross-reference anchor
  would break.
- For diagram-contract findings, edit only diagram declarations and Mermaid
  blocks — never touch requirement content.
...
Write `PRD-NN.F_fix_report_vNNN.md` with: **Summary** (issues in / fixed /
...
- Audit (input): `../doc-prd-audit/SKILL.md` · Create: `../doc-prd/SKILL.md`
- Orchestration: `../doc-prd-autopilot/SKILL.md` · IDs: `../doc-naming/SKILL.md`
