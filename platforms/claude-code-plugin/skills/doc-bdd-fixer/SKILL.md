...
2. **Resolve responsible lenses per finding.** Each blocking finding
   ...
   - **No-persona / orphan finding** (empty or missing `personas`):
     dispatch the BDD crew's author lens (per
     `REVIEW_CREWS.yaml`: `qa_lead` for BDD) as the
   ...
   BDD crew weights: `{qa_lead: 35, tech_lead: 25, chaos_engineer: 14,
   security_engineer: 6, operator: 10, auditor: 10}`.
...
4. **Validate non-regression.** For each responsible lens identified
   ...
   `.aidoc/review/04_BDD/<BDD-id>/<persona>.fix_<N>.json` (`<N>` =
...
6. **Dispatch the synthesizer once**
   ...
   `BDD-NN.F_fix_report_vNNN.md` with both the Fixes Applied table
...
**Element ID re-derivation:** `BDD.{doc_id}.{section_id}.{hash}` — assign a
...
field extraction is defined for BRD §7 only (Phase 2+ for BDD). Preserve an
existing valid ID — re-deriving one breaks every downstream citation.
Authority: `framework/governance/ID_NAMING_STANDARDS.md`. Document-level refs (`SPEC-NN`, `ADR-NN`, `IPLAN-NN`) stay in dash
