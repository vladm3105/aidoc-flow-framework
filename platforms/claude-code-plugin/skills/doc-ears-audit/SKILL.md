...
1. **Prepare the blackboard.** `mkdir -p .aidoc/review/03_EARS/<EARS-id>/`
   where `<EARS-id>` is the EARS's short artifact ID (e.g. `EARS-01`), not
   the nested folder name. This keeps blackboard paths stable when
   slugs change.
2. **Read the EARS crew** from
   `${CLAUDE_PLUGIN_ROOT}/framework/governance/REVIEW_CREWS.yaml` —
   `{requirements_specialist: 35, tech_lead: 25, qa_lead: 20, chaos_engineer: 12, security_engineer: 8}`. Weights sum to 100. Rationale: chaos-heavy split (12 > 8) — EARS layer's
   failure-mode AC emphasis over abuse-case ACs;
...
4. **Fan out.** Dispatch one `Task` subagent per lens (`subagent_type=`
   ...
   - The slot path `.aidoc/review/03_EARS/<EARS-id>/<lens>.json`
...
6. **Dispatch the synthesizer.** Run a `Task` subagent
   ...
   **both** companion files (per `agents/synthesizer.md` §"Output"):
   - `.aidoc/review/03_EARS/<EARS-id>/verdict.json` — the authoritative
     machine-readable verdict (`combined_status`, `content_score`,
     `structural_status`, `coverage.*`, `blocking_findings_count`,
     `lens_scores`).
   - `.aidoc/review/03_EARS/<EARS-id>/report.md` — the human narrative
     (mirrors verdict.json values).
7. **Compose the combined audit report.** Read
   `.aidoc/review/03_EARS/<EARS-id>/verdict.json` and `report.md`. The
   final audit report at `.aidoc/audit/03_EARS-audit.md` contains: (a)
   ...
   line surfacing `coverage.quorum_met` for consumers (`doc-ears-fixer`,
   `doc-ears-autopilot`).
...
Report: .aidoc/audit/03_EARS-audit.md
```

Read `combined_status`, `content_score`, `structural_status`, and
`coverage.quorum_met` from `verdict.json`. **Do NOT echo the EARS's
self-claimed BDD-Ready score** (the value the EARS document writes into
...
lens (requirements_specialist / tech_lead / qa_lead / chaos_engineer / security_engineer) sequentially in
...
```sh
Bash: mkdir -p .aidoc/review/03_EARS/<EARS-id>/ && date +%s > .aidoc/review/03_EARS/<EARS-id>/.skill-start.audit
```
...
```sh
Bash: echo $(( $(date +%s) - $(cat .aidoc/review/03_EARS/<EARS-id>/.skill-start.audit) ))
```
...
If `.aidoc/review/03_EARS/<EARS-id>/saga.json` does NOT exist (e.g., a
user runs `/aidoc-flow:doc-ears-audit` directly outside the autopilot
...
Output: `.aidoc/audit/03_EARS-audit.md`, with sections — **Summary** (ID,
...
consumes the `.aidoc/audit/03_EARS-audit.md` report.
...
- Create: `../doc-ears/SKILL.md` · Fix: `../doc-ears-fixer/SKILL.md` · Generate:
  `../doc-ears-autopilot/SKILL.md`
- Authority: `${CLAUDE_PLUGIN_ROOT}/framework/layers/03_EARS/README.md`,
  `${CLAUDE_PLUGIN_ROOT}/framework/layers/03_EARS/EARS-TEMPLATE.yaml`,
