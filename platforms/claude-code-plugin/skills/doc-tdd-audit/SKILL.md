...
1. **Prepare the blackboard.** `mkdir -p .aidoc/review/07_TDD/<TDD-id>/`
   where `<TDD-id>` is the TDD's short artifact ID (e.g. `TDD-01`), not
   the nested folder name. This keeps blackboard paths stable when
   slugs change.
2. **Read the TDD crew** from
   `${CLAUDE_PLUGIN_ROOT}/framework/governance/REVIEW_CREWS.yaml` —
   `{qa_lead: 35, tech_lead: 25, chaos_engineer: 10, security_engineer: 10, operator: 10, auditor: 10}`. Weights sum to 100. Rationale: equal chaos/security split (10/10) — security_engineer co-owns SECTEST per its agent brief; failure-test cases balance security-test cases;
...
3a. **Load the layer-and-lens playbook.** For each lens in the crew,
   ...
   `${CLAUDE_PLUGIN_ROOT}/../../framework/playbooks/07_TDD/<lens>.md`.
...
4. **Fan out.** Dispatch one `Task` subagent per lens (`subagent_type=`
   ...
   - The slot path `.aidoc/review/07_TDD/<TDD-id>/<lens>.json`
...
6. **Dispatch the synthesizer.** Run a `Task` subagent
   ...
   **both** companion files (per `agents/synthesizer.md` §"Output"):
   - `.aidoc/review/07_TDD/<TDD-id>/verdict.json` — the authoritative
     machine-readable verdict (`combined_status`, `content_score`,
     `structural_status`, `coverage.*`, `blocking_findings_count`,
     `lens_scores`).
   - `.aidoc/review/07_TDD/<TDD-id>/report.md` — the human narrative
     (mirrors verdict.json values).
7. **Compose the combined audit report.** Read
   `.aidoc/review/07_TDD/<TDD-id>/verdict.json` and `report.md`. The
   final audit report at `.aidoc/audit/07_TDD-audit.md` contains: (a)
   ...
   line surfacing `coverage.quorum_met` for consumers (`doc-tdd-fixer`,
   `doc-tdd-autopilot`).
...
Report: .aidoc/audit/07_TDD-audit.md
```

Read `combined_status`, `content_score`, `structural_status`, and
`coverage.quorum_met` from `verdict.json`. **Do NOT echo the TDD's
self-claimed IPLAN-Ready score** (the value the TDD document writes into
...
```sh
Bash: mkdir -p .aidoc/review/07_TDD/<TDD-id>/ && date +%s > .aidoc/review/07_TDD/<TDD-id>/.skill-start.audit
```
...
```sh
Bash: echo $(( $(date +%s) - $(cat .aidoc/review/07_TDD/<TDD-id>/.skill-start.audit) ))
```
...
If `.aidoc/review/07_TDD/<TDD-id>/saga.json` does NOT exist (e.g., a
user runs `/aidoc-flow:doc-tdd-audit` directly outside the autopilot
