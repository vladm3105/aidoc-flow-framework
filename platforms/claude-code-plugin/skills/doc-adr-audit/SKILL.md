...
`.aidoc/audit/05_ADR-audit.md` and an optional fix-cycle trigger.
...
(`.aidoc/audit/05_ADR-audit.md`) overwritten in place each run — no version cleanup
needed. Keep `ADR-NN.F_fix_report_v*.md` and `.drift_cache.json`.
...
review → 4) merge/normalize findings → 5) write `.aidoc/audit/05_ADR-audit.md`
...
```sh
Bash: mkdir -p .aidoc/review/05_ADR/<ADR-id>/ && date +%s > .aidoc/review/05_ADR/<ADR-id>/.skill-start.audit
```
...
```sh
Bash: echo $(( $(date +%s) - $(cat .aidoc/review/05_ADR/<ADR-id>/.skill-start.audit) ))
```
...
If `.aidoc/review/05_ADR/<ADR-id>/saga.json` does NOT exist (e.g., a
user runs `/aidoc-flow:doc-adr-audit` directly outside the autopilot
...
Output: `.aidoc/audit/05_ADR-audit.md`, with sections — **Summary** (ID,
...
consumes the `.aidoc/audit/05_ADR-audit.md` report.
