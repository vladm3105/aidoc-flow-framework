...
`.aidoc/audit/04_BDD-audit.md` and an optional fix-cycle trigger.
...
(`.aidoc/audit/04_BDD-audit.md`) overwritten in place each run — no version cleanup
needed. Keep `BDD-NN.F_fix_report_v*.md` and `.drift_cache.json`.
...
review → 4) merge/normalize findings → 5) write `.aidoc/audit/04_BDD-audit.md`
...
1. **Prepare the blackboard.** `mkdir -p .aidoc/review/04_BDD/<BDD-id>/`
   where `<BDD-id>` is the BDD's short artifact ID (e.g. `BDD-01`), not
   the nested folder name. This keeps blackboard paths stable when
   slugs change.
2. **Read the BDD crew** from
   `${CLAUDE_PLUGIN_ROOT}/framework/governance/REVIEW_CREWS.yaml` —
   `{qa_lead: 35, tech_lead: 25, chaos_engineer: 14, security_engineer: 6, operator: 10, auditor: 10}`. Weights sum to 100. Rationale: chaos-heavy split (14 > 6) — BDD layer's
   failure-scenario emphasis over abuse-case scenarios;
...
4. **Fan out.** Dispatch one `Task` subagent per lens (`subagent_type=`
   ...
   - The slot path `.aidoc/review/04_BDD/<BDD-id>/<lens>.json`
...
6. **Dispatch the synthesizer.** Run a `Task` subagent
   ...
   **both** companion files (per `agents/synthesizer.md` §"Output"):
   - `.aidoc/review/04_BDD/<BDD-id>/verdict.json` — the authoritative
     ...
     `lens_scores`).
   - `.aidoc/review/04_BDD/<BDD-id>/report.md` — the human narrative
     (mirrors verdict.json values).
7. **Compose the combined audit report.** Read
   `.aidoc/review/04_BDD/<BDD-id>/verdict.json` and `report.md`. The
   final audit report at `.aidoc/audit/04_BDD-audit.md` contains: (a)
   ...
   line surfacing `coverage.quorum_met` for consumers (`doc-bdd-fixer`,
   `doc-bdd-autopilot`).
...
Report: .aidoc/audit/04_BDD-audit.md
```

Read `combined_status`, `content_score`, `structural_status`, and
`coverage.quorum_met` from `verdict.json`. **Do NOT echo the BDD's
self-claimed ADR-Ready score** (the value the BDD document writes into
...
lens (qa_lead / tech_lead / chaos_engineer / security_engineer / operator / auditor) sequentially in
...
```sh
Bash: mkdir -p .aidoc/review/04_BDD/<BDD-id>/ && date +%s > .aidoc/review/04_BDD/<BDD-id>/.skill-start.audit
```
...
```sh
Bash: echo $(( $(date +%s) - $(cat .aidoc/review/04_BDD/<BDD-id>/.skill-start.audit) ))
```
...
If `.aidoc/review/04_BDD/<BDD-id>/saga.json` does NOT exist (e.g., a
user runs `/aidoc-flow:doc-bdd-audit` directly outside the autopilot
...
Output: `.aidoc/audit/04_BDD-audit.md`, with sections — **Summary** (ID,
...
consumes the `.aidoc/audit/04_BDD-audit.md` report.
