...
metadata:
  ...
  custom_fields:
    ...
    version: "0.4.5"                  # DEFAULT = plugin VERSION (see §3)
    framework_spec_version: "0.11.3"  # = FRAMEWORK_SPEC_VERSION
...
- A skill's `version` **defaults to the plugin `VERSION`** (currently `0.4.5`)
...
- [ ] `version: "0.4.5"`, `framework_spec_version: "0.11.3"` present.
