...
Deprecated in v0.4.0 but still ship as redirect stubs under `skills/` (marked `deprecated: true`, scheduled for hard removal in v0.7.0): `doc-review`, `trace-check` — folded into `doc-validator` (scope=prose and traceability pass respectively).
...
metadata:
  ...
  custom_fields:
    ...
    version: "0.17.1"                  # DEFAULT = plugin VERSION (see §3)
    framework_spec_version: "0.20.1"  # = FRAMEWORK_SPEC_VERSION
...
- A skill's `version` **defaults to the plugin `VERSION`** (currently `0.17.1`)
...
- [ ] `version: "0.17.1"`, `framework_spec_version: "0.20.1"` present.
