...
Deprecated in v0.4.0 but still ship as redirect stubs under `skills/` (marked `deprecated: true`, scheduled for hard removal in v1.0.0): `doc-review`, `trace-check` — folded into `doc-validator` (scope=prose and traceability pass respectively).
...
metadata:
  ...
  custom_fields:
    ...
    version: "0.23.4"                  # DEFAULT = plugin VERSION (see §3)
    framework_spec_version: "0.39.0"  # = FRAMEWORK_SPEC_VERSION
...
- A skill's `version` **defaults to the plugin `VERSION`** (currently `0.23.4`)
...
- [ ] `version: "0.23.4"`, `framework_spec_version: "0.39.0"` present.
