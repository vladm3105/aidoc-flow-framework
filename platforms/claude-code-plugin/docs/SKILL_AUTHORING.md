...
metadata:
  ...
  custom_fields:
    ...
    version: "0.16.1"                  # DEFAULT = plugin VERSION (see §3)
    framework_spec_version: "0.19.1"  # = FRAMEWORK_SPEC_VERSION
...
- A skill's `version` **defaults to the plugin `VERSION`** (currently `0.16.1`)
...
- [ ] `version: "0.16.1"`, `framework_spec_version: "0.19.1"` present.
