...
metadata:
  ...
  custom_fields:
    ...
    version: "0.17.0"                  # DEFAULT = plugin VERSION (see §3)
    framework_spec_version: "0.20.0"  # = FRAMEWORK_SPEC_VERSION
...
- A skill's `version` **defaults to the plugin `VERSION`** (currently `0.17.0`)
...
- [ ] `version: "0.17.0"`, `framework_spec_version: "0.20.0"` present.
