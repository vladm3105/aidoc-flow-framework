...
Deprecated in v0.4.0 but still ship as redirect stubs under `skills/` (marked `deprecated: true`, scheduled for hard removal in v0.6.0): `doc-review`, `trace-check` — folded into `doc-validator` (scope=prose and traceability pass respectively).
...
metadata:
  ...
  custom_fields:
    ...
    version: "0.5.0"                  # DEFAULT = plugin VERSION (see §3)
    framework_spec_version: "0.12.0"  # = FRAMEWORK_SPEC_VERSION
...
- A skill's `version` **defaults to the plugin `VERSION`** (currently `0.5.0`)
...
- [ ] `version: "0.5.0"`, `framework_spec_version: "0.12.0"` present.
