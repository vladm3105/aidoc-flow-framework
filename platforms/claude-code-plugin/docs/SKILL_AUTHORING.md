...
metadata:
  ...
  custom_fields:
    ...
    version: "0.22.0"                  # DEFAULT = plugin VERSION (see §3)
    framework_spec_version: "0.23.1"  # = FRAMEWORK_SPEC_VERSION
...
- A skill's `version` **defaults to the plugin `VERSION`** (currently `0.22.0`)
...
- [ ] `version: "0.22.0"`, `framework_spec_version: "0.23.1"` present.
