...
metadata:
  ...
  custom_fields:
    ...
    version: "0.25.0"                  # DEFAULT = plugin VERSION (see §3)
    framework_spec_version: "0.41.3"  # = FRAMEWORK_SPEC_VERSION
...
- A skill's `version` **defaults to the plugin `VERSION`** (currently `0.25.0`)
...
- [ ] `version: "0.25.0"`, `framework_spec_version: "0.41.3"` present.
