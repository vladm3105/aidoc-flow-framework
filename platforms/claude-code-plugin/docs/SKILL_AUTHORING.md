...
metadata:
  ...
  custom_fields:
    ...
    version: "0.9.0"                  # DEFAULT = plugin VERSION (see §3)
    framework_spec_version: "0.14.2"  # = FRAMEWORK_SPEC_VERSION
...
- A skill's `version` **defaults to the plugin `VERSION`** (currently `0.9.0`)
...
- [ ] `version: "0.9.0"`, `framework_spec_version: "0.14.2"` present.
