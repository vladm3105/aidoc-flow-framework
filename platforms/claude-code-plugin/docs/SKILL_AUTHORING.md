...
metadata:
  ...
  custom_fields:
    ...
    version: "0.6.2"                  # DEFAULT = plugin VERSION (see §3)
    framework_spec_version: "0.13.0"  # = FRAMEWORK_SPEC_VERSION
...
- A skill's `version` **defaults to the plugin `VERSION`** (currently `0.6.2`)
...
- [ ] `version: "0.6.2"`, `framework_spec_version: "0.13.0"` present.
