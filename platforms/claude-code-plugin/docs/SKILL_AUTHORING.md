...
metadata:
  ...
  custom_fields:
    ...
    version: "0.13.1"                  # DEFAULT = plugin VERSION (see §3)
    framework_spec_version: "0.17.0"  # = FRAMEWORK_SPEC_VERSION
...
- A skill's `version` **defaults to the plugin `VERSION`** (currently `0.13.1`)
...
- [ ] `version: "0.13.1"`, `framework_spec_version: "0.17.0"` present.
