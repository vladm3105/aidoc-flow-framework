...
vendored `framework/` bundle re-generated by `tools/sync-plugin-framework.sh`
(123 files, byte-identical to canonical per
`tests/conformance/platforms/test_plugin_framework_bundle.py`).
`platforms/claude-code-plugin/VERSION` stays `0.25.0`.

One agent file changed outside the bundle: `agents/requirements-analyst.md`
presented `@adr: ADR-NN` as the canonical cumulative-upstream form, which
framework **GD-13** reconciles — ADR is element-declaring, so an upstream `@adr`
trace citation is `ADR.NN.SS.xxxx`; the dash form is reserved for an ADR's own
self-tag and for `@depends:`. No skill and no plugin code changed.

What the bundle carries from framework **GD-13**: `09_CHG` named as an
operational namespace rather than a 9th lifecycle layer; `change_source: spec`
stated as always `>= C2` routing to GATE-SPEC; a five-step bubble-up procedure in
`GATE-CODE_IMPLEMENTATION.md` §6; and six surfaces reconciled to GD-03's citation
granularity (`ID_NAMING_STANDARDS.md`, `TRACEABILITY.md`, the ADR and TDD auditor
playbooks, `IPLAN-TEMPLATE.yaml`, and the agent file above).

What the bundle now carries, from framework **GD-12** — corrections to the CHG
gate documents, all of them in `governance/chg/`. `GATE_APPROVAL_FORM.md` gains
the `GATE-03-E008` and `GATE-SPEC-W003` rows it was missing (both
Security-category, both defined and catalogued since `817d9a1a` but never added
to the form a reviewer fills in), corrects its §2.2 upstream-tag counts from
2/3/4 to the registry's 1/1/2, and gains a §2 note routing a bubble-up to a
separate dependent CHG. `GATE-03_REQUIREMENTS_ARCHITECTURE.md` and
`GATE_ERROR_CATALOG.md` stop stating the ADR requirement as the full
`@brd @prd @ears @bdd` chain, and now say explicitly that `@prd`/`@brd` above
the required set are optional provenance E007 does not fail on.
`GATE-CODE_IMPLEMENTATION.md` §6.2 names the form the new CHG carries, and
`CHG-TEMPLATE.yaml`'s Feedback routing cell states its full reach including ADR.

> The `0.41.0` entry below states `FRAMEWORK_SPEC_VERSION` `0.40.0 → 0.41.0`.
> That was true when it shipped; this entry supersedes it. The `0.41.0` text is
> not edited — the changelog is append-only.

### Changed — re-pinned to framework spec `0.41.0` (2026-08-16)

`FRAMEWORK_SPEC_VERSION` `0.40.0 → 0.41.0`, and the vendored `framework/` bundle
re-generated by `tools/sync-plugin-framework.sh` (123 files, byte-identical to
canonical per `tests/conformance/platforms/test_plugin_framework_bundle.py`). No
plugin code, skill or agent changed and `platforms/claude-code-plugin/VERSION`
stays `0.25.0`.

What the bundle now carries, from framework **GD-11**: `saga.schema.json` accepts
registry-valid 3+ digit document IDs; `governance/TAG_SYNTAX.md` **defines**
`@chg: CHG-NN` as a provenance back-reference (not a trace tag), which the CHG
auditor's C1 had required at P1 while it was defined nowhere; and the PRD
auditor's C3 mandatory-section list matches `PRD-TEMPLATE.yaml`. The fourth
GD-11 item, `AI_ASSISTANT_RULES.md`, is **not** vendored to any platform
(`tools/sync-plugin-framework.sh` `ROOT_FILES` carries only
`SPEC_DRIVEN_DEVELOPMENT_GUIDE.md`), so it does not reach this bundle — no plugin
surface cites it, and the wider gap is tracked as
[#513](https://github.com/vladm3105/aidoc-flow-framework/issues/513).

> The `0.25.0` entry below states `FRAMEWORK_SPEC_VERSION` stays `0.40.0`. That
> was true when it shipped; this entry supersedes it. The `0.25.0` text is not
> edited — the changelog is append-only.
