...
Each (layer, lens) pair has a **playbook** — a layer-specific reasoning frame plus a deterministic checklist of evidence checks. Playbooks live at `framework/playbooks/<NN>_<LAYER>/<lens>.md` (one file per lens per layer; ~45 files total across the 10 layers).
...
**CHG layer (CHG-RT-001, 2026-06-12)** — CHG is an overlay (not a lifecycle layer per `framework/layers/09_CHG/README.md`) but uses the same playbook contract. Playbooks at `framework/playbooks/09_CHG/` cover the CHG crew (integration_lead/architect/chaos_engineer/operator/auditor/security_engineer with weights 30/20/15/15/10/10 summing to 100). the CHG audit engine injects them per the per-lens dispatch pattern.
