...
### Per-document `framework_version`

Each SDD document carries `metadata.framework_version` — the `framework/VERSION`
at creation time. This is the document's "build dependency" (same convention as
software: a binary records the library version it was built against). When the
framework bumps, existing documents retain their old `framework_version` until a
CHG touches them. The CHG's `version_action` field (`keep` or `upgrade`) decides
whether the document is rewritten to the new version. Conformance stays
per-platform, not per-document — the per-document field is an audit trail.
