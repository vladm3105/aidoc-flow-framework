...
- Its `metadata.framework_version` matches the current `framework/VERSION` (set
  at creation time; updated via `version_action` in a CHG when the framework bumps).
...
- All YAML files **parse without errors** after every edit (`python3 -c "import yaml; yaml.safe_load(open('file.yaml'))"` must exit 0).

## Change-level (a change to the spec)

A change to the framework spec (`framework/**` — templates, governance, registry,
`VERSION`) is **Done** when it has **passed GATE-SPEC** (`layers/09_CHG/gates/GATE-SPEC_FRAMEWORK.md`):
...
- `layers/09_CHG/gates/GATE-SPEC_FRAMEWORK.md` + `DECISIONS.md` (GD-01) — the spec-change gate.
- `TRACEABILITY.md` — the necessary-upstream tag chain a review checks.
- `DOC_GOVERNANCE_CORE.md` — Mandatory SDD sync (§3.2), IPLAN file_manifest accuracy
  (§3.5), TDD↔IPLAN cross-layer consistency (§6.10).
