...
- **IPLAN Lifecycle (Layer 8):** `Draft` | `Approved` | `In Progress` | `Completed` | `Verified`
...
## SDD Reference Integrity Rules

These rules prevent hallucinated cross-references and ensure traceability
between SDD layers.

### Source IDs must be passed to subagents

When delegating SDD document creation to subagents, the delegation prompt
MUST include the actual source document IDs. Never ask a subagent to generate
cross-references without the real upstream IDs.
...
**Forbidden:** Generating IDs like `EARS.09.00.xxx` or `BDD.09.01.xxx` — these
are hallucinated references that don't match any real document.

### Post-delegation reference validation

After a subagent completes an SDD document, the parent agent MUST validate all
cross-references before integrating the output:

1. `grep '@ears:' <file>` — verify each EARS ID exists in the upstream EARS
   document
2. `grep '@bdd:' <file>` — verify each BDD ID exists in the upstream BDD
   document
3. `grep '@prd:' <file>` — verify each PRD ID exists in the upstream PRD
   document
4. `grep '@brd:' <file>` — verify each BRD ID exists in the upstream BRD
   document
5. `grep '@adr:' <file>` — verify each ADR ID exists in the upstream ADR
   document
6. `grep -i 'ers\.' <file>` — check for `EARS` typos (missing 'A')
7. `sort | uniq -d` on ID lists — check for duplicate IDs within the file

**Never skip this step.** The cost of validating is ~2 minutes; the cost of
propagating fake references through 7 SDD layers is hours of rework.

### EARS pattern verification

When authoring EARS requirements, verify the pattern matches the requirement
semantics:
...
**Common mistake:** Using `IF` for liveness/always-on checks. "Process is
alive" is a normal state (`WHILE`), not an error condition (`IF`).

### Archive before ID reuse

Before assigning an SDD ID that may have been used in a prior session, check
if a file with that ID already exists. If it exists and is from a different
scope, archive it to `docs/sdd/09-CHG/archive/CHG-SDD09-fix/` before creating
the new document.

### Manual ID generation tracking

When creating documents directly (not delegated), maintain a running set of
assigned IDs to prevent duplicates:

1. Before assigning a new ID, check all previously assigned IDs in the current
   file
2. Use `sort | uniq -d` on the ID list to verify no duplicates
3. For BDD scenarios specifically: each EARS requirement maps to exactly one
   BDD scenario — verify 1:1 mapping

---

## SDD Authoring Quality Rules

### Element ID Format (MANDATORY)

All element IDs MUST follow the zero-padded format:
```
{TYPE}.{doc_id:02d}.{section_id:02d}.{hash:04h}
```
- `doc_id`: Two-digit zero-padded (01, 02, ... 07)
- `section_id`: Two-digit zero-padded (04, 07, 08, 09, 10, 12, 13)
- `hash`: 4-char lowercase hex (SHA256 of normalized title+description)

**Valid:** `BRD.01.07.a841`, `PRD.03.09.f3ed`
**Invalid:** `BRD.1.7.a841`, `PRD.3.9.f3ed`, `BRD.01.07.g7h8`

### Capability Promotion Pipeline (CANONICAL)

The capability promotion pipeline is defined once in AGENTS.md and must be
referenced, not redefined, in all SDD documents:
```
BACKTEST → REPLAY → SHADOW → PAPER → DRAFT_ONLY → CONFIRM_EXECUTION
```
BRD/PRD glossary entries MUST use this exact definition. Do not abbreviate or
modify.

### Seed Disposition Completeness

Every seed document claim MUST have exactly one disposition entry in the
earliest BRD that absorbs it. Validation:
- Read each seed doc (`docs/seed/`)
- Verify every claim appears in some BRD's `seed_disposition:` section
- Misattributions (wrong seed doc cited) are errors

### Cross-BRD Scope Handoff Validation

When authoring BRD-N's `out_of_scope`, verify that deferred items are
explicitly covered by a later BRD's `in_scope`. If BRD-01 defers "Knowledge
graph to V2", BRD-02 MUST have "Knowledge graph" in its scope.

### Glossary Consistency

Duplicate glossary terms across BRDs MUST have identical definitions. If a
term appears in multiple BRDs, use the definition from the earliest BRD that
defines it.

### @depends Tag Semantics

- `@depends: BRD-XX` in `traceability.tags` = direct predecessor only (linear
  chain)
- `cross_brd_dependencies` in `traceability` = all transitive ancestors
- BRD-01 has no @depends (it's the foundation)

### Seed Source Attribution

Seed disposition claims MUST cite the correct seed document:
- Architecture claims → `docs/seed/architecture/ARCH-*.md`, `SAFE-*.md`,
  `INFRA-*.md`, `DOM-*.md`
- Agent surface claims → `docs/seed/agent-surface/AGENT-*.md`
- NEVER cite downstream SDD layers (MODULE-*, PRD-*, etc.) as seed sources

### Subagent Output Validation

When subagents generate SDD content, validate before writing:
- Element IDs must be computed via SHA256, not fabricated
- YAML must parse without errors
- FR count must not exceed 5 per document
- BRD references must resolve to existing element IDs
   BDD scenario — verify 1:1 mapping
