...
| ID | Meaning | Severity | Contract |
|----|---------|----------|----------|
| `ID01` | Malformed trace-tag id (`@<layer>: <id>` value does not match the id grammar). | error | `ID_NAMING_STANDARDS.md`, `TAG_SYNTAX.md` |
| `ID02` | Malformed document id (does not match `id_patterns.document`). | error | `registry/LAYER_REGISTRY.yaml`, `ID_NAMING_STANDARDS.md` |
| `ID03` | Malformed element id (does not match `id_patterns.element`). | error | `registry/LAYER_REGISTRY.yaml`, `ID_NAMING_STANDARDS.md` |
