...
| `EVAL-COV-004` | An EVAL `coverage_matrix.summary.total` does not match the actual count of entries in `test_design.test_cases` (parsed via YAML parser). Counts must be recomputed after final YAML write, not set before. Added after b-local-privy EVAL-01/02 both had wrong counts. | error | `EVAL-TEMPLATE.yaml` §coverage_matrix |
