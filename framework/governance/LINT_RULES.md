...
| `GOV-013` | Code implementation was attempted without an IPLAN. The IPLAN Gate (§3.13) requires an IPLAN with status `In Progress` before any code files may be modified. The IPLAN must reference the authorizing CHG and list the files being modified. Added after CHG-10 had code implemented before IPLAN existed. | error | `DOC_GOVERNANCE_CORE.md` §3.13 IPLAN Gate |
