...
| `CHG-L011` | Cited EARS/BDD IDs exist — every EARS/BDD ID cited in the CHG must exist in the referenced EARS/BDD document. Exact match (error) when the tree is verifiable via `--sdd-root` — with an empty lifecycle the pool falls back to the root's EARS/BDD layer directories, and missing directories warn as unverifiable rather than erroring; nearby-heuristic (warning) otherwise, matching `{BASE}-*.yaml/md` documents. Governance alias: `GOV-017`. | error (exact) / warning (heuristic) | `DOC_GOVERNANCE_CORE.md` §3.4.1 D21/D22 |
