...
class UCRemPhase:
    ...
    def generate_fixes(
        ...
        *,
        ...
        self._enforce_prd_remediation_contract(doc_path, review_report)
    ...
    def _enforce_prd_remediation_contract(self, doc_path: Path, review_report: Path) -> None:
        """Enforce PLAN-012 PRD remediation inputs for API callers.

        PRD remediation must operate on a validation-fixed PRD copy and use a
        UCX review report as the instruction source for LLM-generated fixes.
        """
        is_prd_file = (
            doc_path.is_file()
            and doc_path.suffix.lower() == ".md"
            and doc_path.name.startswith("PRD-")
        )
        if not is_prd_file:
            return
        ...
        if stage != "validation-fixed":
            raise ValueError(
                "PRD remediation must use a _validation PRD copy "
        ...
        if ".UCX_review_report_" not in review_report.name:
            raise ValueError(
                "PRD remediation requires a UCX review report "
                "(*.UCX_review_report_vNNN.md).\n"
