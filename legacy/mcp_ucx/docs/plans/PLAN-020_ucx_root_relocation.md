...
**Goal**: Move `{project}/docs/UCX/` → `{project}/UCX/` across mcp_ucx codebase and all scaffolded projects.

**Status**: Implemented (2026-04-02, mcp_ucx v1.10.0 / framework v0.17.0)
...
| Path simplicity | `docs/UCX/logs/mcp_ucx.log` | `UCX/logs/mcp_ucx.log` |
...
Grep of `docs/UCX` across the entire mcp_ucx codebase: **~100 references across 22 files**.
...
15. Migrate existing projects:
    - `/opt/data/ucx_framework/`: `mv docs/UCX UCX`
...
19. Create mcp_ucx CHANGELOG v1.10.0 and ROADMAP entry
...
1. `python -m pytest mcp_ucx/tests/unit/ -v` — all pass
2. `python -m pytest mcp_ucx/tests/integration/ -v` — all pass
3. `grep -r "docs/UCX" mcp_ucx/src/` returns 0 results (no hardcoded paths)
4. `grep -r "docs/UCX" mcp_ucx/tests/` returns 0 results
...
7. Logging writes to `UCX/logs/mcp_ucx.log`
...
| Third-party tools expecting docs/UCX | None known; UCX is internal to mcp_ucx |
...
- Other projects beyond ucx_framework and b-local-docs
