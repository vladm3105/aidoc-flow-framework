...
#   ./run_ucr.sh prd docs/02_PRD/PRD-01/PRD-01_platform_architecture_validation.md
#
# Arguments:
...
if [[ -z "$DOC_TYPE" || -z "$DOC_PATH" ]]; then
    ...
    echo "  ./run_ucr.sh prd docs/02_PRD/PRD-01/PRD-01_platform_architecture_validation.md"
    echo ""
...
# PLAN-012: PRD reviews must run on the validation-fixed copy.
if [[ "$DOC_TYPE" == "prd" ]]; then
    if [[ -f "$DOC_PATH" ]]; then
        if [[ "$DOC_PATH" == *_validation.md || "$DOC_PATH" == *_remediated.md ]]; then
            :
        else
            REVIEW_TARGET="${DOC_PATH%.md}_validation.md"
            if [[ -f "$REVIEW_TARGET" ]]; then
                echo "PLAN-012: switching review target to validation copy: $REVIEW_TARGET"
                DOC_PATH="$REVIEW_TARGET"
            else
                echo "Error: PLAN-012 requires PRD review on a _validation artifact."
                echo "Expected: $REVIEW_TARGET"
                echo "Run: ucx validate-fix prd $DOC_PATH"
                exit 1
            fi
        fi
    elif [[ -d "$DOC_PATH" ]]; then
        mapfile -t VALIDATION_FILES < <(find "$DOC_PATH" -maxdepth 1 -type f -name 'PRD-*_validation.md' | sort)
        if [[ ${#VALIDATION_FILES[@]} -eq 1 ]]; then
            DOC_PATH="${VALIDATION_FILES[0]}"
            echo "PLAN-012: using validation copy from folder: $DOC_PATH"
        else
            echo "Error: PLAN-012 requires exactly one PRD _validation artifact when using a directory path."
            echo "Found ${#VALIDATION_FILES[@]} matching files in: $DOC_PATH"
            echo "Provide an explicit file path ending with _validation.md"
            exit 1
        fi
    fi
fi
