...
class UCCPhase:
    ...
    def _build_output_contract(self, doc_type: DocType, output_path: Path) -> str:
        ...
        if doc_type == DocType.PRD:
            ...
            contract.append(
                "- Priority notation MUST use MoSCoW labels only (`Must`, `Should`, `Could`, `Won't`)\n"
            )
            contract.append(
                "- FORBIDDEN priority tokens: `P0`, `P1`, `P2`, `P3`, `P4`, and compound forms like `P1-Must`\n"
            )
            contract.append(
                "- Apply size budgeting: keep source draft concise and target <=850 lines to avoid corpus-size warnings\n"
            )
            contract.append(
                "- Prefer compact tables and concise bullets; avoid verbose narrative duplication across sections\n"
            )
    ...
    def _apply_prd_output_guardrails(self, content: str, output_path: Path) -> str:
        ...
        body = self._normalize_prd_priority_notation(body)
    ...
    def _normalize_prd_priority_notation(self, body: str) -> str:
        """Normalize PRD priority notation to MoSCoW labels."""
        map_code = {
            "0": "Must",
            "1": "Must",
            "2": "Should",
        ...
        # Normalize compound forms first (e.g., P1-Must -> Must).
        body = re.sub(
            r"\bP([0-4])-(Must|Should|Could|Won'?t)\b",
            lambda m: map_code[m.group(1)],
            body,
            flags=re.IGNORECASE,
        )

        # Normalize remaining bare numeric priority tokens (e.g., P1 -> Must).
        body = re.sub(r"\bP([0-4])\b", lambda m: map_code[m.group(1)], body)
        return body
