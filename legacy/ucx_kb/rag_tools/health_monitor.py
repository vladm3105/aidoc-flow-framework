#!/usr/bin/env python3
"""Health monitor for ucx_kb prerequisites."""
...
def check_ucx_kb_mcp(host: str = "localhost", port: int = 8101) -> ServiceStatus:
    """Check optional ucx_kb MCP endpoint health."""
    try:
...
def check_python_package() -> ServiceStatus:
    """Check if ucx_kb package is importable."""
    try:
        __import__("ucx_kb")
        return ServiceStatus("PythonPackage", True, "ucx_kb import OK")
    except Exception as e:
...
def check_all(verbose: bool = False) -> list[ServiceStatus]:
    ...
    statuses = [
        ...
        check_ucx_kb_mcp(host=mcp_host, port=mcp_port),
    ]
