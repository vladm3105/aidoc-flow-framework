...
from ucx_kb.rag.exceptions import EmbedError
...
class TestEmbedDocument:
    ...
    @patch("ucx_kb.rag.embed.is_real_document")
    ...
    @patch("ucx_kb.rag.embed.is_real_document", return_value=True)
    @patch("ucx_kb.rag.embed._get_document_by_id")
    @patch("ucx_kb.rag.embed._compute_file_hash")
    ...
    @patch("ucx_kb.rag.embed.is_real_document", return_value=True)
    @patch("ucx_kb.rag.embed._get_document_by_id", return_value=None)
    @patch("ucx_kb.rag.embed._compute_file_hash", return_value="abc123")
    @patch("ucx_kb.rag.embed.chunk_yaml_document")
    @patch("ucx_kb.rag.embed.get_embeddings_batch")
    @patch("ucx_kb.rag.embed._store_document")
    @patch("ucx_kb.rag.embed._log_embed")
    @patch("pathlib.Path.exists", return_value=True)
    @patch(
        "builtins.open",
    ...
    def test_embeds_new_document(
        ...
        from ucx_kb.rag.models import ChunkResult
...
class TestEmbedText:
    ...
    @patch("ucx_kb.rag.embed.get_embedding")
    @patch("ucx_kb.rag.embed._store_document")
...
class TestReembedAll:
    ...
    @patch("psycopg.connect")
    @patch("ucx_kb.rag.embed.embed_document")
    @patch("pathlib.Path.exists", return_value=True)
