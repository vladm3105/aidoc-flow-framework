...
| File | Change |
|------|--------|
| `mcp_ucx/src/mcp_server/remediation/runner.py` | Enriched `_build_remediate_fix_prompt`, added phased finding groups, priority-sorted cap, new `verify_remediation_quality()` |
| `mcp_ucx/src/mcp_server/remediation/review_parser.py` | `recommended_action` truncation 300 to 2000 chars |
| `mcp_ucx/src/mcp_server/tool_registry.py` | Post-fix validation in pipeline, quality check in remediate_fix handler |
| `mcp_ucx/skills/persona_mappings.yaml` | `fact_checker` added to BRD review sequence |
| `mcp_ucx/skills/personas/chairperson.md` | Applicability Veto (principle #4) |
| `mcp_ucx/skills/personas/auditor.md` | APPLICABILITY CHECK guard in Compliance section |
