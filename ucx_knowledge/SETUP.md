...
```bash
python ucx_knowledge/scripts/backfill_legacy.py --source /path/to/legacy --dry-run
```

Execute:

```bash
python ucx_knowledge/scripts/backfill_legacy.py --source /path/to/legacy --pattern "*.yaml"
```
...
```bash
python ucx_knowledge/rag_tools/health_monitor.py -v
python ucx_knowledge/rag_tools/query_router.py "what requirements exist" --analyze-only
python ucx_knowledge/rag_tools/batch_indexer.py --source ucx_knowledge --dry-run --service both
```
...
| `ModuleNotFoundError: ucx_knowledge` | `PYTHONPATH` not set to framework root | `export PYTHONPATH=/opt/data/ucx_framework` |
...
- [ ] Export `PYTHONPATH`
- [ ] Start `ucx_knowledge.mcp.server`
