#!/usr/bin/env python3
"""Batch indexer for ucx_knowledge contract jobs.
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Generate ucx_knowledge indexing jobs")
    parser.add_argument("--source", required=True, help="Source directory to scan")
    parser.add_argument("--service", choices=["rag", "graph", "both"],
                       default="both", help="Which ucx_knowledge pipeline to target")
    parser.add_argument("--out", default="ucx_knowledge/tmp/jobs", help="Output directory for job files")
