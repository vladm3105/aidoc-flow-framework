#!/usr/bin/env python3
"""Query router for ucx_knowledge contracts.

Routes queries to ucx_knowledge contract types based on query intent:
...
if "ucx_knowledge" not in sys.modules:
    repo_root = Path(__file__).resolve().parents[2]
    if str(repo_root) not in sys.path:
        sys.path.insert(0, str(repo_root))

from ucx_knowledge.models.contracts import GraphContextRequest, SearchRequest
...
def classify_query(query: str) -> RoutingDecision:
    ...
    if max_score == 0:
        # Default to ucx_knowledge RAG search for unknown queries
        return RoutingDecision(
            service="rag",
            query_type=QueryType.UNKNOWN,
            confidence=0.5,
            reasoning="No clear indicators, defaulting to ucx_knowledge RAG search"
        )
    ...
    if query_type == QueryType.FACTUAL:
        service = "rag"
        reasoning = "Factual/documentation query - using ucx_knowledge RAG search"
    else:
        service = "graph"
        reasoning = f"{query_type.value.title()} query - using ucx_knowledge graph context"
...
def execute_query(query: str, decision: RoutingDecision) -> dict:
    """Build ucx_knowledge contract payload for the selected route.
    ...
    Returns:
        Dict payload compatible with ucx_knowledge contracts.
    """
...
def main():
    parser = argparse.ArgumentParser(description="Route queries to ucx_knowledge contracts")
