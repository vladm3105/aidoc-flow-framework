...
def _to_int(value: Any, default: int) -> int:
    """Convert configured numeric value to int with fallback."""
    try:
        return int(value)
...
MAX_TOKENS = _to_int(
    _get_config_value("max_tokens", _chunk_config.get("max_tokens", 768), "CHUNK_MAX_TOKENS"),
    768,
)
MIN_TOKENS = _to_int(
    _get_config_value("min_tokens", _chunk_config.get("min_tokens", 50), "CHUNK_MIN_TOKENS"),
    50,
)
OVERLAP_TOKENS = _to_int(
    _get_config_value(
        "overlap_tokens",
        _chunk_config.get("overlap_tokens", 150),
        "CHUNK_OVERLAP",
    ),
    150,
)
