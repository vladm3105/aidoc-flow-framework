...
from ucx_knowledge.rag.embed import (
    _compute_file_hash,
    _infer_doc_type,
    _parse_date,
    delete_document,
...
from ucx_knowledge.rag.exceptions import EmbedError
...
class TestEmbedDocument:
    ...
    @patch("ucx_knowledge.rag.embed.is_real_document")
    ...
    @patch("ucx_knowledge.rag.embed.is_real_document", return_value=True)
    @patch("ucx_knowledge.rag.embed._get_document_by_id")
    @patch("ucx_knowledge.rag.embed._compute_file_hash")
    ...
    @patch("ucx_knowledge.rag.embed.is_real_document", return_value=True)
    @patch("ucx_knowledge.rag.embed._get_document_by_id", return_value=None)
    @patch("ucx_knowledge.rag.embed._compute_file_hash", return_value="abc123")
    @patch("ucx_knowledge.rag.embed.chunk_yaml_document")
    @patch("ucx_knowledge.rag.embed.get_embeddings_batch")
    @patch("ucx_knowledge.rag.embed._store_document")
    @patch("ucx_knowledge.rag.embed._log_embed")
    @patch("pathlib.Path.exists", return_value=True)
    @patch(
        "builtins.open",
    ...
    def test_embeds_new_document(
        ...
        from ucx_knowledge.rag.models import ChunkResult
...
class TestEmbedText:
    ...
    @patch("ucx_knowledge.rag.embed.get_embedding")
    @patch("ucx_knowledge.rag.embed._store_document")
...
class TestReembedAll:
    ...
    @patch("psycopg.connect")
    @patch("ucx_knowledge.rag.embed.embed_document")
    @patch("pathlib.Path.exists", return_value=True)
