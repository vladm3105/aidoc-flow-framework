...
def mock_neo4j():
    ...
    with patch("ucx_knowledge.graph.layer.GraphDatabase") as mock_db:
...
def mock_ollama():
    ...
    with patch("ucx_knowledge.graph.extract._call_ollama_rate_limited") as mock:
        mock.return_value = "[]"
        yield mock
...
def mock_config():
    ...
    with patch("ucx_knowledge.graph.extract._config", config):
        yield config
...
def test_client():
    ...
    from ucx_knowledge.graph.webhook import app
...
def mock_trading_graph():
    ...
    with patch("ucx_knowledge.graph.webhook.TradingGraph") as mock_class:
...
def mock_extract_document():
    ...
    with patch("ucx_knowledge.graph.webhook.extract_document") as mock:
        yield mock
...
def mock_extract_text():
    ...
    with patch("ucx_knowledge.graph.webhook.extract_text") as mock:
        yield mock
