...
|---------|---------|-------------|
| `VALIDATION_v1_archive/` | 11 stale validation/schema/splitting docs | Unified YAML templates + mcp_sdd |
| `scripts_v1_archive/` | 37 per-layer validation scripts | mcp_sdd `sdd_validate` |
| `automation_v1_archive/` | AI_EXPERTS + pipelines | mcp_sdd tools |
| Root `scripts/` | 2 standalone scripts | mcp_sdd `sdd_validate_links` |
...
- Validation: per-layer scripts → mcp_sdd `sdd_validate` / `sdd_validate_links`
...
- mcp_sdd: 186 passed (18 new link validation tests), 0 regressions
