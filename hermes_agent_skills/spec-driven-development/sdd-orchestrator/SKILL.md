...
If the governance docs are stale after a UCX framework sync, re-sync with the `update-sdd-from-ucx` skill.
...
- Framework: `ucx_flow_v3/05_ADR/ADR-TEMPLATE.yaml`
...
The SDD v3.2 framework lives at `/opt/data/ucx_framework/ucx_flow_v3/`.
...
**Template sync procedure**: When syncing Hermes local templates with UCX canonical sources, do NOT sed-only replace the server header. Copy fresh from `ucx_flow_v3/` first, THEN stamp `server: ucx_hermes`. See `references/ucx-sdd-bridge/template-sync-procedure.md` and `scripts/ucx-sdd-bridge/sync-ucx-templates.sh`.

### Update SDD from UCX
One-command synchronization of Hermes SDD skills/templates from the UCX v3.2 framework source.

**File mapping** (UCX → Hermes):
- Layer templates (8): `ucx_flow_v3/0N_TYPE/TYPE-TEMPLATE.yaml` → `templates/0N_TYPE-TEMPLATE.yaml`
- ucx_flow_v3 references: `README.md`, `AI_ASSISTANT_RULES.md`, `DOC_GOVERNANCE_CORE.md`, etc. → `references/`
- Root documentation: `README.md`, `HERMES_UCX_RUNTIME_ENVIRONMENT.md`, etc. → `root-docs/`
- Governance: Recursive copy of `governance/` tree preserving directory structure

**Safety rules**: Never delete from Hermes during sync. Only copy and overwrite. Always validate after copy. Backup log written to `references/.sync-backlog.json`.

**Pitfall**: UCX `@brd:` keys in YAML are unquoted and break `yaml.safe_load()`. Flagged during validation but not auto-fixed — fix upstream in UCX repo.
