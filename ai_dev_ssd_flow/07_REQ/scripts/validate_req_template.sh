...
if [ ! -f "$REQ_FILE" ]; then
  echo "[FAIL] File not found: $REQ_FILE"
  exit 1
fi
...
for i in "${!required_fields[@]}"; do
  if grep -q "${required_fields[$i]}" "$REQ_FILE"; then
    echo "  [PASS] Found: ${field_names[$i]}"
  else
    echo "  [FAIL] MISSING: ${field_names[$i]}"
    ((ERRORS++))
  fi
done
...
if grep -q "### .*Upstream Sources" "$REQ_FILE"; then
  echo "  [PASS] Found: Upstream Sources subsection"
else
  echo "  [FAIL] MISSING: Upstream Sources subsection in Section 11"
  ((ERRORS++))
fi

if grep -q "### .*Downstream Artifacts" "$REQ_FILE"; then
  echo "  [PASS] Found: Downstream Artifacts subsection"
else
  echo "  [WARN]  WARNING: Downstream Artifacts subsection recommended"
  ((WARNINGS++))
fi

if grep -q "### .*Code Implementation Paths" "$REQ_FILE"; then
  echo "  [PASS] Found: Code Implementation Paths subsection"
else
  echo "  [WARN]  WARNING: Code Implementation Paths subsection recommended"
  ((WARNINGS++))
fi
...
if [[ $version =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
  echo "  [PASS] Version format valid: $version (semver X.Y.Z)"
else
...
if [[ $date_created =~ ^[0-9]{4}-[0-9]{2}-[0-9]{2}$ ]]; then
  echo "  [PASS] Date Created format valid: $date_created"
else
  echo "  [FAIL] INVALID Date Created format: '$date_created' (expected YYYY-MM-DD)"
  ((ERRORS++))
fi

if [[ $date_updated =~ ^[0-9]{4}-[0-9]{2}-[0-9]{2}$ ]]; then
  echo "  [PASS] Last Updated format valid: $date_updated"
else
  echo "  [FAIL] INVALID Last Updated format: '$date_updated' (expected YYYY-MM-DD)"
  ((ERRORS++))
fi

# Compare dates
if [[ "$date_updated" < "$date_created" ]]; then
  echo "  [FAIL] ERROR: Last Updated ($date_updated) is before Date Created ($date_created)"
  ((ERRORS++))
fi
...
if echo "$priority_line" | grep -qE "(Critical|High|Medium|Low) \(P[1-4]\)"; then
  echo "  [PASS] Priority format valid (includes P-level)"
else
  echo "  [WARN]  WARNING: Priority should include P-level: Critical (P1), High (P2), Medium (P3), Low (P4)"
  ((WARNINGS++))
fi
...
if echo "$source_doc" | grep -qE "[A-Z]+-[0-9]+ Section [0-9]"; then
  echo "  [PASS] Source Document format valid: $source_doc"
else
  echo "  [WARN]  WARNING: Source Document should include section number (e.g., 'SYS-02 Section 3.1.1')"
  ((WARNINGS++))
fi
...
if echo "$spec_ready_line" | grep -qE "[PASS].*[0-9]+%"; then
  score=$(echo "$spec_ready_line" | grep -oE "[0-9]+%" | head -1 | tr -d '%')
  if [ "$score" -ge 90 ]; then
    echo "  [PASS] SPEC-Ready Score ≥ 90%: $score%"
  else
    echo "  [WARN]  WARNING: SPEC-Ready Score below 90%: $score%"
    ((WARNINGS++))
  fi
else
  echo "  [FAIL] MISSING: SPEC-Ready Score with [PASS] emoji and percentage"
  ((ERRORS++))
fi
...
if [ -n "$ctr_ready_line" ]; then
  if echo "$ctr_ready_line" | grep -qE "[PASS].*[0-9]+%"; then
    ctr_score=$(echo "$ctr_ready_line" | grep -oE "[0-9]+%" | head -1 | tr -d '%')
    if [ "$ctr_score" -ge 90 ]; then
      echo "  [PASS] CTR-Ready Score ≥ 90%: $ctr_score%"
    else
      echo "  [WARN]  WARNING: CTR-Ready Score below 90%: $ctr_score%"
      ((WARNINGS++))
    fi
  else
    echo "  [WARN]  WARNING: CTR-Ready Score present but format invalid (expected [PASS] XX%)"
    ((WARNINGS++))
  fi
else
  echo "  ℹ  CTR-Ready Score not provided (optional for MVP)"
fi
...
if [ -n "$template_version" ]; then
  if [ "$template_version" = "1.1" ]; then
    echo "  [PASS] Template Version is 1.1"
  else
    echo "  [WARN]  WARNING: Template Version is '$template_version' (expected 1.1 for MVP)"
    ((WARNINGS++))
  fi
else
  echo "  ℹ  Template Version not provided (optional for MVP)"
fi
...
echo "-----------------------------------------"
echo "  ℹ  Change History is not included in MVP 11-section format; skipping"
...
if [[ $filename =~ ^REQ-[0-9]{2,}([-.][0-9]{2,3})?_[a-z0-9_]+\.md$ ]]; then
  echo "  [PASS] Filename format valid: $filename"
  ...
  if echo "$h1_header" | grep -qE "^# ${file_id}:"; then
    echo "  [PASS] H1 header ID matches filename: $file_id"
  else
    echo "  [FAIL] ERROR: H1 header ID doesn't match filename"
    echo "           Filename ID: $file_id"
    echo "           H1 Header: $h1_header"
    ((ERRORS++))
  fi
else
  echo "  [FAIL] ERROR: Invalid filename format: $filename"
...
if [ "$template_version" = "1.1" ]; then
  ...
  if echo "$h1_line" | grep -q '\['; then
    ...
    if [ -n "$resource_tag" ]; then
      echo "  [PASS] Resource tag found in H1 header: $resource_tag"
    else
      echo "  [FAIL] ERROR: Failed to extract resource tag from H1"
      echo "           Current H1: $h1_line"
      ((ERRORS++))
    fi
  else
    echo "  [FAIL] ERROR: Template 1.1 requires [RESOURCE_INSTANCE] tag in H1"
    echo "           Current H1: $h1_line"
  ...
  echo "  ℹ  Resource tag check skipped (Template Version not 1.1)"
fi
...
if [[ ${#missing_tags[@]} -gt 0 ]]; then
  ...
  while IFS= read -r tag_line; do
    if ! [[ $tag_line =~ ^@(brd|prd|ears|bdd|adr|sys):[[:space:]]([A-Z]+-[0-9]{2,}|[A-Z]+\.[0-9]{2,}(\.[0-9]+)*)(:[A-Z0-9_-]+)? ]]; then
      echo "  [FAIL] ERROR: Invalid tag format: $tag_line"
  ...
  if [[ $tag_format_errors -gt 0 ]]; then
    ...
    echo "  [PASS] All tag formats valid (@type: DOC-ID:REQ-ID)"
  fi
fi
...
for type in "${required_upstream[@]}"; do
  ...
  if [ $found -eq 0 ]; then
    echo "  [PASS] Found: $type upstream source"
  else
...
if [[ ${#missing_upstream[@]} -gt 0 ]]; then
  echo "  [FAIL] ERROR: Incomplete upstream chain - missing: ${missing_upstream[*]}"
  echo "           Complete chain required: BRD → PRD → EARS → BDD → ADR → SYS"
  echo "           Reference: REQ-MVP-TEMPLATE.md Section 15 (RULE 2)"
  ((ERRORS++)) || true
else
  echo "  [PASS] Complete upstream traceability chain (all 6 layers)"
fi
...
while IFS= read -r link; do
  ...
  if [ ! -f "$resolved_path" ]; then
    echo "  [FAIL] ERROR: Broken link - file not found"
    echo "           Link: $path"
    ...
    if ! grep -qE "^#{1,6} ${anchor_part}:|^#{1,6} .*${anchor_part}" "$resolved_path"; then
      echo "  [WARN]  WARNING: Anchor possibly missing in $(basename "$file_part"): #$anchor_part"
      ((WARNINGS++))
    fi
  fi
...
if [ $broken_links -gt 0 ]; then
  echo "  [FAIL] Found $broken_links broken links (checked $checked_links links)"
  ((ERRORS += broken_links))
elif [ $checked_links -gt 0 ]; then
  echo "  [PASS] All markdown links resolve correctly ($checked_links links checked)"
else
  echo "  ℹ  No markdown links found in document"
fi
...
if grep -q "### .*Traceability Matrix\|### 11.4\|## 11.4\|### 10.4\|## 10.4" "$REQ_FILE"; then
    echo "  [PASS] Traceability Matrix section found (complex REQ)"
    echo "     Upstream sources: $upstream_count, Sub-components: $has_subcomponents"
  else
    echo "  [WARN]  WARNING: Complex REQ detected but Section 11.4 (Traceability Matrix) missing"
  ...
  echo "  ℹ  Simple REQ - Traceability Matrix optional"
  echo "     Upstream sources: $upstream_count, Sub-components: $has_subcomponents"
fi
...
if [ -n "$score" ] && [ "$score" -ge 90 ]; then
  ...
  if grep -qE "class.*Protocol|class.*ABC|from typing import Protocol|from abc import.*ABC" "$REQ_FILE"; then
    echo "  [PASS] Section 3.4: Protocol/ABC class found"
  else
    echo "  [FAIL] ERROR: SPEC-Ready ≥90% but no Protocol/ABC class in Section 3.4"
    ((content_warnings++))
  fi
  ...
  if grep -qE "BaseModel|@dataclass|from pydantic import|from dataclasses import dataclass" "$REQ_FILE"; then
    echo "  [PASS] Section 4: Pydantic/dataclass models found"
  else
    echo "  [FAIL] ERROR: SPEC-Ready ≥90% but no Pydantic/dataclass models in Section 4"
    ((content_warnings++))
  fi

  # Section 5.3: Exception Definitions
  if grep -qE "class.*Error\(.*Exception\)|class.*Error\(.*Error\)" "$REQ_FILE"; then
    echo "  [PASS] Section 5.3: Exception definitions found"
  else
    echo "  [FAIL] ERROR: SPEC-Ready ≥90% but no exception definitions in Section 5.3"
    ((content_warnings++))
  fi
  ...
  if grep -qE "\`\`\`yaml|\`\`\`yml" "$REQ_FILE"; then
    if grep -q "### .*Configuration Schema" "$REQ_FILE" || grep -q "7.3 Configuration Schema" "$REQ_FILE"; then
       echo "  [PASS] Section 7.3: YAML configuration found"
    else
       # Fallback for old templates if yaml exists but not explicitly in 7.3
       echo "  [WARN]  WARNING: YAML configuration found but not in Section 7.3"
    fi
  else
    echo "  [FAIL] ERROR: SPEC-Ready ≥90% but no YAML configuration in Section 7.3"
    ((content_warnings++))
  fi

  if [ $content_warnings -gt 0 ]; then
    echo "  [FAIL] SPEC-Ready content validation: $content_warnings ERRORS"
    echo "           Required: Protocol/ABC, Pydantic Models, Exceptions, YAML Config"
    ((ERRORS += content_warnings))
  else
    echo "  [PASS] SPEC-Ready content validation complete - all critical sections present"
  fi
else
  echo "  ℹ  SPEC-Ready score <90% - content validation skipped"
fi
  ...
  if grep -q "### 8.1 Unit Tests" "$REQ_FILE"; then
    ...
    if grep -q "| Test Case | Input | Expected Output | Coverage |" "$REQ_FILE"; then
       ...
       if [ "$row_count" -ge 3 ]; then
         echo "  [PASS] Found: $row_count Unit Test entries (min 3)"
         
         # Check for category prefixes
         if echo "$section_content" | grep -qE "\[Logic\]|\[State\]|\[Validation\]|\[Edge\]"; then
           echo "  [PASS] Found: Category prefixes ([Logic], [State], etc.)"
         else
           echo "  [WARN]  WARNING: Unit Tests entries missing category prefix [Logic/State/Validation/Edge]"
           ((WARNINGS++))
         fi
       else
         echo "  [WARN]  WARNING: Unit Tests table has < 3 entries (found: $row_count, required: ≥3)"
         ((WARNINGS++))
       fi
    else
       echo "  [WARN]  WARNING: Unit Tests table missing required columns: Test Case | Input | Expected Output | Coverage"
       ((WARNINGS++))
    fi
  else
    echo "  [WARN]  WARNING: Section 8.1 missing 'Unit Tests' title - drives SPEC interface design"
    ((WARNINGS++))
  fi
else
  echo "  ℹ  Standard Profile: Skipping Unit Tests check"
fi
...
if [ $ERRORS -eq 0 ] && [ $WARNINGS -eq 0 ]; then
  echo "[PASS] PASSED: All validation checks passed with no warnings"
  echo ""
  ...
  echo "[WARN]  PASSED WITH WARNINGS: Document valid but has $WARNINGS warnings"
  echo ""
  ...
  echo "[FAIL] FAILED: $ERRORS critical errors found"
  echo ""
