...
**Required Structure**: `docs/10_TSPEC/SECTEST/SECTEST-NN_{slug}/SECTEST-NN_{slug}.md`

**Validation**:
1. Check document is inside a nested folder
2. Verify folder name matches SECTEST ID pattern
3. Verify file name matches folder name
4. Parent path must be: `docs/10_TSPEC/SECTEST/`

**This check is BLOCKING** - SECTEST must pass folder structure validation before other checks proceed.
...
### SECTEST-Specific Tags

| Tag | Pattern | Required |
|-----|---------|----------|
| `@sec` | `SEC\.\d{2,}\.\d{2}` | Yes |
| `@ctr` | `CTR-\d{2,}` | Recommended |
| `@sys` | `SYS\.\d{2,}\.\d{2}\.\d{2,}` | Recommended |
| `@spec` | `SPEC-\d{2,}` | Yes |
