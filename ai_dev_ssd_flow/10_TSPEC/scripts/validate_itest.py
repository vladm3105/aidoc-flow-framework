...
class ITESTValidator:
    ...
    def _collect_issues(self, gate_scores: dict) -> list:
        ...
        if not self.has_ctr_tag:
            if HAS_ERROR_CODES:
                issues.append(format_error("TSPEC-E007", "missing @ctr traceability tag"))
            else:
                issues.append("Missing @ctr traceability tag")

        if not self.has_spec_tag:
            if HAS_ERROR_CODES:
                issues.append(format_error("TSPEC-E007", "missing @spec traceability tag"))
            else:
        ...
        for tc in self.test_cases:
            if not tc.ctr_ref:
                if HAS_ERROR_CODES:
                    issues.append(format_error("ITEST-E003", tc.id))
                else:
                    issues.append(f"{tc.id}: Missing @ctr reference")

            if not tc.has_contract_table:
                if HAS_ERROR_CODES:
                    issues.append(format_error("TSPEC-E009", f"{tc.id} - missing contract compliance table"))
                else:
                    issues.append(f"{tc.id}: Missing contract compliance table")

            if tc.component_count > 1 and not tc.has_sequence_diagram:
                if HAS_ERROR_CODES:
                    issues.append(format_error("ITEST-E002", tc.id))
                else:
                    issues.append(f"{tc.id}: Missing sequence diagram for multi-component test")

        # Quality gate checks
        if gate_scores.get("ctr_coverage", 0) < 90:
            if HAS_ERROR_CODES:
                score = gate_scores.get("ctr_coverage", 0)
                issues.append(format_error("ITEST-E001", f"CTR coverage {score:.1f}% < 90%"))
            else:
                issues.append(f"GATE ctr_coverage: Score {gate_scores.get('ctr_coverage', 0):.1f}% below threshold")

        for gate, score in gate_scores.items():
            if gate != "ctr_coverage" and score < self.WARN_THRESHOLD:
                if HAS_ERROR_CODES:
    ...
    def _collect_warnings(self) -> list:
        ...
        if not self.has_sys_tag:
            if HAS_ERROR_CODES:
                warnings.append(format_warning("TSPEC-E010", "missing @sys tag (recommended)"))
            else:
        ...
        for tc in self.test_cases:
            if not tc.has_side_effects:
                if HAS_ERROR_CODES:
                    warnings.append(format_warning("ITEST-W001", f"{tc.id} - missing side effects"))
                else:
