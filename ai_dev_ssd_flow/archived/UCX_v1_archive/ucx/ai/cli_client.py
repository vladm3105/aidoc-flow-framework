...
class CLIClient(BaseAIClient):
    ...
    # Per-agent preflight contract. Keep probes minimal and deterministic.
    PREFLIGHT_PROBE_CONFIG: dict[str, dict[str, str]] = {
        "claude": {
            "format": "epoch",
            "prompt": "Return ONLY a 10-digit Unix epoch timestamp for current UTC time. No prose.",
            "retry_prompt": "Return ONLY one token: a 10-digit Unix epoch timestamp for current UTC time.",
        },
        "codex": {
            "format": "iso",
            "prompt": "Return ONLY today's UTC date in YYYY-MM-DD format. No prose.",
            "retry_prompt": "Return ONLY one token in this format: YYYY-MM-DD.",
        },
        "gemini": {
            "format": "iso",
            "prompt": "Return ONLY today's UTC date in YYYY-MM-DD format. No prose.",
    ...
    def _run_availability_preflight(self, return_details: bool = False, **kwargs):
        ...
        # ── Phase 3: Strict date probe (tool-adaptive) ──────────────────────
        self.logger.debug("Preflight Phase 3: strict date probe (%s)", self.cli_tool)
        expected_utc_date = datetime.datetime.now(datetime.timezone.utc).date()
        probe = self.PREFLIGHT_PROBE_CONFIG.get(
            self.cli_tool,
            {
                "format": "epoch",
                "prompt": self.PREFLIGHT_PROMPT,
                "retry_prompt": "Return ONLY one token: YYYY-MM-DD or a 10-digit UTC epoch timestamp.",
            },
        )

        response = self._execute_cli(probe["prompt"], system_prompt=None, **kwargs).strip()
        ...
        detected_date = self._extract_strict_preflight_date(
            response,
            probe["format"],
        )

        # One strict retry can recover transient instruction-following drift.
        if detected_date != expected_utc_date:
            retry_response = self._execute_cli(
                probe["retry_prompt"],
                system_prompt=None,
                **kwargs,
            ).strip()

            retry_embedded_error = self._detect_embedded_cli_error(retry_response)
            if retry_embedded_error is not None:
                raise AIClientError(
                    "LLM availability preflight failed: CLI returned error-like text response on retry: "
                    f"{retry_embedded_error}",
                    model=self.cli_tool,
                )

            retry_detected_date = self._extract_strict_preflight_date(
                retry_response,
                probe["format"],
            )
            if retry_detected_date is not None:
                detected_date = retry_detected_date
    ...
    def _extract_strict_preflight_date(
        self,
        text: str,
        expected_format: str,
    ) -> Optional[datetime.date]:
        """Parse a strict single-value preflight response into a UTC date.

        Accepts exactly one normalized scalar token after trimming markdown wrappers.
        Supported formats: ``epoch`` (10/13 digits) and ``iso`` (YYYY-MM-DD).
        """
        token = self._extract_single_value_token(text)
        if token is None:
            return None

        if expected_format == "epoch":
            if not re.fullmatch(r"\d{10}|\d{13}", token):
                return None
            return self._extract_epoch_utc_date(token)

        if expected_format == "iso":
    ...
    def _extract_single_value_token(self, text: str) -> Optional[str]:
        """Extract a single scalar token from model output.

        Rules:
        - Remove markdown fences/backticks and surrounding whitespace.
        - Keep only non-empty lines.
        - Require exactly one non-empty line and one whitespace-delimited token.
        """
        if not text:
            return None
