---
title: "ADR Quality Gate Validation"
tags:
...
# ADR Quality Gate Validation Rules
...
| **Quality Gate Validation** | After ALL ADR complete | Entire ADR set | This document |
...
| Category | Pattern | Quality Gate Validation |
|----------|---------|-------------------|
...
This section documents errors discovered during Quality Gate validation. Copy and populate for each project.
...
echo "=== ADR Quality Gate Validation ==="
...
Use this template to track Quality Gate validation progress:
...
name: ADR Quality Gate Validation
...
# ADR Quality Gate validation on staged ADR files
if git diff --cached --name-only | grep -q "^docs/05_ADR/"; then
  echo "Running ADR Quality Gate validation..."
  ./scripts/validate_adr_corpus.sh docs/ADR --errors-only
  if [ $? -ne 0 ]; then
    echo "ADR Quality Gate validation failed. Fix errors before committing."
    exit 1
  fi
  echo "ADR Quality Gate validation passed"
fi
```
...
ADR Quality Gate validation should pass before creating SYS documents:
