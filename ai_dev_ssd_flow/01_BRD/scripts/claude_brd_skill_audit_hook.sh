...
for file_path in "$@"; do
  ...
  report_file="${REPORT_DIR}/${base_name}.doc-brd-audit.txt"

  echo "[INFO] Skill audit: /doc-brd-audit ${resolved_file_path}"
  if ! timeout 120 claude -p "/doc-brd-audit ${resolved_file_path}" --output-format text >"${report_file}" 2>&1; then
...
echo "[PASS] Claude skill audit completed for BRD files."
