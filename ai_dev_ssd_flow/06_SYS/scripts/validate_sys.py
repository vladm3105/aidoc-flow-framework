...
def validate_metadata(metadata: Optional[Dict], result: ValidationResult, is_template: bool = False):
    ...
    if status is None and legacy_status is not None:
        if legacy_status in LEGACY_STATUS_VALUES:
            ...
            result.add_warning("SYS-W005", "Legacy custom_fields.development_status detected; migrate to custom_fields.status")
        else:
            result.add_error("SYS-E002", f"Invalid legacy development_status: '{legacy_status}'")
    elif legacy_status is not None:
        result.add_warning("SYS-W005", "Both status and legacy development_status detected; status is authoritative")
