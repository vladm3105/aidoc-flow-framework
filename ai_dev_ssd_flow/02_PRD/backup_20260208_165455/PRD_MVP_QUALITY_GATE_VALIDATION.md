---
title: "PRD Quality Gate Validation"
tags:
...
# PRD Quality Gate Validation Rules
...
| **Quality Gate Validation** | After ALL PRDs complete | Entire PRD set | This document |
...
name: PRD Quality Gate Validation
...
jobs:
  validate-prd:
    ...
    steps:
      ...
      - name: Run PRD Quality Gate Validation
        id: validation
        run: |
        ...
        with:
          script: |
            ...
            github.rest.issues.createComment({
              ...
              body: '## PRD Quality Gate Validation Failed\n\n```\n' + output.substring(0, 60000) + '\n```'
            });
...
# Run PRD Quality Gate validation if PRD files changed
if git diff --cached --name-only | grep -q "docs/02_PRD/"; then
  echo "PRD files changed, running Quality Gate validation..."
  ./scripts/validate_prd_corpus.sh docs/PRD

  if [ $? -ne 0 ]; then
    echo "PRD Quality Gate validation failed. Fix errors before committing."
    exit 1
  fi
fi
```
