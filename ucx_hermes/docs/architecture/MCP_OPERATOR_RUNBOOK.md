...
- If `--out` is omitted, artifacts are written to `<document_dir>/.ucx/creation` when `--sections-json` is provided.
...
3. Confirm `{DOC_ID}.ucx.validate.json` and `{DOC_ID}.ucx.validate.txt` are written in output path.
4. When the validation report contains errors, confirm that derived `_validated` artifact and `{DOC_ID}.ucx.validate_fix.*` files are also produced automatically. These fix artifacts were previously generated by the separate `sdd_validate_fix` tool, which is now merged into `sdd_validate`.
5. Review report errors and warnings, then remediate source document if required.
...
2. Confirm remediation report artifacts `{DOC_ID}.ucx.remediate.json/.txt` are produced.
...
4. Confirm versioned `_remediate_v{N}` artifact and `{DOC_ID}.ucx.remediate_fix.*` artifacts are produced.
...
| validate fix artifacts missing from sdd_validate output | sdd_validate completed but derived `_validated` artifact or `{DOC_ID}.ucx.validate_fix.*` absent | verify document path and output path permissions; confirm validation report contains errors (fix artifacts are only produced when errors exist); rerun sdd_validate |
