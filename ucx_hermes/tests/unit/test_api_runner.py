...
def _api_config(name: str = "api/test", model: str = "test-model",
                api_key_env: str = "TEST_API_KEY", **kw) -> ExecutorConfig:
    return ExecutorConfig(
        name=name, executor_type=ExecutorType.API,
        model=model, api_key_env=api_key_env, **kw,
    )
    ...
    def test_system_prompt_included(self):
        ...
        with patch.dict("sys.modules", {"litellm": mock_lit}):
            ...
            asyncio.get_event_loop().run_until_complete(
                run_api_executor(config, "user msg", system_prompt="system msg",
                                 project_env={"TEST_API_KEY": "key"})
            )
...
class TestResolveOverrides:
    ...
    def test_api_base_override(self):
        ...
        _, api_base, _, _ = _resolve_overrides(config, {"UCX_EXECUTOR_API_BASE": "https://override.api.com"})
    ...
    def test_api_key_env_redirect(self):
        ...
        _, _, _, api_key_env = _resolve_overrides(config, {"UCX_EXECUTOR_API_KEY_ENV": "MY_CUSTOM_KEY"})
    ...
    def test_empty_overrides_use_config(self):
        from mcp_server.executor.api_runner import _resolve_overrides
        config = _api_config(model="my-model", api_base="https://api.com", timeout=300, api_key_env="MY_KEY")
...
class TestProjectExecutorConfig:
    ...
    def test_global_registry_unchanged_after_project_load(self, tmp_path):
        import json
        from mcp_server.executor.registry import load_project_executor_config, _registry
        ucx_dir = tmp_path / "UCX"
        ucx_dir.mkdir()
        (ucx_dir / "executors.json").write_text(json.dumps({
            "executors": [
    ...
    def test_array_format_accepted(self, tmp_path):
        ...
        ucx_dir.mkdir()
        (ucx_dir / "executors.json").write_text(json.dumps([
            {"name": "arr-exec", "executor_type": "api", "model": "m"}
        ]))
        result = load_project_executor_config(tmp_path)
